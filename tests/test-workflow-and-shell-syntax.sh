...
grep -q 'git-lfs' "$workflow" || fail "workflow does not install git-lfs"
grep -q 'lfs: true' "$workflow" || fail "workflow checkout does not fetch Git LFS objects"
grep -q 'persist-credentials: false' "$workflow" || fail "workflow persists checkout credentials into the work tree"
grep -q 'github.ref == '"'"'refs/heads/main'"'" "$workflow" || fail "workflow can run off main"
