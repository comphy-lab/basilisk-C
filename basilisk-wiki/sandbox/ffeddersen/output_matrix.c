...
int main(int argc, char * argv[])
{
  ...
  int dump_num_start, dump_num_end;
  double Lx, Ly;
  int NX, NY;
  char scalar_flag;
  double xsoliton;
  ...
  if (argc < 9){
    printf("Arguments: output_matrix  dump_num_start dump_num_end  Lx  NX  ymin ymax NY  scalar\n");
    return 1;
  }
  ...
  printf("OUPUT MATRIX: dnum_start=%d  dnum_end=%d, Lx=%.3lf, NX=%d, ymin=%.3lf, ymax=%.3lf, NY=%d scalar=%c\n",
	 dump_num_start,dump_num_end,Lx, NX, ymin, ymax, NY, scalar_flag);
  ...
  while(restore(file=dumpname))  {
    ...
    case 'a':
      sprintf(binaryname,"shoal_binary/dump_f_d%04d.bdat",dumpnum);
      output_matrix_ff (f, binaryname,  xmin, xmax, NX, ymin, ymax, NY);      
      sprintf(binaryname,"shoal_binary/dump_p_d%04d.bdat",dumpnum);
      output_matrix_ff (p, binaryname,  xmin, xmax, NX, ymin, ymax, NY);      
      sprintf(binaryname,"shoal_binary/dump_u_d%04d.bdat",dumpnum);
      output_matrix_ff (u.x, binaryname,  xmin, xmax, NX, ymin, ymax, NY);            
      sprintf(binaryname,"shoal_binary/dump_v_d%04d.bdat",dumpnum);
      output_matrix_ff (u.y, binaryname,  xmin, xmax, NX, ymin, ymax, NY);                  
      sprintf(binaryname,"shoal_binary/dump_z_d%04d.bdat",dumpnum);
      vorticity(u,zeta);
      output_matrix_ff (zeta, binaryname,  xmin, xmax, NX, ymin, ymax, NY);                        
      break;
  default: fprintf(stderr,"** Error: unknown scalar field\n");
