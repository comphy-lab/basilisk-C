...
/** The symmetric_bathymetry flag switches the bathymetry boundary 
    condition between a no-slip wall (false) and a symmetry plane (true)*/

bool symmetric_bathymetry = false;
bool navier_surface = false;
...
/**
The capillary and Van der Waals accelerations macros are set to zero.
These macros are overloaded in hydro-tension.h and van-der-waals.h. */
...
/** The total acceleration is the sum of the barotropic, capillary, and Van der Waals 
accelerations. All three are included even though a_cap and a_VdW are zero by default 
in order to allow the code to work with any combination of the three accelerations. 
This implementation should be improved in the future */
...
/**
Diffusion (including viscosity) is added by this code. By default
only vertical diffusion is active. cap_diffusion.h is an expanded
version of diffusion.h  */
...
/** hpg_2 is used to compute the horizontal pressure gradient for the
normal stress balance. This is necessary since hpg applies a zero pressure
at the free surface while hpg_2 applies a neumann zero condition on the 
