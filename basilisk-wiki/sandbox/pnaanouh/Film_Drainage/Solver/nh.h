...
$w$. Unlike the original solver the boundary condition for $w$ is 
$\frac{\partial w}{\partial z}= -\nabla \cdot \mathbf{u}$ instead of 
...
event viscous_term (i++)
{
  if (nu > 0.){
    ...
    /**
Top $w$ boundary condition is calculated from the $u$ boundary condion and last layer values using the multilayer continuity equation.

$w_T= w_{nl-3/2}+ \mathbf{u}_T \cdot \mathbf{{\nabla}} \eta -\mathbf{u}_{nl-3/2} \cdot \mathbf{{\nabla}} z_{nl-3/2} - \mathbf{{\nabla}} \cdot \left( h \mathbf{u} \right)_{nl-1}$
     */
    foreach(){
      wb[]=0.;
      wt[]= 0.;
	...
	if(nl>1){
	  if (navier_surface){
	    wt[] += u_t.x[]*(zl[1,0,nl-1]+h[1,0,nl-1]-zl[-1,0,nl-1]-h[-1,0,nl-1])/(2.*Delta);
	    wt[] -= ((h[0,0,nl-2]*(3.*h[0,0,nl-1]+2.*h[0,0,nl-2])*u.x[0,0,nl-1]+sq(h[0,0,nl-1])*u.x[0,0,nl-2]-h[0,0,nl-2]*(h[0,0,nl-1]+h[0,0,nl-2])*u_t.x[0,0])/sq(h[0,0,nl-1]+h[0,0,nl-2]))*(zl[1,0,nl-1]-zl[-1,0,nl-1])/(2.*Delta);
	  }else{
	    wt[] += (((3.*sq(h[0,0,nl-1])+3.*h[0,0,nl-1]*h[0,0,nl-2]+sq(h[0,0,nl-2]))*u.x[0,0,nl-1]-sq(h[0,0,nl-1])*u.x[0,0,nl-2])/((h[0,0,nl-1]+h[0,0,nl-2])*(2.*h[0,0,nl-1]+h[0,0,nl-2]))+(1./2.)*dut.x[0,0]*h[0,0,nl-1]*(h[0,0,nl-1]+h[0,0,nl-2])/(2.*h[0,0,nl-1]+h[0,0,nl-2]))*(zl[1,0,nl-1]+h[1,0,nl-1]-zl[-1,0,nl-1]-h[-1,0,nl-1])/(2.*Delta);
	    wt[] -= ((h[0,0,nl-2]*(3.*h[0,0,nl-1]+h[0,0,nl-2])*u.x[0,0,nl-1]+2.*sq(h[0,0,nl-1])*u.x[0,0,nl-2])/((h[0,0,nl-1]+h[0,0,nl-2])*(2.*h[0,0,nl-1]+h[0,0,nl-2]))-(1./2.)*dut.x[0,0]*h[0,0,nl-1]*h[0,0,nl-2]/(2.*h[0,0,nl-1]+h[0,0,nl-2]))*(zl[1,0,nl-1]-zl[-1,0,nl-1])/(2.*Delta);
	  }
	}
	else{
	  if (navier_surface){
	    ...
	    if (symmetric_bathymetry){
	      wt[] -= ((3.*u.x[0,0,nl-1]-u_t.x[0,0])/2.-h[0,0,nl-1]*dub.x[1,0]/4.)*(zl[1,0,nl-1]-zl[-1,0,nl-1])/(2.*Delta);
	    }
	    else{
	    ...
	    if (symmetric_bathymetry){
	      wt[] += (u.x[0,0,nl-1]+h[0,0,nl-1]/6.*(2.*dut.x[0,0]+dub.x[0,0]))*(zl[1,0,nl-1]+h[1,0,nl-1]-zl[-1,0,nl-1]-h[-1,0,nl-1])/(2.*Delta);
	      wt[] -= (u.x[0,0,nl-1]-h[0,0,nl-1]/6.*(dut.x[0,0]+2.*dub.x[0,0]))*(zl[1,0,nl-1]-zl[-1,0,nl-1])/(2.*Delta);
	    }
	    else{
	      wt[] += ((3.*u.x[0,0,nl-1]-u_b.x[0,0])/2.+h[0,0,nl-1]*dut.x[0,0]/4.)*(zl[1,0,nl-1]+h[1,0,nl-1]-zl[-1,0,nl-1]-h[-1,0,nl-1])/(2.*Delta);
	      wt[] -= u_b.x[0,0,nl-1]*(zl[1,0,nl-1]-zl[-1,0,nl-1])/(2.*Delta);
	    }
	  }
	}
	...
	wt[] /= (2.*h[0,0,nl-2]+h[0,0,nl-1])/(h[0,0,nl-1]+h[0,0,nl-2]);
      ...
      vertical_diffusion_NavierNavier (point, h, w, dt, nu, wt[], 0., wb[], 0.);
    }
    /**
The horizontal diffusion is also added if the h_diffusion flag is true.
     */
    if (h_diffusion)
      horizontal_diffusion_DirichletDirichlet( w, nu, dt,wt,wb);
  }
}
