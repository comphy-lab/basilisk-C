...
scalar f2[],f3[],f1[],f4[];
int nb_phases=4;
scalar * interfaces = {f2,f3,f1,f4};
double rho1 = 1., mu1 = 0., rho2 = 1., mu2 = 0., rho3 = 1., mu3 = 0., rho4 = 0., mu4 = 0.;
...
#define rho(f2,f3,f4) (clamp(1-f2-f3-f4,0,1)*rho1+clamp(f2,0,1)*rho2+clamp(f3,0,1)*rho3+clamp(f4,0,1)*rho4)
#endif

#ifndef mu
// two phases definition was mu(f)  (clamp(f,0.,1.)*(mu1 - mu2) + mu2)
// three phases definition
#define mu(f2,f3,f4)  (clamp(1-f2-f3-f4,0,1)*mu1+clamp(f2,0,1)*mu2+clamp(f3,0,1)*mu3+clamp(f4,0,1)*mu4)
#endif
...
event tracer_advection (i++) {
  ...
  foreach()
    sf4[] = (4.*f4[] + 
      2.*(f4[0,1] + f4[0,-1] + f4[1,0] + f4[-1,0]) +
      f4[-1,-1] + f4[1,-1] + f4[1,1] + f4[-1,1])/16.;
#else // dimension == 3
  foreach()
    sf4[] = (8.*f4[] +
      4.*(f4[-1] + f4[1] + f4[0,1] + f4[0,-1] + f4[0,0,1] + f4[0,0,-1]) +
      2.*(f4[-1,1] + f4[-1,0,1] + f4[-1,0,-1] + f4[-1,-1] + 
    f4[0,1,1] + f4[0,1,-1] + f4[0,-1,1] + f4[0,-1,-1] +
    f4[1,1] + f4[1,0,1] + f4[1,-1] + f4[1,0,-1]) +
      f4[1,-1,1] + f4[-1,1,1] + f4[-1,1,-1] + f4[1,1,1] +
      f4[1,1,-1] + f4[-1,-1,-1] + f4[1,-1,-1] + f4[-1,-1,1])/64.;
#endif
#endif
  ...
  sf4.prolongation = refine_bilinear;   
  //old version before 22nd of december 2021 : 
  boundary ({sf1,sf2,sf3,sf4});
#endif
