...
void vof_advection(scalar * interfaces, int i){
	...
	for (d = 0; d < dimension; d++){
		int counter_advection=0;
		scalar total_c[];
		foreach(){
			total_c[]=0;
		}
		for (j=0;j<nb_phases;j++){
			scalar c=interfaces[(j+i) % nb_phases];
		//for (scalar c in interfaces) {
			counter_advection+=1;
			scalar cc[], * tcl = NULL, * tracers = c.tracers;    
			for (scalar t in tracers) {
			...
			foreach(){
				test=total_c[]*c[];
				total_c[]+=c[];
				if (d+1==dimension && counter_advection==nb_phases && fabs(total_c[]-1)!=1e-6 && test>1e-6){
					c[]+=1-total_c[]; //crée des zones <0 depuis la permutation des c
				}	
				#if TREE
				//c.restriction = restriction_volume_average;
				//c.refine = vof_concentration_refine;
				#endif
				/*else
					for (scalar c in interfaces){
						c[]=clamp(c[],0,1);
