...
static void momentum_refine (Point point, scalar u) {
  ...
  double rhou = 0.;
  foreach_child()
    rhou += cm[]*rho(f2[],f3[],f4[])*u[];
  double du = u[] - rhou/((1 << dimension)*cm[]*rho(f2[],f3[],f4[]));
  foreach_child()
    u[] += du;
}
...
static void momentum_restriction (Point point, scalar u)
{
  ...
  foreach_child()
    rhou += cm[]*rho(f2[],f3[],f4[])*u[];
  u[] = rhou/((1 << dimension)*cm[]*rho(f2[],f3[],f4[]));
}
#endif // TREE
...
foreach_dimension()
static double boundary_q4_x (Point neighbor, Point point, scalar q4, void * data)
{
  return clamp(f4[],0,1)*rho4*u.x[];
}

foreach_dimension()
static double boundary_q1_x (Point neighbor, Point point, scalar q1, void * data)
{
  return clamp(1.-f4[]-f2[]-f3[],0,1)*rho1*u.x[];
}
...
foreach_dimension()
static void prolongation_q4_x (Point point, scalar q4) {
  foreach_child()
    q4[] = clamp(f4[],0,1)*rho4*u.x[];
}

foreach_dimension()
static void prolongation_q1_x (Point point, scalar q1) {
  foreach_child()
    q1[] = clamp(1.-f4[]-f2[]-f3[],0,1)*rho1*u.x[];
}
#endif
...
event vof (i++) {
  ...
  foreach_dimension() {
    ...
    q4.x.prolongation = prolongation_q4_x;
  }
#endif
  ...
  foreach()
    foreach_dimension() {
      ...
      double fc4 = clamp(f4[],0,1); 
      double fc1 = clamp(1-f4[]-f2[]-f3[],0,1);           
      q2.x[] = fc2*rho2*u.x[];
      q3.x[] = fc3*rho3*u.x[];      
      q1.x[] = fc1*rho1*u.x[];
      q4.x[] = fc4*rho4*u.x[];
    }
  // old version after 22/12/21 : 
     boundary ((scalar *){q1,q2,q3,q4});
...
//    q2.x.inverse = true;
    ...
    q3.x.inverse = false; 
    q4.x.inverse = false;            
    q4.x.gradient = q2.x.gradient = q3.x.gradient = q1.x.gradient = u.x.gradient;
  }
...
//  f.tracers = (scalar *){q1,q2};
  ...
  f4.tracers = (scalar *){q4};
      
  vof_advection ({f2,f3,f1,f4}, i);

  foreach()
    foreach_dimension()
      u.x[] = (q1.x[] + q2.x[] + q3.x[] + q4.x[])/rho(f2[],f3[],f4[]);
  // old version 
  boundary ((scalar *){u});
