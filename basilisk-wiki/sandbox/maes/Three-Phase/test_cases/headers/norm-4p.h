...
*/
double fractions_error (scalar f1, scalar f2, scalar f3, scalar f4, double v0){
	double res = 0.;
#if AXI  
	foreach(reduction(+:res))
	res += 2.*pi*dv()*fabs(f1[]+f2[]+f3[]+f4[] - 1.);
#else // 2D
	foreach(reduction(+:res))
	res += dv()*fabs(f1[]+f2[]+f3[]+f4[]-1);
#endif  // AXI
	return res/v0;
}
...
*/
double shape_error (scalar f1, scalar f2, scalar f3, scalar f4, scalar f1e, scalar f2e, scalar f3e, scalar f4e, double v0){
	double res = 0.;
#if AXI
	foreach(reduction(+:res))
	res += 2.*pi*dv()*(fabs(f1[]-f1e[])+fabs(f2[]-f2e[])+fabs(f3[]-f3e[]))+fabs(f4[]-f4e[]);
#else // 2D
	foreach(reduction(+:res))
	res += dv()*(fabs(f1[]-f1e[])+fabs(f2[]-f2e[])+fabs(f3[]-f3e[])+fabs(f4[]-f4e[]));  
#endif // AXI
	return res/v0;
}