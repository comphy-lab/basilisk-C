...
scalar cs[];
face vector fs[];
double robin_a=0.,robin_b=0.,robin_c=0.;
...
foreach_dimension()
static inline double robin_bc_x (Point point, scalar s, scalar cs, coord n, coord p, double alpha, double lambda, double gamma, double * coef)
{
  foreach_dimension()
    n.x = - n.x;
  double d[2], v[2] = {nodata,nodata};
  double a[3] = {nodata,nodata,nodata};
  bool defined = true;
  foreach_dimension()
  if (defined && !fs.x[(n.x > 0.)])
    defined = false;
...
// bi-quadratic interpolation
      ...
      if (v[0] == nodata) {
        d[0] = max(1e-3, fabs(p.x/n.x));
        *coef = - alpha/(lambda+alpha*d[0]*Delta);
        return gamma/(lambda+alpha*d[0]*Delta);
      }
      *coef = 0.;  
  if (v[1] != nodata){ // third-order gradient
    a[2] = d[0]/(d[1]*(d[1]-d[0])*Delta); 
    a[1] = -d[1]/(d[0]*(d[1]-d[0])*Delta); 
    a[0] = (d[1]+d[0])/(d[1]*d[0]*Delta);
    return  (gamma*a[0]+alpha*(a[1]*v[0] + a[2]*v[1]))/(alpha+lambda*a[0]);
  }
  else
  {
    return (gamma-alpha*v[0])/(lambda+alpha*d[0]*Delta); 
  }

}

double robin_bc (Point point, scalar s, scalar cs,
 coord n, coord p, double alpha, double lambda, double gamma, double * coef)
{
  foreach_dimension()
  if (fabs(n.x) >= fabs(n.y))
    return robin_bc_x (point, s, cs, n, p, alpha, lambda, gamma, coef);
  return nodata;
}

double robin_gradient (Point point, scalar s, scalar cs,
 coord n, coord p, double alpha, double lambda, double gamma, double * coef)
{
  if (s.v.x.i < 0){
    return robin_bc(point,s,cs,n,p,alpha,lambda,gamma,coef);
  } 
  else {
    assert( s.v.x.i >= 0);
    vector u = s.v;
    double dundn=0, dutdn=0;
    scalar un[], ut[];
    coord t = {-n.y, n.x}; 
    foreach(){
      un[]=0, ut[]=0;
    ...
    dutdn = robin_bc(point,ut,cs,n,p,alpha,lambda,gamma,coef);
...
double embed_flux (Point point, scalar s, face vector mu, double * val)
{
  ...
  *val = 0.;
  ...
  if (dirichlet == 2){
    normalize (&n);
    grad = robin_gradient (point, s, cs, n, p, robin_a, robin_b, robin_c, &coef);
 }
...
double dirichlet (double expr, Point point = point,
      scalar s = _s, int * data = data)
{
  return data ? embed_area_center (point, &x, &y, &z),
    *((int *)data) = 1, expr : 2.*expr - s[];
}
...
double neumann_homogeneous (double expr, Point point = point,
          scalar s = _s, int * data = data)
{
  return data ? *((int *)data) = 0, 0 : s[];
}

macro2
double robin(double alpha, double lambda, double expr,Point point = point,
          scalar s = _s, int * data = data)
{
  return (data ? embed_area_center (point, &x, &y, &z),
  *((int *)data) = 2 , robin_a = alpha , robin_b = lambda , robin_c = expr , expr :
  (((2.*Delta*(expr))/(Delta*alpha + 2.*(lambda))) + ((2.*(lambda) - Delta*alpha)/(2.*(lambda) + Delta*alpha))*s[])); //(alpha == 0 ? neumann(expr/lambda) : lmbda == 0 ? dirichlet(expr/alpha) : 
}

macro2
double robin_homogeneous(double alpha, double lambda, double expr,Point point = point, scalar s = _s, int * data = data)
{
  return (data ? *((int *)data) = 2, robin_a = alpha , robin_b = lambda , robin_c = 0, 0 :
  ( (2.*(lambda) - Delta*alpha)/(2.*(lambda) + Delta*alpha))*s[]); //alpha == 0 ? neumann_homogeneous(expr) : lambda == 0 ? dirichlet_homogeneous(expr) : (
}
/**
