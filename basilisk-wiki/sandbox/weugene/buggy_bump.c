...
event adapt (i++) {
  scalar haha[];
  foreach() haha[] = h[]*h[]; boundary((scalar*){haha});
  astats s = adapt_wavelet ({h, haha}, (double[]){1e-3, 1e-3}, LEVEL);
...
~~~literatec 
$ CC99='mpicc -std=c99' qcc -events -O2 -D_MPI=1 buggy_bump.c -lm
$ mpirun -np 2 ./a.out >out 2>log
~~~
 */