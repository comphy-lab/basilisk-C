/**
When I run the default file src/examples/bubble.c, at or near the
beginning of the simulation I see orange color in graphs for mgp.i and
mgpf.i.

Around the time the file "dump-004" was created I saw red color in
graph for mgpf.nrelax.

I started the simulation and set the computer to turn off after it
finishes. After checking the results, in file "log" I see warnings:

- first that convergence for pf was not reached,

- second that convergence for u.x was not reached,

- third that convergence for p was not reached,

- fourth that CFL must be <= 0.5 for VOF (cfl - 0.5 = 1.66753e+45)

- ...

while I didn't see a bubble in the mp4 file that was created.

Did I do something wrong or is the example set-up wrongly? When I view
some starting dump files with command bview2D, I see a bubble, so I
guess that I didn't do something wrong.

I already wrote about other examples. Maybe it would be nice if you
could warn us if the program is not going to be maintained or something
because some time is needed to learn it.*/
