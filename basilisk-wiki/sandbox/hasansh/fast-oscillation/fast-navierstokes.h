...
variable-density Navier--Stokes equations using the "constant density" method of [Dodd and
  Ferrante,
...
/**
The primary variables are the centered pressure field $p$, the auxiliary centered pressure field $pn$ for storing pressure at prevoius timestep,  and the centered velocity field $\mathbf{u}$. The centered vector field
...
face vector uf[];
/**
We define enumarator for determining projection method. For FASTPn and FASTP* methods we need $\rho_0$ which is the menimum of $\rho_1$ and $\rho_2$. If not specified by the user, this variable is set to one. 
*/
...
{
  /**
  We use switch case for projecting with on of the  usplit, fastPn, fastpstar methods which is specified by the user.
  */
  boundary ({p}); 
