 ...
 ##Constant density projection Method
   For using constant density method we need to solve a constant coefficient Poisson equation:
$$
\nabla ^2 p^{n+1}= \nabla \cdot \left[\left(1-\frac{rho_0}{\rho^{n+1}}\right)\nabla \hat{p}\right]+\frac{\rho_0}{\Delta t} \nabla \cdot u^{*}
$$ 

After solving the Poisson equation velocity field will be updated based on the new pressure field

$$
u^{n+1} = u^{*}-\Delta t \left[1/\rho_0 \nabla p^{n+1}+\left(1/rho^{n+1}-1/rho_0\right)\nabla \hat{p}\right]
$$
Notations are based on the paper of [Dodd and  Ferrante,  2014](http://dx.doi.org/10.1016/j.jcp.2014.05.024).

*/
mgstats fast_project (int method,face vector u, scalar p,scalar pn, (const) face vector alpha, double rho0, double dt)
{
 
  /**
    We initialize $\hat{p}=pn$ for fastpn method and $\hat{p}=1.5 p^n-0.5 p^{n-1}$ forst fastpstar method. 
  */   
  scalar phat[];
  switch(method){
  case 1:
...
*/
  ...
  foreach_face()
    u.x[] -= dt*(((1/rho0)*(p[] - p[-1])/(Delta))+((alpha.x[]-(1/rho0))*((phat[] - phat[-1])/(Delta))));
  boundary ((scalar *){u});   
