...
interface tracking and surface tension. Since we're using the new fast
poisson solver we include *fast-navierstokes.h* instead of
...
event amplitude (t += 0.00225584983639310905; t <= 1.66481717925811447992) {
  ...
  method *poisson_solver* and the number of grid points *N*. */
...
/**
At the end of the simulation, we output on a file, indexed with the
method *poisson_solver*, the resolution (number of grid points per
wavelength) and the relative RMS error. */
...
* [Same test with Gerris](https://gerris.dalembert.upmc.fr/gerris/tests/tests/capwave.html#density)
*/
