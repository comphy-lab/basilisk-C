					   ...
					   double * coef)
{
  ...
  double slip_length = s.lambda[];
  // fprintf(stderr, "sl, x, y %g %g %g\n", slip_length, x, y);  
  ...
  *coef = 0.;
  ...
  // if (s.lambda.i){
    // fprintf(stderr, "Lambda is defined and is %g at %g %g\n", slip_length, x, y);
  // }else{
    // fprintf(stderr, "Lambda is not defined");
  // }

  if (v[1] != nodata && slip_length < 1e-12){ // third-order gradient
    // fprintf(stderr, "if statement (sl = 0) (sl, x, y) %g %g %g\n", slip_length, x, y);
    return (d[1]*(bc - v[0])/d[0] - d[0]*(bc - v[1])/d[1])/((d[1] - d[0])*Delta); 
  }
  // fprintf(stderr, "(sl, d[0]*Delta, v[0], x, y, bc) %g %g %g %g %g %g\n", slip_length, d[0]*Delta, v[0], x, y, bc);
  return (bc - v[0])/(slip_length + d[0]*Delta);
