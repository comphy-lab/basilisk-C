...
center of volume, their VOF tracer index etc. */
 ...
 int lendat = dat->len/sizeof(clsncdat);;
  fprintf(stderr,"lendat %d %g\n", lendat, t);

  array_free(bub);

  /**
     Here we loop through the list named dat and find the elements
     whose ft is greater than prescribed time and add them to the new 
     list named datlist. Add all other elements to list named dat1. */
  ...
  /** Replace the VOF tracer for each element in the datlist*/
  int len = datlist->len/sizeof(clsncdat);
  fprintf(stderr,"len %d %g\n", len, t);
  int len1= dat1->len/sizeof(clsncdat);
  fprintf(stderr,"len1 %d %g\n", len1, t);
  if(len)
    {
      /** Store the replacement VOF tracers corresponding to each element in
	  datlist in an array named rep*/
      int rep[len];
      clsncdat * aa = datlist->p;
      for (int i = 0; i<len; i++) 
	{
	  rep[i] = aa->vof2;
	  aa++;
      ...
      for(scalar c in maybe_close)
	{
	  foreach()
	    {
	      clsncdat * ab = datlist->p;
	      for (int i = 0; i<len; i++)
		{    
		  if (T[] == ab->b) 
		    if(c.i == ab->vof1 && c[]>EPS1) 
		      {
			scalar t = {rep[i]};
			t[] = c[]; c[] = 0.;
		      }
      ...
      fprintf(stderr,"coalesced at %g %d\n", t, len);
    }

 /**
  Finally we will free all the arrays and lists 
  */
  free (maybe_close);
  array_free(dat);
  array_free(datlist);
...
passed through the control coalescence function in the next time step */

  return dat1;
}

/** Calling the cntrl_nclsnc function after updating
VOF. Here at the first time step NULL pointer is passed
...
/*
event data_clsnc(t+=1.)
{
  char name[80];
  sprintf (name, "restore_control_coalescence");
  static FILE * fp = fopen (name, "w");
  clsncdat * pr = dat->p;
  for(int j=0; j<(dat->len/sizeof(clsncdat)); j++)  
     {
       fprintf (fp, "%g %g %g %g %g %g %d %g %g %d %d %g\n", pr->cv[0].x, \
		pr->cv[0].y, pr->cv[0].z, pr->cv[1].x, pr->cv[1].y, \
		pr->cv[1].z, pr->b, pr->t, pr->ft, pr->vof1, pr->vof2, t);
     }
  fflush (fp);  
}
*/
