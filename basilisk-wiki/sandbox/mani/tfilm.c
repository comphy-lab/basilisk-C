...
one-dimensional approximation considering viscosity.
$$
...
int main() {
  for (wavenumber = 0.02; wavenumber <= 1.; wavenumber += 0.02) { 
    counter++;
    growth_rate = sqrt(0.5 * pow(wavenumber,2.) + 1.75 * pow(wavenumber,4.)) - (1.5 * pow(wavenumber,2.));
...
event integration (i++) {
  ...
  foreach_face()
    h2u.x[] += - dt*(h2uflux[] - h2uflux[-1])/Delta + dt*(h2[]*K[] - h2[-1]*K[-1])/Delta + 1.5*dt*MU*((h2[1]+h2[])*d1u[] - (h2[1]+h2[-1])*d1u[-1])/Delta;
...
~~~bash
qcc -O2 -Wall tfilm.c -o tfilm -lm
./tfilm > out
~~~
Growthrate vs wavenumber comparison for theoretical and numerical values
~~~gnuplot
reset
...
set samples 1000
set xrange [0:1.2]
plot real(sqrt(0.5*x**2 + 1.75*x**4) - 1.5*x**2) t 'Theory'
~~~
