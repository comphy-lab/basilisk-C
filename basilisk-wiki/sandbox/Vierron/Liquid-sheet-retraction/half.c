...
#define smooth 0 // set to 1 to get a smooth initial circular shape
	...
	#define frq 8. // frequency
	#define circle (sq(x)+sq(y)-sq(R + A*fabs(sin(atan2(y,x)*frq+M_PI/2.)))) // Corrugate circle
	#define Co (R + A*fabs(sin(atan2(y,x)*frq+M_PI/2.))) // Corrugate radius
...
#endif
		iteration++;
	 }while (adapt_wavelet ({f}, (double[]){1e-5}, 12).nf !=0 && 
	iteration<=10);
	fprintf(stdout,"%d\n", iteration);
	fflush(stdout);
	boundary ({f,u});
...
event interface (i++) {
	clear();
	view (fov = 7.35722, quat = {0.633604,-0.00534031,-0.00647799,0.773612}, tx = -0.453663, ty = -0.156885, bg = {0.3,0.4,0.6}, width = 1310, height = 560, samples = 4);
	draw_vof("f");
	char time[80];
...
event python(t=tEnd){
	...
	sprintf(command4, "python h.py\t%g\t%g",Oh, H);
#endif
#if smooth
...
/**
## Results
![fraction field Oh=0.01.](half/Half0.01.mp4)
![refinement Oh=0.01.](half/grid0.01.mp4)
*/
