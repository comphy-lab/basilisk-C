...
This code simulates the retraction of a liquid layer of thickness H with a velocity close to that of TaylorCulick in the inertial domain. We are in the referentiel of the bulge.
*/
...
#define R0 H //initial radius of bulge
#define xo 21. //position of center circle
...
/**
## Boundary conditions
We place ourselves in the referentiel of the bulge
*/
u.n[left] = dirichlet(-TaylorCulick);
u.n[right] = dirichlet(-TaylorCulick);

/**
## Initial conditions
*/
event init (t = 0) {
	fraction(f, x<=xo ? Circle : line);// f=0=fluid2=water
	foreach(){
		u.x[] = -TaylorCulick; //moving referential
	}
	boundary ({f,u});
}
...
![fraction field Oh=0.1](plan2D/vof0.1.mp4)
![refinement Oh=0.1](plan2D/grid0.1.mp4)

### low Oh
![fraction field Oh=0.01](plan2D/vof0.01.mp4)
![refinement Oh=0.01](plan2D/grid0.01.mp4)

![fraction field Oh=0.005](plan2D/vof0.005.mp4)
![refinement Oh=0.005](plan2D/grid0.005.mp4)
...
set key left top
set key font ",10"
plot 'data0.1' u ($3/sqrt(2)):4 every 4 ps 0.5 t 'Oh=0.1',\
'data0.01' u ($3/sqrt(2)):4 every 4 ps 0.5  t 'Oh=0.01',\
'data0.005' u ($3/sqrt(2)):4 every 4 ps 0.5  t 'Oh=0.005',\
...
set key bottom right
plot for [i=8:10:1] 'logs'.i using 2:6 w l lw 2 t sprintf("N=2^{%i}",i)
~~~
*/
