...
//droplet PDF:
    ...
    double l[Nbin], pdf[Nbin], pdf_plate[Nbin], pdf_box[Nbin];  
...
//print the Frequency
   fprintf(fout, "bin_center Frequency_Domain Frequency_Near Frequency_Far\n");
   for (int k = 0; k < Nbin; k++)
        fprintf (fout, "%g %g %g %g \n",  l[k]+bin/2,
                 pdf[k], pdf_box[k], pdf_plate[k] );
        fflush (fout);
...
if (mpi_rank==0) {
  ...
  fprintf(fout, "bin_center PDF_domain_avg PDF_Near_avg  PDF_Far_avg\n");
   for (int k = 0; k < Nbin; k++){
        
	 l[k]=l0+k*bin;
        fprintf (fout, "%g %g %g %g\n", (l[k]+bin/2)*1e+6,
                 pdf_domain_avg[k]/(bin*1e+6*sum_domain), pdf_near_avg[k]/(bin*1e+6*sum_box), pdf_far_avg[k]/(bin*1e+6*sum_plate));   ///(bin*sum_box)
     }
