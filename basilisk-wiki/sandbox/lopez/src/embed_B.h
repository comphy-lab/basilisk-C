...
be also a component of vector **u**). In this case the Dirichlet
boundary condition *bc* at the embed boundary is unknown and it has to be
calculated solving the system,

$$
\mathbf{v} \cdot \mathbf{n} = 0
\quad \text{and} \quad \frac{\partial (\mathbf{v} \cdot \mathbf{t}) }{\partial n}= 0
$$

First equation impose the impenetrability in the embed body while the
second corresponds to the absence of tangential viscous stresses at
the interface of the body. For sake of simplicity we implement only
the 2D case. The 3D would be straightforward. First we need to compute
the normal gradient of the normal velocity. */

#if dimension == 2
static inline double normal_velocity_gradient (Point point, scalar s, scalar cs,
					       coord n, coord p, double bc,
					       double * coef)
{
  ...
  if (defined)
	...
	/**
        We need to interpolate the normal velocity. */
	
	double phi1 = 0., phi2 = 0., phi3 = 0.;
	foreach_dimension() {
	  phi1 += u.x[i,j-1]*n.x; phi2 += u.x[i,j]*n.x;  phi3 = u.x[i,j+1]*n.x; 
	}
	v[l] = quadratic (y1, phi1, phi2, phi3);
      }
      else
	break;
    }
...
/**
If the velocity boundary is slip, the computation of the gradients of
the velocity components would be simply in 2D:

$$
\left.
\begin{array}{lcl}
\displaystyle\frac{\partial u_x}{\partial n} n_x + 
\displaystyle\frac{\partial u_y}{\partial n} n_y & 
= & \left. \displaystyle\frac{\partial u_n}{\partial n} \right|_{u_n=0} \\
 & & \\
\displaystyle\frac{\partial u_x}{\partial n} n_y - 
\displaystyle\frac{\partial u_y}{\partial n} n_x & = &  
\displaystyle\frac{\partial u_t}{\partial n} = 0
\end{array}
\right] \quad \Rightarrow \quad \left[
\begin{array}{l}
\displaystyle\frac{\partial u_x}{\partial n} = 
\frac{n_x}{n_x^2 + n_y^2} \frac{\partial u_n}{\partial n} \\
\\
\displaystyle\frac{\partial u_y}{\partial n} = 
\frac{n_y}{n_x^2 + n_y^2} \frac{\partial u_n}{\partial n}
\end{array}
\right.
$$
...
double slip_gradient (Point point, scalar s, scalar cs,
			   coord n, coord p, double * coef)
{
  assert( s.v.x.i >= 0);
  vector u = s.v;
  
  double dunn = normal_velocity_gradient (point, s, cs, n, p, 0., coef);
  return (s.i == u.x.i ? n.x : n.y)*dunn/(sq(n.x) + sq(n.y));
}
...
conditions on the embedded surface into account.   We associate the
slip boundary condition to a BC of type Dirichlet imposing an
unphysical *nodata*. */
...
coord embed_gradient (Point point, vector u, coord p, coord n)
{
  ...
  foreach_dimension() {
    ...
    if (dirichlet) {
      double val;
#if dimension == 2
      dudn.x = (vb == nodata ? slip_gradient (point, u.x, cs, n, p, &val) :
		dirichlet_gradient (point, u.x, cs, n, p, vb, &val));
#else
