...
This function returns the nomal gradient of the scalar *s* (which must
be also a component of vector **u**). In this case the value of **u**
at the embed boundary *bc* is unknown and it has to be calculated
solving the system of equations obtained by imposing the
impenetrability of the embed body (null normal velocity),

$$ 
\mathbf{u} \cdot \mathbf{n}|_{bc} = 0 \quad \Rightarrow \quad
\mathbf{u}^o_x \, \mathbf{n}_x + \mathbf{u}^o_y \, \mathbf{n}_y = 
\mathbf{u}^o_x \, \mathbf{t}_y - \mathbf{u}^o_y \, \mathbf{t}_x = 0 
$$

and the Navier BC for the tangential component of the velocity

$$ 
(\mathbf{u} \cdot \mathbf{t})|_{bc}=\mathbf{u}_t|_{bc} = \lambda \, \left. \frac{\partial \mathbf{u}_t }{\partial n} \right|_{bc} \quad \Rightarrow \quad
\mathbf{u}^o_x \, \mathbf{t}_x + \mathbf{u}^o_y \,\mathbf{t}_y = \frac{\lambda (a_1 \mathbf{u}_t^1  + a_2 \mathbf{u}_t^2)}{1 - \lambda a_o} 
$$

where the superscript *o* denotes that the velocity component is evaluated at the boundary *bc*.

The normal gradient of the tangential velocity, $\mathbf{u}_t$, writes following [Johansen and Colella, 1998](#johansen1998) as
$$
\partial_n \mathbf{u}_t = a_o \mathbf{u}_t^o + a_1 \mathbf{u}_t^1 +  a_2 \mathbf{u}_t^2
$$  
...
#if dimension == 2
foreach_dimension()
static inline double  navier_gradient_x (Point point, scalar s, scalar cs,
					 coord n, coord p, coord t,
					 double lambda, double * coef)
{
  double d[2], vt[2];
  coord  v[2] = {{nodata, nodata},{nodata, nodata}};

  /**
  We check if the scalar *s* is also a vector component. We
  ...
  /**
  Since the location of the interpolation points for the normal
  gradient depends strictly in geometrical issues, this part follows
  closely *dirichlet_gradient_x()*. */
  ...
  if (defined)
	...
	/**
	We calculate the tangential component ($vt = u.x \, t.x + u.y
	\, t.y$) at each interpolation point as well as the
	interpolation values for each vector component. */

	vt[l] = 0;
	foreach_dimension() {
	  scalar s = u.x;
	  v[l].x = quadratic (y1, (s[i,j-1]), (s[i,j]), (s[i,j+1]));
	  vt[l] += t.x*v[l].x;
	}
      }
      else break;
  ...
  *coef = 0.;
  double bc;
  if (v[1].x != nodata) {// third-order gradient

    /**
    Now we return the normal gradient of the component.Althougth
    previously we compute the value of that component resulting of the
    above system. */

    bc = lambda*(sq(d[1])*vt[0] - sq(d[0])*vt[1])/
      (sq(d[1])*d[0] - sq(d[0])*d[1] + lambda*(sq(d[1])-sq(d[0])));

    if (s.v.x.i == s.i) {
      bc *= t.x/(sq(t.x)+sq(t.y));
      return (d[1]*(bc - v[0].x)/d[0] - d[0]*(bc - v[1].x)/d[1])/((d[1] - d[0])*Delta);
    }
    else {
      bc *= t.y/(sq(t.x)+sq(t.y));
      return (d[1]*(bc - v[0].y)/d[0] - d[0]*(bc - v[1].y)/d[1])/((d[1] - d[0])*Delta);
    }
  }

  if (s.v.x.i == s.i) {
    bc = t.x*vt[0]/(sq(t.x)+sq(t.y));
    return (bc - v[0].x)/(d[0]*Delta);
  }
  else {
    bc = t.y*vt[0]/(sq(t.x)+sq(t.y));
    return (bc - v[0].y)/(d[0]*Delta);
  }
}

static inline
double navier_gradient (Point point, scalar s, scalar cs,
			coord n, coord p, double lambda, double * coef)
{
  coord t = {-n.y,  n.x};
  foreach_dimension()
    if (fabs(n.x) >= fabs(n.y))
      return navier_gradient_x (point, s, cs, n, p, t, lambda, coef);
  return nodata;
}
#endif
...
coord embed_gradient (Point point, vector u, coord p, coord n)
{
  ...
  foreach_dimension() {
    ...
    if (dirichlet == 1) {
      ...
      double val;
      dudn.x = navier_gradient (point, u.x, cs, n, p, vb, &val);
    }
#endif    
    else // Neumann
      dudn.x = vb;
...
double embed_flux (Point point, scalar s, face vector mu, double * val)
{
  ...
  *val = 0.;
  ...
  normal gradient. In case the Dirichlet value is equal to *nodata*
  the boundary condition is of type **slip**. */
  ...
  if (dirichlet == 1) {
    ...
    normalize (&n);
    grad = navier_gradient (point, s, cs, n, p, grad,  &coef);
  }
#endif
...
@def _neumann_homogeneous(...) (data ? *((int *)data) = 0, (0) :
				val(_s,0,0,0))
@
@undef _dirichlet
...
@def _dirichlet_homogeneous(...) (data ? *((int *)data) = 1, (0) :
				  - val(_s,0,0,0))
@
@undef _navier
@def navier(expr,...) (data ? embed_area_center (point, &x, &y, &z),
		       *((int *)data) = 2, (expr) :
		       2.*(expr) - val(_s,0,0,0))
@
@undef _navier_homogeneous
@def _navier_homogeneous(...) (data ? *((int *)data) = 0, (0) :
			       - val(_s,0,0,0))
@
