...
\quad \frac{\partial (\mathbf{u} \cdot \mathbf{t}) }{\partial n}= 0
$$

First equation impose the impenetrability of the embed body while the
second corresponds to the absence of tangential viscous stresses at
the interface of the body. */

#if dimension == 2
foreach_dimension()
static inline double  slip_gradient_x (Point point, scalar s, scalar cs,
				       coord n, coord p,
				       coord t, double * coef)
{
  double d[2], vt[2], v[2] = {nodata, nodata};
  ...
  if (defined)
	...
	/**
	We calculate the tangential component ($vt = u.x t.x + u.y t.y$) at
	each interpolation point as well as the interpolation values
	...
	foreach_dimension() {
	  scalar f = u.x;
	  vt[l] += t.x*quadratic (y1, (f[i,j-1]), (f[i,j]), (f[i,j+1]));
	}
      }
      else break;
    }
  ...
  *coef = 0.;
  ...
  if (v[1] != nodata) {// third-order gradient
    ...
    bc = (sq(d[1])*vt[0] - sq(d[0])*vt[1])/(sq(d[1]) - sq(d[0]));
    if (s.v.x.i == s.i) {
      bc *= t.x;
    }
    else {
			    ...
			    coord n, coord p, double * coef)
{
  ...
  foreach_dimension()
   if (fabs(n.x) >= fabs(n.y))
     return slip_gradient_x (point, s, cs, n, p, t, coef);
  return nodata;
}
#endif
...
double embed_flux (Point point, scalar s, face vector mu, double * val)
{
  ...
  *val = 0.;
  ...
  if (dirichlet) {
    normalize (&n);
#if dimension == 2
    grad = (grad == nodata ? slip_gradient (point, s, cs, n, p, &coef) :
