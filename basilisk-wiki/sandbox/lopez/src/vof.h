...
attribute). The boolean attribute *comp* switch off the divergence 
free correction in the tracer equation if it is set to "true". */
...
static void sweep_x (scalar c, scalar cc, scalar * tcl)
{
  ...
  For tracers, the one-dimensional update is, following the same
  scheme that above, simply 
  $$ 
  \partial_tt_j =
  -\nabla_x\cdot(\mathbf{u}_f t_j) + f_j \nabla_x\cdot\mathbf{u}_f 
  $$
  The second term, a correction for non perfect divergence free
  velocity field, can be removed setting to `true` the attribute *comp*. 
  */
