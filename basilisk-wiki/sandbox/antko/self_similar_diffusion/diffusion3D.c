/**
# Self-similar diffusion in 3D

A 3D blob of concentration $c(r,t)$ initially occupying the unit sphere freely diffuses in space according to:
$$
\frac{\partial c}{\partial t} = \Delta c.
$$
The concentration is initially constant and evolves under the constraint:
$$
4 \pi \int c(r,t) \, r^2 \,\mathrm{d}r = 1
$$*/
...
event integration (i++) {
  ...
  foreach()
    volumic_metric[] = cm[];
  boundary({volumic_metric});
  mgconcentration = diffusion (concentration, dt, D = diffusion_coef, theta = volumic_metric);
}

/** Regularly we output the concentration profile, in native variables and in the self-similar space.*/

event output (t+=0.5; t<tend) {
  static FILE * fp = fopen("cprof_axi", "w");
  static FILE * fpr = fopen("cprof_axi_resc", "w");
