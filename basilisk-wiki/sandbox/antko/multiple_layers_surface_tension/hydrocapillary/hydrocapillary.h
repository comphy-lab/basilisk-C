...
#define tz(i)        (sigma[i]*slope(i)/pow(1.+ sq(slope(i)),0.5))
#define curvature(i) (-(tz(i+1)-tz(i))/Delta)
...
event face_fields (i++, last)
{
  ...
  foreach_face (reduction (min:dtmax)) {
    double ax = - gmetric(0)*(G*(eta[] - eta[-1])/Delta + (curvature(0) - curvature(-1))/Delta);
    double H = 0., um = 0.;
    foreach_layer() {
