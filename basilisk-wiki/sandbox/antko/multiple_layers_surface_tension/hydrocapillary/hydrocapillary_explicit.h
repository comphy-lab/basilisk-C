/**
# The hydro-capillary static multilayer solver for free-surface flows

This solver is built upon the [hydrostatic multilayer solver](/src/layered/hydro.h)
and adds capillary effects on the free surface. The theoretical basis
and main algorithms for this solver are described in [Popinet,
2020](/Bibliography#popinet2020). Note however that this version of
the solver is more recent and may not match the details of Popinet,
2020.
...
& = 0,\\ \partial_t \left( h \mathbf{u} \right)_k + \mathbf{{\nabla}}
\cdot \left( h \mathbf{u} \mathbf{u} \right)_k & = - gh_k
\mathbf{{\nabla}} (\eta) \color{red} - h_k\nabla(\frac{\gamma}{\rho}
\kappa) + \left.\nabla(\frac{\gamma}{\rho}) \sqrt{1+\nabla \eta^2}\right|_t
\end{aligned} 
$$
with $\mathbf{u}_k$ the velocity vector, $h_k$ the layer thickness,
$g$ the acceleration of gravity and $\eta$ the free-surface
height. **Note: the capillary contributions are only valid in 2D right
now (that is, 1+1 D). Check of the equations and of the implementation
needed.** Note also that though the equations are written in terms of
curvature, the *implementation* involves tangent vectors. Finally the last term is only active on the topmost layer. The
non-hydrostatic pressure $\phi_k$ and vertical velocity $w_k$ can be
added using [the non-hydrostatic extension](/src/layered/nh.h). See
also the [general introduction](/src/layered/README).

![Definition of the $n$ layers.](/src/figures/layers.svg){ width="60%" }
...
#define slope(i) ((eta[i]-eta[i-1])/Delta)
#define tdotez(i) (sigma[i]*slope(i)/pow(1.+slope(i)*slope(i),0.5))
#define curv(i) (-(tdotez(i+1)-tdotez(i))/Delta)
//#define curvgradient(i) ((curvature(i)-curvature(i-1))/(Delta))
...
event face_fields (i++, last)
{
  ...
  foreach_face (reduction (min:dtmax)) {
    double ax = - gmetric(0)*(G*(eta[] - eta[-1])/Delta + (curvature[] - curvature[-1])/Delta);
    double H = 0., um = 0.;
    foreach_layer() {
    ...
    if (H > dry) {
      ...
      // double c = um/CFL + sqrt((G*Delta/pi+sigma[]*pi/Delta)*tanh(H*pi/Delta))/CFL_H;
       double c = um/CFL + sqrt(G*H+sigma[]*H*sq(pi/Delta))/CFL_H;
