...
event logfile (i++; t <= T_END)
{
  ...
  if (i < 10) {
    ...
    foreach() {
      fprintf (fpp, "phi_th : %g -- ", - gam[] * kappa[]);
...
/**  static FILE * fpv0 = fopen ("vel0.txt", "w");
  double l = 0;
  vector u[], uf[];
  scalar w[];
  fprintf(fpv0, "\n -- t = %g --\n", t);
  for(u, uf, w in ul, ufl, wl){
    l += 1;
    fprintf(fpv0, " -- l = %g -- \n", l);
    foreach()
      fprintf(fpv0, "%g     %g     %g \n", u.x[], uf.x[], w[]);
  ...
  static FILE * fpvol = fopen ("volume.txt", "w");
  fprintf(fpvol, "%g\n", statsf(H).sum);
}
