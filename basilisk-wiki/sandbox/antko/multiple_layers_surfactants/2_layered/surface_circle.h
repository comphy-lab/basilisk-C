...
double area_f (Point point, scalar H)
{
  ...
  double l = sqrt(1. + sq(hx));
  double ki = (kappa_comp(point, H) + kappa_comp(neighborp(1), H))/2;
  if (ki == 0)
    return l;
  else 
    return asin(l*ki/2) * 2./ki;
}
...
struct Vect tangent_center (Point point, scalar H)
{
  ...
  T.v = (T0.v + T1.v);
#else
  double theta0 = atan(T0.v/T0.xy.x);
  double theta1 = atan(T1.v/T1.xy.x);
