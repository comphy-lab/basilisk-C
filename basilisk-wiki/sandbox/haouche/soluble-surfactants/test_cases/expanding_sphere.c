...
event init(i = 0) {
  refine((sq(0.3) - pow(x, 2) - pow(y, 2)) - pow(z, 2) > 0 && level < LEVEL);
  fraction(f, -(sq(0.2) - pow(x, 2) - pow(y, 2) - pow(z, 2)));
  event("properties2");
...
event stability(i++) {
  trash({u, uf});
  foreach_face(y) uf.x[] = K*(sqrt(x*x + y*y + z*z))*cos(atan2(y,x))*sin(atan2(sqrt(x*x + y*y), z));
  foreach_face(x) uf.y[] = K*(sqrt(x*x + y*y + z*z))*sin(atan2(y,x))*sin(atan2(sqrt(x*x + y*y), z));
  foreach_face(z) uf.z[] = K*(sqrt(x*x + y*y + z*z))*cos(atan2(sqrt(x*x + y*y), z));
  foreach() {
    u.x[] = K*(sqrt(x*x + y*y + z*z))*cos(atan2(y,x))*sin(atan2(sqrt(x*x + y*y), z));
    u.y[] = K*(sqrt(x*x + y*y + z*z))*sin(atan2(y,x))*sin(atan2(sqrt(x*x + y*y), z));
    u.z[] = K*(sqrt(x*x + y*y + z*z))*cos(atan2(sqrt(x*x + y*y), z));
  }
}
...
event logfile(t = 0; t += 0.01; t <= 0.7) {
  ...
  char filename[200];
  sprintf(filename,"expanding-sphere-test-%d",(1 << grid->maxdepth));
...
ts32, gammat32,cnet32 = np.loadtxt('expanding-sphere-test-32',delimiter=' ',unpack=True)
ts64, gammat64,cnet64 = np.loadtxt('expanding-sphere-test-64',delimiter=' ',unpack=True)
ts128, gammat128,cnet128 = np.loadtxt('expanding-sphere-test-128',delimiter=' ',unpack=True)
...
plt.savefig('expanding_sphere_test.png')
plt.figure()
...
plt.savefig('expanding_sphere_test_mc.png')
plt.figure()
...
plt.savefig('expanding_sphere_test_order.png')
plt.figure()
