...
In this example we show we may improve on the [Kutamato test](/src/test/kuramoto.c), using the implicit multigrid solver to achieve a second-order accurate timestepping for all terms.
...
int main()
{
  ...
  /**
  The timestep is set to 1, which is significantly larger than that
  in Duchemin & Eggers (0.014), or the test case */
