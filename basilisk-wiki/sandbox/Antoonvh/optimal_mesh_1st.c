/**
# Optimal bitree meshes for the first derivative

We seek to minimize a given $L_p$ error norm of a second-order
accurate estimate for the first derivative for a given number of
...
For a second-order accurate first derivative in 1D,

$$\epsilon_i \propto |\frac{\mathrm d^3 f}{\mathrm d x^3}\Delta_i^2|^{p}\Delta_i.$$

This third derivative can be estimated using a 3-th order accurate
multilevel interpolation (wavelet: `refine_3rd`). For $p \geq 0$,
...
that they are not concererd with.
 */
...
double flux_div (double x, double Delta) {
  double xl = Gaussian(x - Delta/2.);
  double xr = Gaussian(x + Delta/2.);
  return (xr - xl)/Delta;
}
...
double diagnose_error (scalar s, int norm) {
  ...
  foreach(reduction(+:err)) {
    double errl = flux_div(x, Delta) - (s[1] - s[-1])/(2*Delta);
...
int main() {
  ...
  for (int j = 0 ; j < 3; j++) {
	...
	wavelet (s, w);
	foreach_cell() 
	  w[] = pow(fabs(w[])/(Delta), (double)norm)*pow(Delta, dimension);
