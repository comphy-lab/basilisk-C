...
This is mostly [tree.h](/src/grid/tree.h), but modified such that

1. `foreach_face()` iterates the coarse faces at resolution boundaries.
2. The tree also contains a cache for all active faces
3. This cache can be iterated with the non-rotating `foreach_face_level(l)` iterator.    
4. Perhaps more stuff to facilitate flux prolongation.

The bits that are modified have marked-down comments
 */
...
/**
We define an additonal macro that checks if an cell in the argument is
coarse.
 */
@define is_coarsec(cell)   ((cell).neighbors > 0)
// Caches
...
typedef struct {
  ...
  Cache        faces;    /* leaf face indices */
  Cache        vertices; /* vertex indices */
  Cache        refined;  /* refined cells */
  /**
A cache for all active faces
   */
  Cache        all_faces;
  ...
  /**
facelevel is not used because they cant use flags??
   */
  // CacheLevel * facelevel 
...
/**
The `update_cache_f()` function defines the various Caches in the
`tree`.
 */
...
#endif
    /**
       All faces (including coarse ones) are stored:
     */
    if (is_active(cell)) {
      foreach_dimension() {
	if (is_active(neighbor(-1)) || is_boundary(neighbor(-1))) {
	  unsigned short flags = 0;
	  flags |= face_x;
	  cache_append (&q->all_faces, point, flags);
	}
	if (is_boundary(neighbor(1))) {
	  unsigned short flags = 0;
	  flags |= face_x;
	  cache_append (&q->all_faces, neighborp(1), flags);
	}
      }
    }

    /**
       The leaf-faces always touch atleast one leaf cell.
    */
    if (is_leaf (cell)) {
      if (is_local(cell)) {
	...
	unsigned short flags = 0;
	/**
Add coarse faces instead of fine...

i.e.
$$\mathtt{is\_prolongation()} \rightarrow \mathtt{is\_coarsec()}$$
	 */
	foreach_dimension()
	  if (is_boundary(neighbor(-1)) || is_coarsec(neighbor(-1)) ||
	      is_leaf(neighbor(-1)))
	    flags |= face_x;
	if (flags)
	  cache_append (&q->faces, point, flags);
	foreach_dimension()
	  if (is_boundary(neighbor(1)) || (is_coarsec(neighbor(1)) && !is_leaf(neighbor(1))))
...
@define end_foreach()   end_foreach_cache()
				  ...
				  /**
All active face iterator on levels:
				   */
@def foreach_face_level(l) {
  update_cache();
  foreach_cache (tree->all_faces) {
    if (level == l) {
      @
@define end_foreach_face_level() }}} end_foreach_cache()
...
void check_two_one (void)
{
 ...
 Point locate (double x = 0, double y = 0, double z = 0) {
  for (int l = depth(); l >= 0; l--) {
    Point point = {0};
    point.level = l;
    int n = 1 << point.level;
    point.i = (x - X0)/L0*n + GHOSTS;
#if dimension >= 2
    point.j = (y - Y0)/L0*n + GHOSTS;
#endif
#if dimension >= 3
    point.k = (z - Z0)/L0*n + GHOSTS;
#endif
...
//#define boundary multigrid_restriction
