...
A dipolar vortex propages itself through a modest scalar
gradient. hereby the scalar gradients sharply increase at the edge of
the dipole's atmosphere.

![](gradient-sharpening/Re800-overview.mp4)

The results are presented and discussed in a publication:  

*A note on scalar-gradient sharpening in the stable atmospheric
boundary layer*  
...
We solve the Navier-stokes equations and a tracer advection-diffusion
equation.
*/
...
/**
The equations of motion are solved in the frame of reference that is
...
int main () {
  ...
  /**
     Uncomment the following line for a sweep op $Re$ values and the
     other two for a convergence study. Here we only run $Re = 800$
     and `se` = `ue` = 0.02.
   */
...
We initalize the Lamb-Chaplygin dipolar vortex and the initally modest
scalar front.
 */
...
![](gradient-sharpening/Re800-grid.mp4)
 */
event adapt (i++) {
  int minlevel = 6;
