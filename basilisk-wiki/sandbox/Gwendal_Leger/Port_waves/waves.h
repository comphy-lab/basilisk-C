...
// Just in case...
#ifndef waves_normalize
#define waves_normalize 0
#endif
#ifndef waves_amplification
#define waves_amplification 1.
#endif
#ifndef waves_dilatation
...
wave_spectrum init_waves () {
  ...
  for (ii = 0; ii < waves.N; ii++) {
    ...
    waves.lambda[ii] = waves_lambda[ii] * waves_dilatation;
  ...
  for (ii = 0; ii < waves.N; ii++) {
    if (waves_normalize)
      waves.A[ii] /= cumsum;
    waves.A[ii] *= waves_amplification;
  }
  ...
  fprintf (fp,
	   ...
	   "Dilatation factor = %g\n",
	   waves.N, waves_amplification, waves_dilatation);
  if (waves_normalize)
