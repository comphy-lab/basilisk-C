...
// Function to animate a discrete fourier transform, from an file produced by output_gauges
void discrete_Fourier_transform_animated (char * filename, int N, double t) {
  double samples[N];
  double signal[N];
  int k;
  FILE * fp;
  ...
  if (N > 0) {
    ...
    // Fourier transform
    DFT signal_Fourier_transformed = nonuniform_discrete_Fourier_transform (N, samples[N-1], samples, signal);

    char fn[128];
    sprintf (fn, "%s_animation_data", filename);
    fp = fopen (fn, "a");
    for (k = 0; k < N; k++)
      fprintf (fp, "%g %g %g %g\n", samples[k], signal[k], signal_Fourier_transformed.f[k], signal_Fourier_transformed.n[k]);
    fprintf (fp, "\n\n");
    fclose (fp);

    sprintf (fn, "%s_animation_time", filename);
    fp = fopen (fn, "a");
    fprintf (fp, "%i %g\n", N, t);
    fclose (fp);
    ...
    fp = fopen ("plot.plot", "w"); // Animation plot needs to be performed at the end because it requires the 'data_out' file
    fprintf (fp,
	     "reset session\n"
	     "set term gif animate delay 10\n"
	     "set output '%s_DFT.gif'\n"
	     "getValue (row, col, filename) = system('awk ''{if (NR == '.row.') print $'.col.'}'' '.filename.'')\n"
	     "set autoscale noextend\n"
	     "set tics nomirror\n"
	     "file_data = '%s_animation_data'\n"
	     "file_t = '%s_animation_time'\n"
	     "if (getValue (16, 1, 'data_out') != 0) {\n"
	     "  do for [i=2:getValue (16, 1, 'data_out')] {\n"
	     "  set multiplot layout 2,1\n"
	     "  set title getValue (i, 2, file_t)\n"
	     "  set xlabel 'Time'\n"
	     "  set ylabel 'Amplitude'\n"
	     "  unset logscale x\n"
	     "  unset logscale y\n"
	     "  plot file_data using 1:2 index (i-1) with lines lc rgb 'red' title 'signal'\n"
	     "  unset title\n"
	     "  set xlabel 'Frequency'\n"
	     "  set logscale x\n"
	     //"  set logscale y\n"
	     "  set ylabel 'Magnitude'\n"
	     "  nn = int(0.6 * getValue (i, 1, file_t))\n"
	     "  plot file_data every ::0::nn using 3:4 index (i-1) with lines lc rgb 'red' title '||S||_2'\n"
	     "  unset multiplot\n"
	     "  }\n"
	     "} else {\n"
	     "set title 'Dummy plot'\n"
	     "plot sin(x) title 'Dummy plot'\n"
	     "}\n",
