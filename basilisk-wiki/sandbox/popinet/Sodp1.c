/**
# Shock tube problem for two ideal gases
...
event properties (i++) {
  ...
  foreach() {
    ...
    if (c[] > 0.5) {
      ps[] = pstate (rho1[]/c[], rhoL, pL, 0., gammaL);
      rhoc2v[] = rhov[]*cstate (rho1[]/c[], rhoL, pL, 0., gammaL);
    }
    else {
      ps[] = pstate(rho2[]/(1. - c[]), rhoR, pR, 0., gammaR);
      rhoc2v[] = rhov[]*cstate(rho2[]/(1. - c[]), rhoR, pR, 0., gammaR);
    }
  }
...
set ylabel '{/Symbol r}'
set cblabel 't'
p "theory.dat" u 1:3 not w l lc 0,  "./numerical.dat" u ($1/$2):4 not w l
~~~ 
...
set ylabel 'u'
set cblabel 't'
p "theory.dat" u 1:4 not w l lc 0,  "./numerical.dat" u ($1/$2):5 not w l
~~~ 
