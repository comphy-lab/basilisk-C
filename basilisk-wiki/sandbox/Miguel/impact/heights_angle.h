...
The bondary condition to impose an angle between 45 and 135 degrees is included. The solid wall must be on the left side.
...
static void column_propagation (vector h, float theta)
{
  foreach()
    for (int i = -2; i <= 2; i++)
      foreach_dimension()
  ...
  boundary ((scalar *){h});
	...
	{        
	h.y[-1,0] = height(h.y[]) + (1/tan(theta));
	}
...
#if !TREE
trace
void heights (scalar c, vector h, float theta)
{
