...
#else
//# include "viscosity_new.h"
# include "viscosity-simple.h"
#endif
...
#define neumann_pressure(i) (a.n[i]*fm.n[i]/(alpha.n[i] + SEPS))
#define neumann_pressure_ar(i) (a.n[i]*fm.n[i]/(alpha.n[i] + SEPS)*sq(ar))
...
#else // !AXI
#  if dimension > 1
p[top]    = neumann (neumann_pressure_ar(ghost));
p[bottom] = neumann (- neumann_pressure_ar(0));
#  endif
#  if dimension > 2
...
event advection_term (i++,last)
{
  if (!stokes) {
    prediction();
    const face vector alpha_ar[] = {1.0, 1/sq(ar)};
    mgpf = project (uf, pf, alpha_ar, dt/2., mgpf.nrelax);
...
void centered_gradient (scalar p, vector g)
{
  ...
  const face vector alpha_ar[] = {1.0, 1/sq(ar)};
  foreach_face()
    gf.x[] = fm.x[]*a.x[] - alpha_ar.x[]*(p[] - p[-1])/Delta;
  boundary_flux ({gf});
...
{
  const face vector alpha_ar[] = {1.0, 1/sq(ar)};
  mgp = project (uf, p, alpha_ar, dt, mgp.nrelax);
