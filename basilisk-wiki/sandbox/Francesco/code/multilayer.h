...
$$
G_{l+1/2} = \sum_{j=1}^{l}(\mathrm{div}_j + \mathrm{layer}_j\mathrm{dh})
$$
with
$$
		      ...
		      scalar dh, scalar * wl)
{
  ...
  /**
  To compute the vertical velocty we use the definition of the mass flux
  term (eq. 2.13 of [Audusse et al,2011](references.bib#audusse2011)):
  $$
  \mathrm{w}(x,y,z_{l+1/2}) = \frac{\partial z_{l+1/2}}{\partial t} + \mathbf{u}_{l+1/2}
  \cdot \nabla z_{l+1/2} - G_{l+1/2}
  $$
  We can write the vertical position of the interface as:
  $$
  z_{l+1/2} = z_{b} + \sum_{j=0}^{l} h_{j}
  $$
  and then compute the vertical velocity as:
  $$
  \mathrm{w}(i,j,l+1/2) = \mathrm{dh}\sum_{j=1}^{l}\mathrm{layer}_{j} + \mathbf{u}_{l+1/2} \cdot 
  \left[\nabla z_{b} + \nabla h \sum_{j=1}^{l}\mathrm{layer}_{j}\right] - G_{l+1/2}
  $$
  */
  double sumjl;
  foreach() {
	...
	[Audusse et al, 2011](references.bib#audusse2011). For second
	order vertical fluxes we need to compute the vertical gradient 
	of the velocity in each layer and reconstruct the interface
	velocity value for the top layer an bottom layer of each interface.
	We store the gradient of the lower layer in *dub* and the one of the
	top layer in *dut*.*/
	...
	if (ub.gradient) {
	  if (l == 0) {
	    scalar u1 = evolving[0].x;
	    scalar u2 = evolving[1].x, u3 = evolving[2].x;
	    dub = (u2[] - u1[])/(h[]*layer[0]);
	    dut = (u1[] - 2*u2[] + u3[])/(h[]*(layer[1] + 0.5*layer[2] + 0.5*layer[0]));
	  } 
	  else if (l > 0 && l < nl -2) {
	    scalar u0 = evolving[l-1].x, u1 = evolving[l].x;
	    scalar u2 = evolving[l+1].x, u3 = evolving[l+2].x;
	    dub = (u0[] - 2*u1[] + u2[])/(h[]*(layer[l] + 0.5*layer[l+1] + 0.5*layer[l-1]));
	    dut = (u1[] - 2*u2[] + u3[])/(h[]*(layer[l+1] + 0.5*layer[l+2] + 0.5*layer[l]));
	  }
	  else {
	    scalar u0 = evolving[l-1].x, u1 = evolving[l].x;
	    scalar u2 = evolving[l+1].x;
	    dub = (u0[] - 2*u1[] + u2[])/(h[]*(layer[l] + 0.5*layer[l+1] + 0.5*layer[l-1]));
	    dut = (u2[] -  u1[])/(h[]*layer[l]);
	  }
	}
	...
	w[] += 0.5*dh[]*sumjl + ui*((zb[1,0] - zb[])/(Delta) + div[]*sumjl) - Gi;
      }
    }
  }
}
