  ...
  for (lvl = 6; lvl <= 11; lvl++) { // minlevel = 4
    ...
    foreach() {
      if (cs[] > 0. && cs[] < 1.) {
        ...
        a2[] = embed_extrapolate2 (point, a2, nn, b, ab);
      }
    }
    boundary ({a,a2});
...
![Solution for *l=7*](extrapolation_ghigo/a.png)

![Error for *l=7*](extrapolation_ghigo/e.png)

![Error for *l=7*](extrapolation_ghigo/e2.png)

![Mesh for *l=7*](extrapolation_ghigo/mesh.png)
