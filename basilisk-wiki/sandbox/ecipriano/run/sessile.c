...
The results looks very bad because the VOF interface has strange artifacts that
must be investigated. Conversely, the distance function is quite clean. Also the
numerical results at the bottom of the page look bad for the same reason (VOF is
used to compute the droplet volume).
...
double theta0 = 30;
d[bottom] = contact_angle (theta0*pi/180.);
