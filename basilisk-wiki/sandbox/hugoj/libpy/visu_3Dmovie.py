#!/usr/bin/env python3
"""
visu3D_movie — render a 3D PyVista movie from a Basilisk/xarray NetCDF output.

Usage:
    visu3D_movie file.nc wave
    visu3D_movie file.nc wave --fps 24 --speed-factor 0.5 --method interp
    visu3D_movie file.nc wave --var-top u.x --clim-top -2.5 2.5 --cmap-top bwr

Run `visu3D_movie --help` for the full list of options.

NOTE: This script expects your existing helper functions
(build_z_at_t, build_3Dgrid, add_mesh_domain_side, add_ticks) to be
importable. Edit the import line below to point at wherever you keep
them (e.g. a local module `viz3d_utils.py`).
"""
...
from tools_plots import build_3Dgrid, add_mesh_domain_side, add_ticks
...
def render_movie(
    ...
    for it_video, t_target in enumerate(target_times):
        ...
        ds = build_z_at_t(ds, "float32")
        grid = build_3Dgrid(ds)

        T = (
            ds[var_side].values.transpose(2, 1, 0)
            if var_side != "T"
            else ds.T.values.transpose(2, 1, 0)
        )
