...
if True:
    ...
    s=ax.pcolormesh(x,y,eta_tile*kp, cmap='Grays_r', vmax=0.3, vmin=-0.15)
    ax.set_xlabel('x (m)')
    ax.set_ylabel('y (m)')
    ax.set_title(r'$\eta k_p$')
    plt.colorbar(s,ax=ax)
    fig.savefig('initial_eta.png')

    kr, F_eta = get_spec_1D(eta_tile, eta_tile, L/N_grid)
    

    fig, ax = plt.subplots(1,1,figsize = (5,5),constrained_layout=True,dpi=100)
    ax.loglog(kmod*L, F_kmod*kp**3, c='k', label='PM')
    ax.loglog(kr*np.pi*2*L, (F_eta/(2*np.pi))*kp**3, c='g', label=r'computed from $\eta$')
