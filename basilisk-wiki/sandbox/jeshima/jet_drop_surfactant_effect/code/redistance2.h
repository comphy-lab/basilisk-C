...
The below code is the same code and comments as [Palas Farsoya](/sandbox/farsoiya/README) as in [Farsoiya et al., 2024] (#farsoiya2024).
For their version, see sandbox/farsoiya/marangoni_surfactant/redistance2.h.
The relevant publication is [Farsoiya et al., 2024](#farsoiya2024).

The only changes were:

1. commenting out of minmod3 function (as this is used in src/redistance.h, called by src/two-phase-clsvof.h)

2. redistance function renamed redistance2 (again, as redistance is used in src/redistance.h)
...
foreach_dimension() static inline double weno_diff_x(Point point, scalar s, int i)
{
...
static void prehamiltonian(Point point, coord *grapl, coord *gramin, scalar s)
{
    foreach_dimension()
    {
...
static double hamiltonian(Point point, scalar s0, coord grapl, coord gramin)
{
    double hamil = 0;
    if (s0[] > 0)
        foreach_dimension()
...
foreach_dimension() static inline double root_x(Point point, scalar s, double eps, int dir)
{
...
static double forward_euler(scalar dist, scalar temp, scalar dist0, double dt)
{
    double res = 0.;
    foreach (reduction(max : res))
    {
        ...
        foreach_dimension() if (dist0[-1] * dist0[] < 0 || dist0[1] * dist0[] < 0)
            interfacial = true;
        ...
        else
        {
            ...
            double size = HUGE;
            foreach_dimension() for (int i = 0; i < 2; i++) if (dist0[] * dist0[1 - 2 * i] < 0.)
            {
...
trace int redistance2(scalar dist, int imax = 1,
...
@article{min2007,
  author        = {Chohong Min and Fr̩d̩ric Gibou},
