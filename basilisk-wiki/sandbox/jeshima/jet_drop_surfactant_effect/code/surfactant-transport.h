...
For their version, see sandbox/farsoiya/marangoni_surfactant/surfactant-transport.h.
The relevant publication is [Farsoiya et al., 2024](#farsoiya2024).

The changes were:

1. tracers are set in the main simulation (burstingBubble_with_surfactant.c) instead, due to the use of CLS-VOF

2. redistance function renamed redistance2 (redistance is used in the src/redistance.h)
...
// scalar *tracers = {c1, d2, pfield}; //commented out since tracers are set in burstingBubble_with_surfactant.c
...
event stability(i++)
{
    ...
    if (surfactant)
    {
        foreach_face(reduction(max : maxvel)) if (u.x[] != 0.)
        {
...
event properties2(i++)
{
    ...
    if (counter >= 0 && counter % reinit_skip_steps == 0)
    {
        ...
        redistance2(d2temp, imax = 0.5 * (1 << grid->maxdepth));
        printf("\n %d redistancing", i);
        fflush(stdout);
        ...
        if (d2max > 6. || d2min < -6.)
        {
            ...
            counter = counter - 2;
            // try again after two computational steps
            printf("Error: Signed distance computation faulty. Retrying after two steps.\n");
        }
