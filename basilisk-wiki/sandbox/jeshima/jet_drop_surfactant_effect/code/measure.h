...
The below code is the same code and comments as [Alexis Berny](/sandbox/aberny/README). Except for:
The relevant publication is [Berny et al., 2020](#berny2020).

1. Total amount of surfactant of the drop added

2. Commenting/editing some codes that were not parallel friendly

3. Commented out the dumping of dumpfiles when DumpSimulation == 1
...
struct Phase
{
    ...
    double totsurf;     // Total amount of surfactant of drop - extended from Berny et al.
};
...
void initPhase(Phase *drop)
{
    ...
    drop->cellsRadius = 0;
    drop->cells = 0;
    drop->totsurf = 0; // surfactant, extended from Berny
}
...
event measure(i++)
{
    ...
    Phase drop[n - 1];
    int nc[n];
    double cnet[n]; // total surfactant

    for (int j = 0; j < n; j++)
    {
        dropVelo[j] = v[j] = b[j].x = b[j].y = b[j].z = cnet[j] = 0.;
        nc[j] = 0;
    }
    ...
    foreach_leaf() if (m[] > 0)
    {
        ...
        /**
        Calculate total amount of surfactant
         */
        cnet[j] += 2 * pi * dv() * c1[];
    ...
    MPI_Allreduce(MPI_IN_PLACE, cnet, n, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD); // surfactant
    ...
    // original code was not parallel friendly
    /**foreach ()
    {
    ...
    */
    ...
    // Got rid of original code to output veloFilm
    /*
    #if _MPI
    ...
    */
    ...
    static FILE *filejet = fopen("jet_data", "a");
#if dimension > 2
#if FILM
#if _MPI
    if (pid() == 0)
        fprintf(filejet, "%d %f %g %f %f %f %f %f %f %f %f %f %d\n",
...
#if _MPI
    if (pid() == 0)
        fprintf(filejet, "%d %f %g %f %f %f %f %d\n",
    ...
    {
#if FILM
        fprintf(filejet, "%d %f %g %f %f %f %f %f %f %d\n",
                i, t, deltaT, xMax, velo, veloAdim, yMin, veloFilm, veloFilmAdim, n);
#else // No film, but MPI, no 3D
        if (i == 0)
            fprintf(filejet, "i, t, deltaT, xMax, velo, veloAdim (Ca), xMin, n");
        fprintf(filejet, "%d %f %g %f %f %f %d\n", i, t, deltaT, xMax, velo, veloAdim, n);
#endif
    }
#else // no mpi
#if FILM
    ...
    static FILE *fp = fopen("dropCorr", "a");
    static FILE *fp2 = fopen("dropErr", "a");
    ...
    for (int j = 1; j < n; j++)
    {
        ...
        drop[k].volume = v[j];
        drop[k].totsurf = cnet[j];
#if dimension > 2
        drop[k]
            .radius = pow((3. / (4 * pi) * drop[k].volume), (1. / 3.));
#endif
#if AXI
    ...
    {
        fprintf((drop[j].cells > minCells) ? fp : fp2, "%d %g %d %g %g %g %g %g %g %g %d %g\n",
                i, t, (j + 1), drop[j].x, drop[j].velo, drop[j].veloAdim,
                drop[j].volume, drop[j].radius, drop[j].reynolds,
                drop[j].cellsRadius, drop[j].cells, drop[j].totsurf);
    ...
    if ((numPhase > phasePrev || compt != 0) && i > 30 && sortie)
    {
        ...
        // dump(file = rupt); //Outputs too many files
