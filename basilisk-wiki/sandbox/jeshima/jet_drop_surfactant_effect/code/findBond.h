...
1. Default truncation of tip is set to 500 (pointLess variable in shapeBond) instead of 200

2. Print statements about the tip added for debugging

3. bubbleShape.h file is directly linked to A. Berny's sandbox
*/
...
#include "../../../aberny/bubble/bubbleShape.h"
...
coord *shapeBond(double Bond, Circle *hollow, Circle *cap)
{
    ...
    coord *data;
    int pointLess = 500;
    data = shape(BoInput, BondGhabache, BoEff, phi, size, pointLess, hollow, cap, 0, 0);

    // Ensure the circle is properly added
    if (hollow != NULL)
    {
        fprintf(stderr, "Circle at tip: Center (%f, %f), Radius %f\n", hollow->x, hollow->y, hollow->r);
    }
    else
    {
        fprintf(stderr, "No circle added at the tip.\n");
    }
    return data;
}
...
coord *shape3D(coord *dataShape)
{
    ...
    for (int i = 0; i <= 180; i++)
    {
        ...
        for (j = 0; j < size; j++)
        {
            ...
            rotateCalc[j].y = rotateOrigin[j].y * cos(theta) - rotateOrigin[j].z * sin(theta);
            rotateCalc[j].z = rotateOrigin[j].y * sin(theta) + rotateOrigin[j].z * cos(theta);

            /**
            Generation of the previous line*/
            rotatePrevious[j].x = rotateOrigin[j].x;
            rotatePrevious[j].y = rotateOrigin[j].y * cos(thetaPrev) - rotateOrigin[j].z * sin(thetaPrev);
            rotatePrevious[j].z = rotateOrigin[j].y * sin(thetaPrev) + rotateOrigin[j].z * cos(thetaPrev);
        }
