...
event movies (i += 10)
{
 ...
 mirror (n = {0, 1, 0}, alpha = 0.) {
   draw_vof ("f1", lw = 2);
   draw_vof ("f2", lw = 2);
   draw_vof ("f3", lw = 2, filled = 1);
 }
  cells();
  save ("movie.mp4");
#if CASE2
 clear();
  view (fov = 1.27812, quat = {0,0,-0.707,0.707}, tx = 0.236446, ty = -0.50002, bg = {1,1,1}, width = 780, height = 382);
...
#else
  clear();
  view (fov = 0.593863, tx = 0.122977, ty = -0.489743, width = 780, height = 382);
  draw_vof ("f1", lw = 2, lc = {1,0,0});
  draw_vof ("f2", lw = 2, lc = {0,1,0});
  draw_vof ("f3", lw = 2, lc = {0,1,1});
  cells();
...
![Animation of the interfaces and adaptive mesh(left), droplet phase colored with interface (right)](testcase-3p/movie.mp4)

![Zoom on the triple point.](testcase-3p/zoom.mp4)

~~~gnuplot Errors in volume conservation partial spreading
set xlabel 't'
set ylabel 'Volume error'
plot 'log' u 1:2 w l t 'f1 partial spreading', '' u 1:3 w l t 'f2 partial spreading', '' u 1:4 w l t 'f3 partial spreading'
~~~

~~~gnuplot Maximum horizontal velocity partial spreading
set xlabel 't'
set ylabel 'u max'
set logscale y
set grid
...
## Case 2
![Animation of the interfaces and adaptive mesh(left), droplet phase colored with interface (right)](testcase-3p/movie_spread.mp4)

![Zoom on the triple point.](testcase-3p/zoom_spread.mp4)

~~~gnuplot Errors in volume conservation total spreading
set xlabel 't'
set ylabel 'Volume error'
plot 'log_spread' u 1:2 w l t 'f1 total spreading', '' u 1:3 w l t 'f2 total spreading', '' u 1:4 w l t 'f3 total spreading'
~~~

~~~gnuplot Maximum horizontal velocity total spreading
set xlabel 't'
set ylabel 'u max'
set logscale y
set grid
plot 'log_spread' u 1:5 w l t ''
~~~
*/
