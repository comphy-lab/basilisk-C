...
which allow to add a third phase explicitly and which does not
depend on the choice of fluid fraction of the less dense fluid.
...
event properties(i++) {
  ...
  foreach_face() {
    ...
    alphav.x[] = (fm.x[]/rho(f1m,f2m,f3m));
    if (mu1 || mu2 || mu3) {
      face vector muv = mu;
      muv.x[] = (fm.x[]*mu(f1m,f2m,f3m));
    }
  }
  foreach(){
