...
is computed. The $Diff.x[]$ value should be zero in every cells that
...
int main() {
  ...
  foreach_vertex() {
    phi[] = (sq(x - 0.5) + sq(y - 0.5) - sq(R));
  }
  boundary ({phi});
  fractions (phi, f, s);

  fp=fopen("interfacellsf.dat", "w");
  fp1=fopen("nointerfacellsf.dat", "w");
  fp2=fopen("averagediff.dat", "w");
  fp3=fopen("noaveragediff.dat", "w");
...
#endif
#if REFINE
  refine (sq(x - 0.5) + sq(y - 0.5) < sq(1.1*R) && level < maxlevel);
#endif
#if UNREFINE
  unrefine (sq(x - 0.5) + sq(y - 0.5) < sq(1.1*R) && level < maxlevel);
#endif
#endif
  output_cells (stdout);
...
unset key
unset tics
plot [0.25:0.5][0.25:0.5][:] 'out' w l, 'log' w l, 'fbis.dat' w l, 'nointerfacellsf.dat' u 1:2:3 with labels
~~~
...
unset border
unset tics
plot [0.25:0.5][0.25:0.5][:] 'out' w l, 'log' w l, 'fbis.dat' w l , 'averagediff.dat' u 1:2:3 with labels
~~~
...
unset border
unset tics
plot [0.25:0.5][0.25:0.5][:] 'out' w l, 'log' w l, 'fbis.dat' w l, 'noaveragediff.dat' u 1:2:3 with labels
~~~
*/
