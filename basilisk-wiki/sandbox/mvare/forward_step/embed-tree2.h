...
/**************************************************************************
 New prolongation function
**************************************************************************/


static inline void refine_embed_prolong_wall (Point point, scalar s)
{
  foreach_child() {
    if (!cs[])
	{
    ...
    else if (cs[] > 0 && cs[] < 1)
      {
     	...
     	// interpolation in the wall-normal direction
		coord init_p;
		coord a = facet_normal(point,cs,fs); // wall-normal vector coords
		coord *n = &a, *p = &init_p; 

		double alpha = plane_alpha (cs[], a);
        double area = plane_area_center (a, alpha, p); // wall center coords

		normalize (&a);

		s[] = embed_prolong_wall2(point, s, *n, *p, 0.0); 
		/* computation of the values of the scalar s at the center of the 
		solid-containing cell using the values of s at points in the 
		wall-normal direction (see work of A.Ghigo). 
		Here, s is equal to the velocity components and is set to 0.0 
		on the wall to impose a no-slip condition */

      } 
    else { // same as in refine_embed_linear
      assert (coarse(cs));
      int i = (child.x + 1)/2, j = (child.y + 1)/2;
#if dimension == 2
      if (coarse(fs.x,i) && coarse(fs.y,0,j) &&
