...
From the paper by Shah et al. "Two-layer fluid flows on inclined surfaces"
 ...
 ![animation  ](visc2layerML/animate.gif)
...
$$\left\{\begin{array}{l}
   ...
   + \frac{-6 (6 h_0 u_0 + 2 h_1 M u_0 - 3 h_0 u_1)}{
   h_0^2 (3 h_0 + 4 h_1 M)}\\
  \partial_t (hu)_1+ \partial_x (hu^2)_1= - g h_1 \partial_x \eta - \partial_x (h q)_1   + \left[q \partial_x z \right]_1
  +\frac{(18 u_0 - 12 u_1)}{R h_1(3 h_0 + 4 h_1 M)}      \end{array}\right.$$
 




- where the friction is laminar say:

$$-h_0C_{00} u_0-h_0C_{01} u_1 =  \frac{-6 (6 h_0 u_0 + 2 h_1 M u_0 - 3 h_0 u_1)}{
   h_0^2 (3 h_0 + 4 h_1 M)}$$
$$-h_1 C_{10} u_0 -h_1 C_{11} u_1=\frac{(18 u_0 - 12 u_1)}{R h_1(3 h_0 + 4 h_1 M)}  $$

The coefficients $C_{ij}$ are such that under a pressure gradient
the equilibrium is (as the flow is very viscous, the convective term is finally small,
and there  is only is a balance between pressure gradient and viscosity) for the flux the lubrication's one of Shah:
$$(hu)_0=-\frac{h_0^2 (h_0 )}{3}(\frac{\partial p_0}{\partial x})-\frac{R}{2}{h_0}^2 (h_1 )(\frac{\partial p_1}{\partial x})
\text{  and  }
(hu)_1=-\frac{h_0^2 h_1 }{2}(\frac{\partial p_0}{\partial x})-\frac{{Rh_1}^2   (3 {h_0}+{h_1}M)}{3} \frac{\partial p_1}{\partial x} $$


- where   $q$ is the (hydrostatic)
pressure deviation $q(z)  = \int_0^z g \frac{\Delta \rho(T)}{\rho}dz$ due to (small) density variations which are described by an artificial scalar field $T$,
 which is
...
event init (i = 0)
{
    foreach(){
        ...
        h[0,0,0]= (0.5*(fabs(x-.5)<.5))/2;
        h[0,0,1]= (0.5*(fabs(x-.5)<.5))/2;  // plus haut plante??
...
$$(hu)_0=-\frac{h_0^2 (h_0 )}{3}(\frac{\partial p_0}{\partial x})-\frac{R}{2 }{h_0}^2 (h_1 )(\frac{\partial p_1}{\partial x})
\text{  and  }
(hu)_1=-\frac{h_0^2 h_1 }{2}(\frac{\partial p_0}{\partial x})-\frac{R{h_1}^2   (3 {h_0}+{h_1}M)}{3} \frac{\partial p_1}{\partial x}$$
...
$$
 \left\{\begin{array}{l}
  \partial_t (u)_0+=
   ...
   h0^3 (3 h_0 + 4 h_1 M)} \\
  \partial_t (u)_1=
  + \frac{(18 u_0 - 12 u_1)}{Rh_1^2(3 h_0 + 4 h_1 M)}
      \end{array}\right.
$$
as (with the already defined $C_{ij}$) :
...
event friction (i++) {
  foreach()
  {
      ...
      u.x[0, 0, 0] = (pow(h0, 3) * ((R * pow(h1, 2) * (3 * h0 + 4 * h1 * M)) + 12 * dt) * U0 + 18 * R * dt * h0 * pow(h1, 2) * U1) * pow((R * pow(h1, 2) * pow(h0, 3) * (3 * h0 + 4 * h1 * M) + 12 * dt * pow(h0, 3) + 36 * dt * h0 * pow(h1, 2) + 12 * R * M * dt * pow(h1, 3) + 36 * pow(dt, 2)), -1);

      u.x[0, 0, 1] = (18 * dt * pow(h0, 3)) * pow((R * pow(h1, 2) * pow(h0, 3) * (3 * h0 + 4 * h1 * M) + 12 * dt * pow(h0, 3) + 36 * dt * h0 * pow(h1, 2) + 12 * R * M * dt * pow(h1, 3) + 36 * pow(dt, 2)), -1) * U0 + (R * pow(h1, 2) * (3 * h0 + 4 * h1 * M)) * pow((R * pow(h1, 2) * (3 * h0 + 4 * h1 * M) + 12 * dt), -1) * U1 + (18 * pow(dt, 2) * R * 18 * h0 * pow(h1, 2)) * pow((R * pow(h1, 2) * (3 * h0 + 4 * h1 * M) + 12 * dt), -1) * pow((R * pow(h1, 2) * pow(h0, 3) * (3 * h0 + 4 * h1 * M) + 12 * dt * pow(h0, 3) + 36 * dt * h0 * pow(h1, 2) + 12 * R * M * dt * pow(h1, 3) + 36 * pow(dt, 2)), -1) * U1;     
  }   
}
