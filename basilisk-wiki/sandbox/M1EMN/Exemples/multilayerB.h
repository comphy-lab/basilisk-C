...
void vertical_viscosity (Point point, double h, vector * ul, double dt)
{
  if (nu == 0.)
    return;
/**
 define an equivalent viscosity $\nu_{eq}$
*/ 
  double nueq[nl];
...
/**
  definition of viscosity (it is a function of shear: Bingham flow)
   ...
   where $\nu$ is the viscosity and $B$ is the yeld stress, we define an equivalent viscosity:
$$\nu_{eq}=\nu + \frac{B}{|\frac{\partial u}{\partial z}|}$$
Note teh regularization introduced to avoid division by zero
*/
 for (l = 1; l < nl; l++) {
     vector um = ul[l-1] ;
     vector up = ul[l] ;
     double shear = (up.x[]-um.x[])/(h*layer[l]);
     nueq[l] = nu + B/sqrt(sq(shear) + sq(0.0000000001));
    }
    nueq[0] = nueq[1];   
    /**
