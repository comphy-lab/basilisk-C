...
What happens if the dam which retains a lake is suddenly removed? That is the dam break problem ("Problème de la rupture de Barrage").
 ![animation of the dam break](damb_MLEL/animate.gif)
...
= -g h \partial_x \eta,\;\;\;\eta=h   
        \end{array}\right. 
$$
with at  $t=0$, a lake on the left $h(|x|<0,t=0)=1$ and zero water  at the right $h(|x|>0,t=0)=0$ for $x>0$.
...
run with only 128 points (less is not enough)
*/
int main() {
  X0 = -5.;
  L0 = 10.;
...
~~~bash
  qcc -DgnuX=1   -O2   -o damb_MLEL damb_MLEL.c -lm;  ./damb_MLEL  


make damb_MLEL.tst
make damb_MLEL/plots
make damb_MLEL.c.html
source ../c2html.sh damb_MLEL
open damb_MLEL.c.html
