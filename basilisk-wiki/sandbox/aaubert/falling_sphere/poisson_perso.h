/**
This is the same file as poisson.h with an extra condition to avoid that the number of relaxation increase when we are close to the tolerance */
...
#if 1
    if (s.resa > tolerance) {
      if (resb/s.resa < 1.2 && s.nrelax < 100 && s.resa>10.*tolerance)  //if we are close from the tolerance, we stop increasing the number of relaxation
	s.nrelax++;
