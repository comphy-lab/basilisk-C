/**
This file is the same as in embed.h with a correction for the computation of the force on the solid in the axisymmetric case and a correction for the advection in case of axisymmetric simulation*/
...
#if AXI
      area*=Delta*2.*pi*(y+b.y*Delta); //the surface needs to be mutliply by the radius to obtain the real surface
#else
...
void update_tracer (scalar f, face vector uf, face vector flux, double dt)
{
  ...
  foreach() {
    ...
    *f* in time. */
      ...
      foreach_dimension()
        f[] += dt*(flux.x[] - flux.x[1])/(Delta*cm[]+SEPS);  //change : add cm
      e[] = 0.;
    }
    ...
    already stores the product $u_f \, f_m$. */
      ...
      embedded fractions. */
        ...
        double scs = 0.;
        foreach_neighbor(1)
          scs += cm[]*cs[];  //We change sq(cm[])   //Correct?
        e[] = (dt - dtmax)*F*cm[]/(scs+SEPS);
      }
    }
  }
