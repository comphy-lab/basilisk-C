...
void advect (scalar * tracers, face vector hu, face vector hf,
	     double dt)
{
...
/**
The function below advect the tracers with a source term */

void advect_tracer (scalar * tracers, face vector hu, face vector hf,
	     double dt, scalar * source)
{
  ...
  foreach_layer() {
	...
	double g = s.gradient ?
	  s.gradient (s[i-1], s[i], s[i+1])/Delta :
	  (s[i+1] - s[i-1])/(2.*Delta);
	double s2 = s[i] +(src[] + src[-1])*dt/(4.*hf.x[])+ a*(1. - a*un)*g*Delta/2.;  //modification h in place of hf.x
