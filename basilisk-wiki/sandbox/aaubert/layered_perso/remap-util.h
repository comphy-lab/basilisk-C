                           ...
                           double f_t, double lambda_t, double df_t)
{
  ...
  double M[4][4], B[4];
  for (int i = 0; i < 4; i++) {
    double zeta1 = (x[i+k-1] - xk)/dx, z1n = zeta1;
    double zeta0 = (x[i+k] - xk)/dx, z0n = zeta0;
  ...
  if (k == 0) {
    ...
    changes on equation of the default linear system to */
  ...
  because at this location $x'=1$ for the top layer and $x'=0$ for the
  bottom layer. */
