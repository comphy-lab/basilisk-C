...
solver, with the addition of a density tracer on one side of the interface.
...
The default 'rho' is overloaded in the main c file: 
...
/**
We will transport the two components of the momentum. We will need to
impose boundary conditions which match this definition. This is done
using the functions below.

The q1_x component is adjusted to take into account the density tracer: */

foreach_dimension()
static double boundary_q1_x (Point neighbor, Point point, scalar q1, void * data)
{
  return (clamp(f[],0.,1.)*(clamp(f3[],0.,1.)*(rho3 - rho1) + rho1))*u.x[];
}
...
static void prolongation_q1_x (Point point, scalar q1) {
  foreach_child()
    q1[] = (clamp(f[],0.,1.)*(clamp(f3[],0.,1.)*(rho3-rho1) + rho1))*u.x[];
}
...
event vof (i++) {
  ...
  foreach()
    foreach_dimension() {
      ...
      q1.x[] = fc*(clamp(f3[],0.,1.)*(rho3-rho1) + rho1)*u.x[];
      q2.x[] = (1. - fc)*rho2*u.x[];
    }
