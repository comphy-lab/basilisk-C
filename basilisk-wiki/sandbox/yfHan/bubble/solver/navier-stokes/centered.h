/**
This is a slightly modified version of [centered.h](/src/navier-stokes/centered.h), which takes into account mass exchange between two phases.
*/

/**
# Incompressible Navier--Stokes solver (centered formulation) with mass transfer.
...
$$
$$
\nabla\cdot\mathbf{u} = \dot{m}\left(\frac{1}{\rho_d} - \frac{1}{\rho_c}\right)\delta_\Sigma
$$
with the deformation tensor 
$\mathbf{D}=[\nabla\mathbf{u} + (\nabla\mathbf{u})^T]/2$. The densities of the disperse (gas) and continuous (liquid) phases are $\rho_d$ and $\rho_c$, respectively; the mass transfer rate is $\dot{m}$.
...
#include "bcg.h"
#if EMBED
# include "../phase_change/viscosity-embed.h"
#else
# include "../phase_change/viscosity.h"
#endif
...
/**
If PHASE_CHANGE is defined, we need the (redistributed) mass transfer field.
*/
...
t/2$ then project it to satisfy the continuity equation. We can then use it to
compute the velocity advection term, using the standard
Bell-Collela-Glaz  scheme for each component of the velocity
field. If phase change occurs, we take into account the mass transfer in the projection step.
*/
...
event advection_term (i++,last)
{
  if (!stokes) {
    prediction();
#ifdef PHASE_CHANGE
    mgpf = project (uf, pf, alpha, dt/2., mgpf.nrelax, m);
#else
...
next timestep). Then compute the centered gradient field *g*. If phase change occurs, we take into account the mass transfer in the projection step.
*/

event projection (i++,last)
{
#ifdef PHASE_CHANGE
  mgp = project (uf, p, alpha, dt, mgp.nrelax, m);
#else
