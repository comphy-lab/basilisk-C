/**
This is a copy of [poisson.h](/src/poisson.h), where mass transfer is taken into account in the [projection](/sandbox/ggennari/phase_change/poisson.h#projection-of-a-velocity-field) step. 
*/
...
The function below "projects" the velocity field *u*
$$
\mathbf{u}_f^{n+1} \leftarrow \mathbf{u}_f - \Delta t\alpha\nabla p
$$
so that the continuity equation is satisfied:
$$
\nabla\cdot\mathbf{u}_f^{n+1} = \dot{m}\left(\frac{1}{\rho_d} - \frac{1}{\rho_c}\right)\delta_\Sigma
$$
where $\dot{m}$ is the mass transfer term and $\rho_d$ ($\rho_c$) is the density of the disperese (continuous) phase.
...
struct Project {
  ...
  (const) scalar m;  // optional: default zeroc
};
...
mgstats project (struct Project q)
{
  ...
  (const) scalar m = q.m.i ? q.m : zeroc;
  
  /**
  We allocate a local scalar field and compute the divergence of
  $\mathbf{u}_f$. The divergence is scaled by *dt* so that the
  pressure has the correct dimension. We add the contribution of mass transfer by  using a redistributed version of $\dot{m}$ that alreday takes into account the desnities ($\rho_d, \rho_c$) and $\delta_\Sigma$, i.e.
  $$
  m \leftarrow \dot{m}\left(\frac{1}{\rho_d} - \frac{1}{\rho_c}\right)\delta_\Sigma
  $$
  see [Gennari et al.,2022](#gennari2022) for details.
  */
