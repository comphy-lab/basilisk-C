...
This is a reference solution obtain using the two-phase Navier-Stokes solvers an
[no-coalescence.h](no-coalescence.h), but without the momentum conserving
advection scheme. 
...
Here, colors indicate the continuous blobs detected by
[no-coalescence.h](no-coalescence.h).

![Baseline results from `collision2-ref.c`](collision2-ref/interfaces.mp4)
