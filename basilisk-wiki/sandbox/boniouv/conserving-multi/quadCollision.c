/**
# Collision of 4 droplets at the center of a periodic box

This test case is an alternative of the 
[collision of 3 droplets](triCollision.c).
*/
...
/**Initializes the domain with four droplets at equidistance 
...
event init (t=0)
{
  ...
  Positions[0].x = 0;
  Positions[0].y = 1.5*D1;
  Positions[1].x = 0;
  Positions[1].y = -1.5*D1;
  Positions[2].x = 1.5*D1;
  Positions[2].y = 0;
  Positions[3].x = -1.5*D1;
  Positions[3].y = 0;
  foreach() {
    coord center = {x,y};
    ...
    if (sq(x-Positions[3].x)+sq(y-Positions[3].y) < sq(0.5*D1)) {
      u.x[] = u_d;
      u.y[] = 0;
    }
  }
...
/** Outputs a video of the vorticity.
*/
...
/**
![Collision of 4 droplets. The color field is the
