...
static void sweep_x (scalar c, scalar cc, scalar * tcl)
{
  ...
  foreach_face(x, reduction (max:cfl)) {
    ...
    if (!c.fsum) {
      ...
      /**
      If *fsum* is activated, then the upwind volume fraction
      *cf* is the sum of all other volume fraction fields stored in
      *cfsum* */
      cf = cfsum.x[];
...
void vof_advection (scalar * interfaces, int i)
{
  ...
  /**
  We finally advance the vof field with attribute *fsum* if it exists 
  with the same tourines as for the other vof fields.
  This needs to be done after all *interfaces* in the list have been 
  transported. */
  int ncomp = 0;
  for (scalar c in interfaces) {
    if (c.fsum) {
      ncomp += 1;
      assert (ncomp < 2); // only one field can be the sum of all other fields
