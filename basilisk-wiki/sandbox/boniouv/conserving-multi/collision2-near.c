...
This test case aims to validate the extension of [the momentum conservation
algorithm](/src/navier-stokes/centered.h) to the [multi-VOF
method](no-coalescence.h). 
...
At $t=0$, the droplets are close enough to trigger
[no-coalescence.h](no-coalescence.h). Here, colors indicate the continuous blobs
detected by [no-coalescence.h](no-coalescence.h).

![Results from `collision2-near.c`](collision2-near/interfaces.mp4)
...
data2 = np.loadtxt('../collision2-ref/out', delimiter=' ', usecols=[0,1])
plt.plot(data1[:,0], data1[:,1], ls='--', lw=1.4, label='Multi-vof w/ conserving');
plt.plot(data2[:,0], data2[:,1], ls='-', lw=1.4, label='Reference w/o conserving');
plt.legend(loc='best');
plt.xlabel('Time')
...
~~~pythonplot Evolution of the elastic energy
import numpy as np
import matplotlib.pyplot as plt
	
plt.figure()
data1 = np.loadtxt('out', delimiter=' ', usecols=[0,2])
data2 = np.loadtxt('../collision2-ref/out', delimiter=' ', usecols=[0,2])
plt.plot(data1[:,0], data1[:,1], ls='--', lw=1.4, label='Multi-vof w/ conserving');
plt.plot(data2[:,0], data2[:,1], ls='-', lw=1.4, label='Reference w/o conserving');
plt.legend(loc='best');
plt.xlabel('Time')
...
plt.xlim([0,3])
plt.tight_layout()
plt.savefig('plot_EE_vs_time.svg')
~~~
...
event init (t=0){
  ...
  drop_centers[0] = (coord) { 0.8, 0.8};
  drop_centers[1] = (coord) {-0.8,-0.8};
