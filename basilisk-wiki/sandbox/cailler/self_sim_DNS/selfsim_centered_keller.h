/**
# Self-similar centered Navier--Stokes solver ([Keller \& Miksis problem](http://basilisk.fr/sandbox/cailler/keller_miksis/keller_fig2_conv.c))
...
/**
We need to store the number of dimensions used during the simulation 
(2D or 3D): */
#if AXI 
  double Nd = 3.; 
#else
...
p[right] = neumann (neumann_pressure(ghost) 
  + (fm.n[ghost]/alpha.n[ghost])*((2./3.)*x*(u.n[1]-u.n[])/Delta)) ;
  
p[left]  = neumann (- neumann_pressure(0)
  - (fm.n[0]/alpha.n[0])*((2./3.)*x*(u.n[]-u.n[-1])/Delta));
...
p[top]    = neumann (neumann_pressure(ghost)
  + (fm.n[ghost]/alpha.n[ghost])*((2./3.)*y*(u.n[0,1]-u.n[])/Delta));
...
#  if dimension > 1
p[top]    = neumann (neumann_pressure(ghost)
  + (2./3.)*y*(u.n[0,1]-u.n[])/(Delta*alpha.n[ghost]));
p[bottom] = neumann (- neumann_pressure(0) 
  + (2./3.)*y*(u.n[]-u.n[0,-1])/(Delta*alpha.n[0]));
