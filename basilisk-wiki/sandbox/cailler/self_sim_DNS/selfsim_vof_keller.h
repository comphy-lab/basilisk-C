...
The face velocity field `lambdaf` will be defined by a solver as well
as the timestep. */
...
extern double dt;
extern vector n_front;
extern scalar alpha_front;
...
static void sweep_x (scalar c, scalar cc, scalar * tcl)
{
  ...
  boundary({c});
  boundary({n_front, alpha_front});

  vector n = n_front; //`n` & `alpha` linked to their new defs (see user file)
  ...
  foreach_face(x, reduction (max:cfl)) {
    ...
    double un = lambdaf.x[]*dt/(Delta*fm.x[] + SEPS), s = sign(un);
    int i = -(s + 1.)/2.;
    ...
    Bell--Colella--Glaz scheme and the gradient computed above. */
  ...
  equation
  $$
  \partial_tc = -\nabla_x\cdot(\mathbf{\overline{\Lambda}}_f c) 
  + c\nabla_x\cdot\mathbf{\overline{\Lambda}}_f
  $$
  ...
  $$
  \partial_tt_j = -\nabla_x\cdot(\mathbf{\overline{\Lambda}}_f t_j) 
  + t_j\nabla_x\cdot\mathbf{\overline{\Lambda}}_f
  $$
...
#if !EMBED
  foreach() {
    c[] += dt*(flux[] - flux[1] + cc[]*(lambdaf.x[1] - lambdaf.x[]))/(cm[]*Delta);
    ...
    for (t, tc, tflux in tracers, tcl, tfluxl)
      t[] += dt*(tflux[] - tflux[1] + tc[]*(lambdaf.x[1] - lambdaf.x[]))/(cm[]*Delta);
  ...
  foreach()
    if (cs[] > 0.) {
      c[] += dt*(flux[] - flux[1] + cc[]*(lambdaf.x[1] - lambdaf.x[]))/Delta;
#if NO_1D_COMPRESSION
      for (t, tflux in tracers, tfluxl)
	t[] += dt*(tflux[] - tflux[1])/Delta;
      ...
      scalar t, tc, tflux;
      for (t, tc, tflux in tracers, tcl, tfluxl)
	t[] += dt*(tflux[] - tflux[1] + tc[]*(lambdaf.x[1] - lambdaf.x[]))/Delta;
#endif // !NO_1D_COMPRESSION
    }
#endif // EMBED
