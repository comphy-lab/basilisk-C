...
As we are expecting that there will be no pressure gradient or transverse 
velocity $\overline{v}$, as well as having $\partial_\xi \overline{u} = 0$ due 
to the longitudinal rectilinear flow, 
we can solve numerically the following modified equations 
without considering the proper scalings in front of these specific terms and their 
derivatives
(we should have rigorously divergent time exponentials of different time scales 
appearing in front of $\overline{v}$, $\partial_\xi \overline{u}$, 
$\partial_\eta \overline{v}$, $\partial_\xi \overline{p}$, 
$\partial_\eta \overline{p}$, $\partial_{\xi \xi} \overline{u}$, etc., **but we 
do not care**, as they will vanish in a few iterations, due to the set initial and 
boundary conditions; **this is a numerical trick that works well only with this 
special problem**):
...
\textcolor{green}{
    - \dfrac{1}{2} \mathbf{\overline{u}}^{n+1/2}
}
$$
$$
...
\left\{ 
  ...
  \, \boldsymbol{\xi}}  \\
  \boldsymbol{\xi} &=& {}^{\mathrm{t}}(0, \, - \eta) / 2 \\ 
...
/**
This problem is a special case where we do not have a self-similar scaling in 
the $x$ direction, therefore 
we can simply take into account this property by setting to one 
the parameter for the number of dimensions: */
double Nd = 1.;
...
// p[top]    = neumann (neumann_pressure(ghost) - 1.);
// p[bottom] = neumann (- neumann_pressure(0) + 1.) ;
p[top]    = neumann (neumann_pressure(ghost));
p[bottom] = neumann (- neumann_pressure(0)) ;
...
void prediction_uf()
{
  ...
  foreach_face() {
    ...
    double s = sign(un);
    int i = -(s + 1.)/2.;
    uf.x[] = u.x[i] + ((g.x[] + g.x[-1] + .5*(u.x[] + u.x[-1])))*dt/4. 
