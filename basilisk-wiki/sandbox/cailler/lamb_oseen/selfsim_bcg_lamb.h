/**
# SELF-SIMILAR Bell-Colella-Glaz advection scheme (*Lamb--Oseen* vortex)
...
face metric), a timestep *dt*, and a source term field *src*, it fills the face 
vector field *flux* with the components of the advection fluxes of *f*. */
		    ...
		    (const) scalar src)
{
  ...
  \textcolor{green}{
    ...
    + \overline{g}_d^n 
    \textcolor{green}{ + \overline{u}_d^n}    
    $$
...
void selfsim_advection (scalar * tracers, face vector u, double dt, 
		scalar * src = NULL)
{
  ...
  for (f,source in tracers,src) {
    face vector flux[];
    selfsim_tracer_fluxes (f, u, flux, dt, source);
#if !EMBED
    foreach()
      foreach_dimension()
