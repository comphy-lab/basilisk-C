/**
# Self-similar centered Navier--Stokes solver ([*Lamb--Oseen* vortex](http://basilisk.fr/sandbox/cailler/lamb_oseen/lamb.c))
...
\left\{ 
  ...
  &\textcolor{Orchid}{=}& 
  \textcolor{Orchid}{\mathrm{Re} \, \overline{\mathbf{u}} - \dfrac{1}{2} 
  \, \boldsymbol{\xi}}  \\
  ...
  &=& \left( 
    ...
    + {}^{\mathrm{t}} \boldsymbol{\nabla} \overline{\mathbf{u}} 
  \right)/2 \\
  \overline{p}_\star &=& \mathrm{Re} \, \overline{p} \\
...
#include "run.h"
#include "timestep.h"
#include "selfsim_bcg_lamb.h"
#include "viscosity.h"


/**
The primary variables are the centered self-similar 
pressure field $\overline{p}_\star$, the
...
centered pressure field $\overline{p}_\star^f$ and a *temporary* 
...
double Rey ; // Reynolds number to be defined in user file
...
p[right] = neumann (neumann_pressure(ghost) 
  + (fm.n[ghost]/alpha.n[ghost])*((1./2.)*x*(u.n[1]-u.n[])/Delta)) ;
  
p[left]  = neumann (- neumann_pressure(0)
  - (fm.n[0]/alpha.n[0])*((1./2.)*x*(u.n[]-u.n[-1])/Delta));
...
lambdaf.n[right] = Rey*uf.n[ghost] - (1./2.)*fm.n[ghost]*xi.n[ghost] ;
lambdaf.t[right] = Rey*uf.t[ghost] - (1./2.)*fm.t[ghost]*xi.t[ghost] ; 
lambdaf.n[left] = Rey*uf.n[ghost] - (1./2.)*fm.n[ghost]*xi.n[ghost] ;
lambdaf.t[left] = Rey*uf.t[ghost] - (1./2.)*fm.t[ghost]*xi.t[ghost] ;
...
p[top]    = neumann (neumann_pressure(ghost)
  + (fm.n[ghost]/alpha.n[ghost])*((1./2.)*y*(u.n[0,1]-u.n[])/Delta));
...
lambdaf.n[top] = Rey*uf.n[ghost] - (1./2.)*fm.n[ghost]*xi.n[ghost] ;
lambdaf.t[top] = Rey*uf.t[ghost] - (1./2.)*fm.t[ghost]*xi.t[ghost] ;
lambdaf.n[bottom] = Rey*uf.n[ghost] - (1./2.)*fm.n[ghost]*xi.n[ghost] ;
lambdaf.t[bottom] = Rey*uf.t[ghost] - (1./2.)*fm.t[ghost]*xi.t[ghost] ; 

#else // !AXI
#  if dimension > 1
p[top]    = neumann (neumann_pressure(ghost)
  + (1./2.)*y*(u.n[0,1]-u.n[])/(Delta*alpha.n[ghost]));
p[bottom] = neumann (- neumann_pressure(0) 
  + (1./2.)*y*(u.n[]-u.n[0,-1])/(Delta*alpha.n[0]));
...
event init (i = 0)
{
  ...
  boundary((scalar *){uf});
  foreach_face()
    lambdaf.x[] = Rey*uf.x[] - (1./2.)*fm.x[]*face_value (xi.x, 0);
...
[advection scheme](http://basilisk.fr/sandbox/cailler/lamb_oseen/selfsim_bcg_lamb.h) 
...
\mathbf{\overline{u}}_{f,p}^{n+1/2} 
    ...
    \mathbf{BCG} \left( 
      ...
      \textcolor{green}{ + \left. \mathbf{\overline{u}}^n \right|_{c \to f} }
    \right)
$$
...
void prediction_uf()
{
  ...
  foreach_face() {
    double un = (dt/Delta)*(Rey*(u.x[] + u.x[-1]) - (1./2.)*(xi.x[] + xi.x[-1]))/2.;
    double s = sign(un);
    int i = -(s + 1.)/2.;
    uf.x[] = u.x[i] + ((g.x[] + g.x[-1]) + 1.*(u.x[] + u.x[-1]))*dt/4. 
      + s*(1. - s*un)*du.x[i]*Delta/2.; 
    #if dimension > 1
    if (fm.y[i,0] && fm.y[i,1]) {
      double fyy = (Rey*u.y[i] - (1./2.)*xi.y[i]) < 0. ? u.x[i,1] - u.x[i] : u.x[i] - u.x[i,-1];
      uf.x[] -= (dt/2.)*(Rey*u.y[i] - (1./2.)*xi.y[i])*fyy/Delta; 
    }
    #endif
    #if dimension > 2
...
*/
    ...
    selfsim_advection ((scalar *){u}, lambdaf, dt, (scalar *){g});
    boundary((scalar *){lambdaf});
    boundary((scalar *){u});
    boundary({xi});
  }
...
event acceleration (i++,last)
{
  trash ({uf});
  foreach_face(){
      uf.x[] = fm.x[]*(face_value (u.x, 0) + dt*a.x[]) - .5*dt*uf_temp.x[];
  }
  boundary((scalar *){uf});
}
...
and the self-similar cell-centered pressure field $\overline{p}_\star$. */
