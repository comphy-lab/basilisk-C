...
// We need to compute correctly for both `n` and `alpha` their direction and 
// value due to the presence of fluxes on top and left boundaries. 
// Indeed, they are set by default for symmetry conditions, inducing errors when 
// complex inflows exist.




// coord ntemp = {cos(BETA_0 + M_PI/2)/(fabs(cos(BETA_0 + M_PI/2)) + fabs(sin(BETA_0 + M_PI/2))), sin(BETA_0 + M_PI/2)/(fabs(cos(BETA_0 + M_PI/2)) + fabs(sin(BETA_0 + M_PI/2)))};
// coord ntemp = {
...
// // alpha_front[bottom] = plane_alpha (f[ghost], ntemp);
...
static void sweep_x (scalar c, scalar cc, scalar * tcl)
{
  ...
  vector n = n_front; // we link `n` and `alpha` to their new definitions (see above)
