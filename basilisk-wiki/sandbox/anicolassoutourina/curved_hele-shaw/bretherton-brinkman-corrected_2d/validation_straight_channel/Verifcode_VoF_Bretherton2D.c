/* ----------------------------------------------------------------------------------------------------------------------- */
/*
Attempt at a 2.5D model : 2D simulation while adding Bretherton forces via acceleration add-on at the interface, from Booth and Wu
*/
/* ----------------------------------------------------------------------------------------------------------------------- */
...
const double Ca = pow(0.894*(0.05/(2*0.1))*1., 3) ; // Capillary number, defined from Katie's Bretherton, initializing of a const with * and / operations so have to initialize by hand
...
// Validation with sym at walls

/* Inlet and outlet boundary conditions */

u.n[bottom] = dirichlet(U_inlet) ; // for validation
u.n[top] = neumann(0.) ; // for validation
p[top] = dirichlet(0.);
...
int main(){
  ...
  f.sigma = (pi/4)*(mu1*U_inlet)/Ca; // Surface tension, defined with Ca, corrected with pi/4 as in Meiburg, 1989, "Bubbles in a Hele–Shaw cell: Numerical simulation of three‐dimensional effects"
 
  fprintf(stderr, "Re = %f, Ca = %f, mu1 = %f, pi/4*sigma = %f",Re,Ca,mu1,f.sigma); 
...
// For the Bretherton forces, interface_x_center needs to be called once for the whole of each iteration, otherwise each interface cell calls it, O(n*n^2) 
...
double x_bubble_slice = bubble_init_x, y_bubble_slice = bubble_init_y, p_bubble = 0. ;
...
/* Bretherton forces, as described in Booth, 2022, "Circular bubbles in a Hele-Shaw channel: a Hele-Shaw Newton’s cradle" :

p_bubble - p_ext = (2 sigma / h) * (1 + beta * Ca^{2/3}) + 2D curvature effect

where   -> Ca = mu1 * vect{u}.vect{outward normal} / sigma
	-> beta = 3.88 if vect{u}.vect{outward normal} > 0
		  -1.33 otherwise

*/

// Helper function to return nb^(2/3) with the correct sign
double power_twothirds(double nb){
  if (nb < 0){
    return - cbrt(pow(-nb,2));
  }
  else{
    return cbrt(pow(nb,2));
  }
...
// Overloading of the acceleration event to add our custom (spatially varying) forces
event acceleration (i++) {
  foreach_face(){ // acceleration is a face vector
  // Brinkman correction 
  foreach_dimension(){
   a.x[] += (-mu1 * u.x[]) / (pow(h_channel,2)/12); 
  }
  // Bretherton forces
    if (interfacial(point,f)){ // interface cell
      coord m = mycs(point, f), fc ;// normal to the interface, from "curvature.h"
      double alpha = plane_alpha(f[],m) ;
      double face_area = h_channel * plane_area_center(m, alpha, &fc) ; // Reconstructed interface area
      
      double u_n = -m.x * u.x[] -m.y * u.y[] ; // normal velocity, - to have normal pointing outward from the bubble
      double beta = (u_n > 0.) ? 3.88 : -1.33 ;
      double Ca_n_2_3 = power_twothirds(mu1 * u_n / ((4/pi)*f.sigma)) ; // f.sigma corrected from the pi/4 of Meiburg, 1989
      double delta_pressure = -(2* (4/pi)*f.sigma / h_channel)*(1 + beta * Ca_n_2_3) ; // Pressure addition, as in Booth and Wu, - instead of + bc : p_bubble - p_ext = Dp
           
      foreach_dimension(){ // Projection of pressure on interface as a force
        a.x[] += m.x * delta_pressure * face_area ;
      }
    }
  }
}
...
event movie (t += (t_max/n_frames)) {
  ...
  squares ("u.y", linear = true); // Draws y velocity component inside squares
  ...
  // Acceleration (forces) images
    
  static FILE * xaccfile = fopen ("x-acceleration.ppm","w");
  output_ppm(a.x, xaccfile, linear = true);
  static FILE * yaccfile = fopen ("y-acceleration.ppm","w");
  output_ppm(a.y, yaccfile, linear = true);
