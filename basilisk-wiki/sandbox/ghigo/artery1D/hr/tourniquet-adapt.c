...
/**
We choose here to use an adaptive cartesian grid to capture the sharp
profile of the perturbation of the artery. */
...
We first the distribution of the mesh levels throughout the artery at
$t={0, 0.01, 0.02, 0.03, 0.04}$ for $lmax=14$.
...
We then plot the spatial evolution of the cross-sectional area $a$ at
$t={0, 0.01, 0.02, 0.03, 0.04}$ for $lmax=14$.

~~~gnuplot $a/a_0$ for $lmax=14$. 
reset
set xlabel 'x'
set ylabel 'a/a_0'
...
area $a$ and the flow rate $q$ with the number of cells $N_{max}$. We
compare the results with those obtained with a [uniform
grid](tourniquet.c).
