/**
# Cylinder moving infinitely slowly towards to a plane wall in a viscoplastic Stokes flow

We consider here a semi-infinite domain and compute the hydrodynamic
normal force $F_n$ acting on a cylinder held fixed but with a velicity
$U$ at its boundary. The value of the force $F_n$ depends on the "gap"
distance $\delta$ between the cylinder and the wall.

We solve here the 2D Stokes equations and add the cylinder using an
...
#endif // DLENGTH
#define cylinder(x,y,diam) (sq ((x)) + sq ((y)) - sq ((diam)/2.))
...
#define T1 (1000./sqrt (2.)) // Yield stress
face vector Tv[];
...
mesh refinement (BMR) around the cylinder. */

#define square(x,hx,y,hy) (union (					\
				  union ((x) - (hx)/2., -(x) - (hx)/2.), \
				  union ((y) - (hy)/2., -(y) - (hy)/2.)))
...
event init (i = 0)
{
  ...
  if (!restore (file = "restart")) {
    ...
    if ((lmem) < (lmin)) {
      refine ((cylinder ((x - p_p.x),
			 (y - p_p.y),
			 2.*((L0)/(1 << (lmem))))) < 0. &&
	      level < (lmin));
      p_shape (cs, fs, p_p);
    }
    // Coarse grid far from the cylinder
    unrefine ((cylinder ((x - p_p.x),
			 (y - p_p.y),
			 (3.*(d)))) > 0. &&
	      level > (1));
    p_shape (cs, fs, p_p);
    // Fine grid (lmax) around the cylinder and coarse grid far from the cylinder
    int lvl = (lmin);
    while (lvl < (lmax)) {
      refine ((cylinder ((x - p_p.x),
			 (y - p_p.y),
    ...
    // Fine grid in the gap and around the cylinder
    lvl = (lmin);
    while (lvl < (lmax)) {
      refine ((wall ((x),   ((h) - (d)/2.*(gap)))) >= 0. &&
	      (cylinder ((x - ((h) - (d)/2.*(gap))),
			 (y - p_p.y),
    ...
    // Unrefine in the cylinder and in the wall
    unrefine ((
    	       (cylinder ((x - p_p.x),
			  (y - p_p.y),
			  ((d) - 2.*(L0)/(1 << (lmax))))) < 0.  ||
...
event adapt (i++)
{
  adapt_wavelet ({cs,u}, (double[]) {1.e-1,(cmax),(cmax)},
  		 maxlevel = (lmax), minlevel = (1));
...
event logfile (i++; t <= 10.*(tref))
{
  ...
  vertex scalar phi[];
  foreach_vertex()
    phi[] = (cylinder ((x - p_p.x), (y - p_p.y), (d)));
  boundary ({phi});
  fractions (phi, col);
  boundary ((scalar *) {col});

  /**
  We compute the hydrodynamic forces acting on the cylinder. */

  coord Fp, Fmu;
  embed_color_force (p, u, mu, col, &Fp, &Fmu);
  double F  = fabs (Fp.x + Fmu.x);
  double dF = fabs (F - Fm1);
  Fm1 = F;
  
  fprintf (stderr, "%g %g %d %d %d %g %g %d %d %d %d %d %d %g %g %g %g %g %g\n",
	   (gap), (L0)/(d), (lmin), (lmax),
	   i, t/(tref), dt/(tref),
...
![Initial mesh around the embedded boundaries](cylinder-near-wall-viscoplastic/vof-t-0.png)
...
set xrange [0:5]
set yrange [0:100]
plot 'log' u 6:18 w l lw 2 lc rgb 'red'   t 'Basilisk, {\delta}/a=0.4'
~~~
