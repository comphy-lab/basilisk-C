/**
# Incompressible flow past a Jabba the Hutt */
...
#define Re   (20.)      // Reynolds number, u*l/nu
#define l    (25.)       // Characteristic size of the Jabba the Hutt
...
#define lmin (4) // Min mesh refinement level (l=4 is 3pt/l)
#define lmax (9) // Max mesh refinement level (l=8 is 50pt/l)
#define cmax (1.e-2*(uref)) // Absolute refinement criteria for the velocity field

int main ()
{
  /**
  The domain is $10l\times 10l\times 10l$. */
 
  L0 = 5.*(l);
  size (L0);
  origin (-(L0)/2. + 22., -(L0)/2. + 6., -(L0)/2); // Centered in x, y, z
...
We use inlet boundary conditions in the x-direction. */
...
event logfile (i++; t <= 100.*(tref))
{
  ...
  /**
  We compute the forces around Jabba the Hutt. */
  
  coord Fp, Fmu;
  embed_force  (p, u, mu, &Fp, &Fmu);

  stats nx = statsf (u.x), ny = statsf(u.y), nz = statsf (u.z), np = statsf(p);
  fprintf (stderr, "%d %g %g %d %d %d %d %d %d %g %g %g %g %g %g %g %g %g %g %g %g %g %g %g %g %g %g %g %g %g %g %g %ld %g\n",
	   i, t/(tref), dt/(tref),
...
![$\lambda_2$ criteria](flow-past-jabba-the-hutt/l2.mp4)(loop)
*/
