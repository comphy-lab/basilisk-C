...
event init (i = 0)
{
  ...
  /**
  We first define the particles' initial position. */
...
event check (i++)
{
  foreach() {
	...
	assert (dirichlet);
	assert (ub +
		p_w[0].x*(y + b.y*Delta - p_p[0].y) == 0.);
	ub = u.y.boundary[embed] (point, point, u.y, &dirichlet);
	assert (dirichlet);
	assert (ub -
		p_w[0].y*(x + b.x*Delta - p_p[0].x) == 0.);
      }
	...
	assert (dirichlet);
	assert (ub +
		p_w[1].x*(y + b.y*Delta - p_p[1].y) == 0.);
	ub = u.y.boundary[embed] (point, point, u.y, &dirichlet);
	assert (dirichlet);
	assert (ub -
		p_w[1].y*(x + b.x*Delta - p_p[1].x) == 0.);
      }
