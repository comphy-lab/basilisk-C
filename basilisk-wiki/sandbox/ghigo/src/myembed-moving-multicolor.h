...
////////////////////////////////////////////////////////////////////////// // // // // // // BEGIN from ghigo/src/myembed-moving-color.h
/**
...
// // //   if (p_col[] > 0. && p_col[] < 1.) {
	    ...
	    coord sgn = {-1, 1};
	    foreach_dimension() {
	      //dw1.x = sgn.x*     dwp.x*(r.y); //FP
	      dw1.x = sgn.x  *pl[i].aw.x*(r.y);
	      //dw2.x = - sq(   wp.x)*(r.x);
	      dw2.x = - sq(pl[i].w.x)*(r.x); // FP
	    } 
	#else // dimension = 3
	    double w1 = 0., w2 = 0.;
	    foreach_dimension() {
	    ...
	    foreach_dimension() {
	      //dw1.x =    dwp.y*(r.z) -      dwp.z*(r.y); // FP
	      dw1.x = pl[i].aw.y*(r.z) - pl[i].aw.z*(r.y);
	      //dw2.x =    w1*wp.x - w2*(r.x);//FP
	      dw2.x = w1*pl[i].w.x - w2*(r.x);
	    }
	#endif // dimension
	    ...
	    foreach_dimension()
	      //gu.x =    dup.x + dw1.x + dw2.x;//FP
	      gu.x = pl[i].au.x + dw1.x + dw2.x;
...
//	     } 
	  ...
	  // Other fixed embedded boundaries (gradp=0)
	  else {
	    foreach_dimension()
	      da->x = 0.;
	  }
	}// loop on particles, FP
}
					      ...
					      double xc, double yc, double zc)
{
  ...
  pressure_acceleration (point, pl, xc, yc, zc, &da);
  return da.x;
}
...
*neumann_ON* to true. The default is false. */

attribute {
  bool neumann_ON ; // changed wrt Arthur's implementation, so default is pressure gradient = neumann(0.),
                    // and not the fancy BC.
}
...
///////////////////////////////////////////////////////////////////////////// // // // // // // END from ghigo/src/myembed-moving-color.h
...
event init (i = 0)
{
  ...
  *csm1* is not dumped. */
  ...
  //p[embed]  = neumann (0);//FP
  //pf[embed] = neumann (0);//FP
	/**
	Adding non-zero pressure gradient...hope to get the lubrication effects! */
	p[embed]  = neumann (p.neumann_ON ? pressure_neumann (point, pl, x, y, z) : 0.);
	pf[embed] = neumann (p.neumann_ON ? pressure_neumann (point, pl, x, y, z) : 0.);
  ...
  //g.n[embed] = dirichlet (0); // FP
  //g.t[embed] = dirichlet (0); // FP
  g.n[embed] = dirichlet (p.neumann_ON ? pressure_acceleration_x (point, pl, x, y, z) : 0.); 
  g.t[embed] = dirichlet (p.neumann_ON ? pressure_acceleration_y (point, pl, x, y, z) : 0.);
#else // dimension == 3
  u.n[embed]   = dirichlet (velocity_noslip_x (point, pl, x, y, z));
  u.t[embed]   = dirichlet (velocity_noslip_y (point, pl, x, y, z));
  ...
  //g.n[embed] = dirichlet (0);// FP 
  //g.t[embed] = dirichlet (0);// FP
  //g.r[embed] = dirichlet (0);// FP
  g.n[embed] = dirichlet (p.neumann_ON ? pressure_acceleration_x (point, pl, x, y, z) : 0.); 
  g.t[embed] = dirichlet (p.neumann_ON ? pressure_acceleration_y (point, pl, x, y, z) : 0.);
  g.r[embed] = dirichlet (p.neumann_ON ? pressure_acceleration_z (point, pl, x, y, z) : 0.);
#endif // dimension
