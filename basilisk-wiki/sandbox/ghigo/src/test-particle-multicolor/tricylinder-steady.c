/**
# Three fixed cylinder (moving) at the same speed as the surrounding inviscid flow
...
event init (i = 0)
{
  ...
  /**
  Next we define the cylinders' initial position. */
...
We do not advect the cylinders in this test case.

We however verify here that the velocity and pressure gradient
...
![Mesh](tricylinder-steady/mesh.png)

We plot the time evolution of the error. We observe a slow but steady
increase of the error. This is, in my opinion, the propagation of
errors made in small cells when using the variable *divfnc* in the
function [update_tracer](../myembed.h#update_tracer). This errors
could be reduced by using a slope limiter for the gradient of the
velocity or by decreasing the timestep.
