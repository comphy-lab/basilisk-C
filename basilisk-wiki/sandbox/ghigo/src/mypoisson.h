		 ...
		 int nrelax = 4)
{
  ...
  pressure has the correct dimension. We also account for a non-zero
  dirichlet boundary condition on *uf*.

  On a more general note, the Poisson equation is multiplied by *cs*
  when considering embedded boundaries, therefore the divergence term
  computed here is also multiplied by *cs*. 

  Finally, in the presence of embedded boundaries, we should use
  embed-specific prolongation and restriction operators for
  *div*. However, *div* is only used to compute the residual in leaf
  cells and its stencil is never accessed. Furtheremore, and more
  importantly, if a *res* variable is not given as a parameter of the
  *poisson* function (which is the case here), than the prolongation
  and restriction operators for the *res* variable used in *mg_solve*
  are copied from those of *div*. However, for the multigrid solver to
  work properly, the prolongation and restriction operators for the
  *res* variable should be similar to the multigrid prolongation and
