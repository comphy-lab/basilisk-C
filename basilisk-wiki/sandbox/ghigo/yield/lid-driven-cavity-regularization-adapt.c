...
{  
  ...
  /**
  We reduce the regularization parameter to perform simulations with a
  large yield stress. */
...
event logfile (t += 0.1; i <= 1000) // No convergence due to mesh adaptation
{
  double du = change (u.x, un);
  fprintf (ferr, "%d %g %g %g %d %d\n",
	   i, t, dt,
	   du,
	   mgp.i, mgu.i);
  fflush (ferr);
