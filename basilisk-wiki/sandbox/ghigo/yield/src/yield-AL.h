...
The approach is based on [Vinay et al,
2005](http://doi.org/10.1016/j.jnnfm.2005.04.005).

We define the yield stress *T*, the pseudo deformation tensor $d=D$
...
d^k = 
\begin{array}{cc}
0 & \mathrm{if} \quad ||\lambda^{k-1} + rD^k|| < T \\
\left(1 - \frac{T}{||\lambda^{k-1} + rD^k||}\right)\frac{\lambda^{k-1}
+ rD^k}{r} & \mathrm{if} \quad ||\lambda^{k-1} + rD^k|| \geq T \\
\end{array}
$$
$$
\lambda^k = \lambda_{k-1} + r(D^k - d^k).
$$
*/
...
int iAL = 1000;
double TOLERANCE_AL = 1.e-6; // Tolerance for *d-D*
...
The following two functions add and substract the Lagrangian parameter
*r/2* to the viscosity. */

static void add_to_viscosity ()
{
  face vector muv = mu;
  foreach_face()
    muv.x[] += fm.x[]*r.x[]/2.;
...
static void substract_to_viscosity ()
{
  face vector muv = mu;
  foreach_face()
    muv.x[] -= fm.x[]*r.x[]/2.;
...
/**
The next function compute $\max (d - D(U))$, used only when
...
static double delta_D()
{
  ...
  double max = 0.;
  foreach_dimension() {
    face vector Dx = D.x, dx = d.x;
    foreach_face(x) {
      Dx.x[] = (u.x[] - u.x[-1])/Delta;
      double a = fabs (dx.x[] - Dx.x[]);
      if (a > max)
	max = a;
    }
#if dimension > 1
    foreach_face(y) {
      Dx.y[] = ((u.x[] - u.x[0,-1]) + // 1/2*duxdy, dy=Delta 
		((u.y[1,-1] + u.y[1,0])/4. - // 1/2duydx, dx=2*Delta
		 (u.y[-1,-1] + u.y[-1,0])/4.))/(2.*Delta);
      double a = fabs (dx.y[] - Dx.y[]);
      if (a > max)
	max = a;
    }
#endif
#if dimension > 2
    foreach_face(z) {
      Dx.z[] = ((u.x[] - u.x[0,0,-1]) + // 1/2*duxdz 
		((u.z[1,0,-1] + u.z[1,0,0])/4. -
		 (u.z[-1,0,-1] + u.z[-1,0,0])/4.))/(2.*Delta);
      double a = fabs (dx.z[] - Dx.z[]);
      if (a > max)
	max = a;
    }
#endif
...
{ 
  /**
  We first compute $D$ and $\lambda = \lambda + rD$. */
  ...
  /**
  Finally, we udapte $d$ and $\lambda = \lambda - rd$. */
  
  foreach_dimension() {
    face vector dx = d.x, lx = lambda.x, Dx = D.x;
    foreach_face() {
      //Newtonian
      if (T.x[] == 0.) {
	dx.x[] = Dx.x[];
...
event viscous_term (i++)
{
  ...
  /**
  We add $r/2$ to the viscosity coefficient, $\mu = 2(\mu + r/2)D$. */
  
  add_to_viscosity ();

  /**
  We then add the Lagrangian constraint (eq. 3.35) to the centered
  velocity field. */
  ...
  foreach_dimension() {
    ...
    foreach() {
      ...
      foreach_dimension()
	div += (
		alpha.x[1]*(lx.x[1] - r.x[1]*dx.x[1]) -
		alpha.x[]* (lx.x[]  - r.x[]*dx.x[])
		)/(cm[]*Delta);
      u.x[] += dt*div;
    }
  }
...
/**
We remove $r/2$ from viscosity the coefficient. */

event acceleration (i++)
{
  substract_to_viscosity (); 
}
...
event end_timestep (i++)
{
  ...
  /**
  This is subcycling, which is turned on by default. This is used to
