...
/**
We need a face-centered field for the viscosity, the yield stress
*T* and the Lagrangian parameter *r*. */

face vector muv[], Tv[], rv[];
...
event properties (i++)
{
  ...
  foreach_face() {
    muv.x[] = fm.x[];
    Tv.x[]  = fm.x[]*(T1);
    rv.x[]  = 30.; // not multiplied by the metric
  }
  boundary ((scalar *) {Tv, rv});
}
...
![*u.x* velocity field](poiseuille-AL-adapt/ux.png)

![Yielded region](poiseuille-AL-adapt/yield.png)

![Error](poiseuille-AL-adapt/err.png)
...
plot 'out' u 1:3 w p ps 0.75 pt 6 lc rgb "black" t "Exact", \
     'out' u 1:2 w l lw 2 lc rgb "blue" t 'AL, adapt'
~~~
...
plot 'out' u 1:5 w p ps 0.75 pt 6 lc rgb "black" t "Exact", \
     'out' u 1:4 w l lw 2 lc rgb "blue" t 'AL, adapt'
~~~
