...
foreach_dimension()
static inline double navier_bc_x (Point point, scalar s, scalar cs,
             coord n, coord p, double lambda, double * coef)
{
  double d[2], a[2], v[2] = {nodata,nodata};
  bool defined = true;
...
//#if dimension == 2
  ...
  if (v[0] == nodata) {
    ...
    This is a degenerate case, we use the boundary value and the
    cell-center value to define the gradient. */
      d[0] = max(1e-3, fabs(p.x/n.x)); a[0] = 1/d[0];;
    *coef = -a[0]/Delta;
    return a[0]/(Delta);
  }
  *coef = 0.;  
  if (v[1] != nodata){ // third-order gradient
    a[2] = d[0]/(d[1]*(d[1]-d[0])*Delta); a[1] = -d[1]/(d[0]*(d[1]-d[0])*Delta); a[0] = (d[1]+d[0])/(d[1]*d[0]*Delta);
    return lambda*(a[1]*v[0] + a[2]*v[1])/(1-lambda*a[0]);
  }
  else
  {
    a[1] = -1/d[1]*Delta; a[0] = 1/d[1]*Delta;
    return (lambda*(a[1]*v[0]))/(1-lambda*a[0]);
  }
}

double navier_bc (Point point, scalar s, scalar cs,
             coord n, coord p, double lambda, double * coef)
{
  foreach_dimension()
    if (fabs(n.x) >= fabs(n.y))
      return navier_bc_x (point, s, cs, n, p, lambda, coef);
    return nodata;
}
// double navier_gradient (Point point, scalar s, scalar cs,   //former version
//           coord n, coord p, double * coef)
// {
//   
...
//       un[]+=u.x[]*(-n.x);
//   }
//   dundn = dirichlet_gradient (point, un, cs, n, p, 0, coef);
//   coord t = {-n.y, n.x};
//   double coefNav=0, dutdn=0;
//   dutdn = t.x*t.y/(lambda+SEPS); //Navier

//   //dutdn = 0; // if free slip

//   coefNav = ( s.i == u.x.i ? sq(t.x)/(lambda+SEPS): sq(t.y)/(lambda+SEPS) );
//   //fprintf (stdout, "in nav %g %g %g %g %g\n", dundn, dutdn, coefNav, sq(t.x)/(lambda+SEPS), sq(t.y)/(lambda+SEPS) );
//   //coefNav*=0; // free slip
//   *coef += coefNav;
//   return (s.i == u.x.i ? dutdn*u.y[]-t.y*dundn : dutdn*u.x[]+t.x*dundn);
//   //return (s.i == u.x.i ? t.x*dutdn-t.y*dundn : t.y*dutdn + t.x*dundn);
// }
...
{
  /**
  We retrieve the corresponding vector of the gradient of scalar *s* */
  assert( s.v.x.i >= 0);
  //fprintf (stdout, "in\n");
  vector u = s.v;
  double dundn=0, dutdn=0, lambda=0.;
  /** We define a scalar field *u_n*, the normal component of the velocity on the embedded boundary */
  scalar un[], ut[];
  coord t = {-n.y, n.x}; 
  foreach(){
    un[]=0, ut[]=0;
  ...
  /** Calculation on the boundary condition *bc* required to set  $$(\mathbf{v} \cdot \mathbf{t})_{bc} = \lambda \, \left. \frac{\partial \mathbf{u}_t }{\partial n} \right|_{bc}$$ on the embedded boundary */

  double bc = navier_bc (point, ut, cs, n, p, lambda, coef);
  dutdn = dirichlet_gradient (point, ut, cs, n, p, bc, coef);

  /** We also use dirichlet gradient function to set $$\mathbf{v} \cdot \mathbf{n} = 0$$ on the embedded boundary */
  dundn = dirichlet_gradient (point, un, cs, n, p, 0, coef);

  fprintf (stdout, "in nav %g %g %g %g\n", dundn, dutdn, lambda, bc );
  return (s.i == u.x.i ? t.x*dutdn-t.y*dundn : t.y*dutdn + t.x*dundn);
}


// double robin_scalar (Point point, scalar s, double * coef) // Very simple implementation of Robin boundary condition on an embedded boundary
// {
//   double a=1, c=0, b=-0.75; // slip length if Navier
//   double ratio1 =a/(b+SEPS), ratio2 = c/(b+SEPS);
//   *coef = -ratio1;
//   //fprintf (stdout, "in rob %g\n", ratio1 );
//   return ratio2;
// }
...
static inline bool interfacial_s (Point point, scalar c) // identical to interfacial in curvature.h
{
  if (c[] >= 1.) {
    for (int i = -1; i <= 1; i += 2)
      foreach_dimension()
...
double embed_flux (Point point, scalar s, face vector mu, double * val)
{
  ...
  *val = 0.;
  ...
  if (dirichlet) {
    normalize (&n);
    #if dimension == 2 
      grad = (grad == nodata ?  navier_gradient (point, s, cs, n, p, &coef) :
      dirichlet_gradient (point, s, cs, n, p, grad, &coef));
   // grad = (grad == nodata ?  robin_scalar (point, s, &coef) :
    //  dirichlet_gradient (point, s, cs, n, p, grad, &coef));
   // fprintf (stdout, "in grad %g %d %g\n", grad, s.i, coef);
    #else
      grad = dirichlet_gradient (point, s, cs, n, p, grad, &coef);
    #endif
  }
  ...
  //area=1.;
  //fprintf (stdout, "in flux mu=%g f=%g area=%g cs=%g %g\n", mua, fa, area, cs[], alpha);
  //fprintf (stdout, "in flux grad=%g coef=%g\n", - mua/(fa + SEPS)*grad*area/Delta, - mua/(fa + SEPS)*coef*area/Delta);
  *val = - mua/(fa + SEPS)*grad*area/Delta;
  return - mua/(fa + SEPS)*coef*area/Delta;
}
/**
