/**
# Poiseuille flow of a yield-stress fluid using embedded boundary
...
~~~gnuplot Convergence of the error on u.y with EPS = 1e-2
reset
...
f2(x)=a2+b2*x
fit f2(x) 'log' every ::0::4 u (log($1)):(log($5)) via a2,b2
fm(x)=am+bm*x
fit fm(x) 'log' every ::0::4 u (log($1)):(log($6)) via am,bm
plot exp (fm(log(x))) t ftitle(am,bm), exp (f2(log(x))) t ftitle(a2,b2), 'log' every ::0::4 u 1:6 t '|h|_{max}' ps 1.5, 'log' every ::0::4 u 1:5 t '|h|_2' ps 1.5
~~~
...
ftitle(a,b) = sprintf("order %4.2f", -b)
f2(x)=a2+b2*x
fit f2(x) 'log' every ::4::7 u (log($1)):(log($5)) via a2,b2
fm(x)=am+bm*x
fit fm(x) 'log' every ::4::7 u (log($1)):(log($6)) via am,bm
plot exp (fm(log(x))) t ftitle(am,bm), exp (f2(log(x))) t ftitle(a2,b2), 'log' every ::4::7 u 1:6 t '|h|_{max}' ps 1.5, 'log' every ::4::7 u 1:5 t '|h|_2' ps 1.5
~~~
