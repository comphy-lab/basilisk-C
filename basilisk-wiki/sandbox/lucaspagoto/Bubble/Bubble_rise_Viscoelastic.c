...
#define Wi 7.0           // Weissenberg number
...
event movie_vof (t+=0.02)
{
  ...
  foreach()
  {
    tau_p_norm[] = sqrt(0.5)*sqrt( sq(tau_p.x.x[]) + 2*sq(tau_p.x.y[]) + sq(tau_p.y.y[]) + sq(tau_qq[]) );
  }

  draw_vof ("f");
  squares("tau_p_norm", linear = true);
  mirror (n = {0,-1})
        {
          draw_vof ("f");
          squares("tau_p_norm", linear = true);
...
![Volume fraction field field](Bubble_rise_Viscoelastic/movie_vf.mp4)

### Volume fraction field
![Polymeric stress field](Bubble_rise_Viscoelastic/movie_tau_p.mp4)

### Mesh
![Mesh](Bubble_rise_Viscoelastic/movie_mesh.mp4)
*/
