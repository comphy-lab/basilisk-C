...
/**
These functions create a restriction function for alphav based on the harmonic mean.
It should provide a better estimate of alphav on coarse grids*/
/*
static inline void alphav_average (Point point, vector v)
{
  foreach_dimension() {
    #if dimension == 1
      v.x[] = fine(v.x,0);
      v.x[1] = fine(v.x,2);
    #elif dimension == 2
      v.x[] = 2*fm.x[]/(fine(fm.x,0,0)/fine(v.x,0,0) + fine(fm.x,0,1)/fine(v.x,0,1));
      v.x[1] = 2*fm.x[1]/(fine(fm.x,2,0)/fine(v.x,0,0) + fine(fm.x,2,1)/fine(v.x,2,1));
    #else // dimension == 3
      v.x[] = 4*fm.x[]/(fine(fm.x,0,0,0)/fine(v.x,0,0,0) + fine(fm.x,0,1,0)/fine(v.x,0,1,0) +
	       fine(fm.x,0,0,1)/fine(v.x,0,0,1) + fine(fm.x,0,1,1)/fine(v.x,0,1,1));
      v.x[1] = 4*fm.x[1]/(fine(fm.x,2,0,0)/fine(v.x,2,0,0) + fine(fm.x,2,1,0)/fine(v.x,2,1,0) +
		fine(fm.x,2,0,1)/fine(v.x,2,0,1) + fine(fm.x,2,1,1)/fine(v.x,2,1,1));
    #endif
  }
}*/
...
static inline void alphav_average (Point point, vector v)
{
  foreach_dimension() {
      ...
      v.x[1] = fine(v.x,2);
    #elif dimension == 2
      v.x[] = 2/(1/fine(v.x,0,0) + 1/fine(v.x,0,1));
      v.x[1] = 2/(1/fine(v.x,0,0) + 1/fine(v.x,2,1));
    #else // dimension == 3
      v.x[] = 4/(1/fine(v.x,0,0,0) + 1/fine(v.x,0,1,0) + 1/fine(v.x,0,0,1) + 1/fine(v.x,0,1,1));
      v.x[1] = 4/(1/fine(v.x,2,0,0) + 1/fine(v.x,2,1,0) + 1/fine(v.x,2,0,1) + 1/fine(v.x,2,1,1));
    #endif
  }
}

static inline void restriction_alphav (Point point, scalar s){
  alphav_average(point, s.v);
}

/** This function describes a weighted bilinear interpolation scheme which is weighted by alphav.
q  in the function is a cell centered scalar is equivalent to 1/alphav which is also equivalent to rho[]
The weighted bilinear interpolation scheme is taken from Kwak and Lee 2002 and is designed to mantain flux continuity.*/
...
/**
Functions from above where use the weighted bilinear interpolation*/
...
mgstats poisson_weighted (struct Poisson_weighted p)
{
  ...
  scalar lambda = p.lambda;
	...
	/*
	foreach_dimension(){
		alpha.x.restriction = restriction_alphav;
	}*/

	rho.restriction = restriction_volume_average;

	boundary({cm, fm});
	restriction({cm, fm});

	boundary({rho, alpha, lambda});


  restriction ({rho, alpha,lambda});
 ...
 mgstats s = mg_solve_weighted({a}, {b}, residual, relax,
	 &p, p.nrelax, p.res, minlevel = max(1, p.minlevel), q = rho);
...
struct Project_weighted {
  ...
  double dt;         // optional: default one
  int nrelax;        // optional: default four
	scalar rho; 				 //optional default unity
};


trace
mgstats project_weighted (struct Project_weighted q)
{
  face vector uf = q.uf;
  scalar p = q.p;
  ...
  mgstats mgp = poisson_weighted(p, div, alpha,
			 tolerance = TOLERANCE/sq(dt), nrelax = nrelax, rho = rho);
