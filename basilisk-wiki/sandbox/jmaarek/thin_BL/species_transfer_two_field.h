...
#define MAX_IT 1000 //maximum number of iterations of diffusion solver per global timestep
#define SORPTION_SCALING 300.0 //used for limiting timestep restriction represents what is the maximum ratio between the subgrid and grid level gradient at interface a rule of thumb would be 100 times smaller than ratio between expected boundary layer thickness and grid size
...
void species_transfer_two_field_henry_v1(scalar c1, scalar c2, scalar f, double dt_global, double henry_coef, double tolerance_diff){
...
void species_transfer_two_field_henry_v2(scalar c1, scalar c2, scalar f, double dt_global, double henry_coef, double tolerance_diff){
 ...
 for (int ii = 0; ii<(int)num_it; ii++){
   ...
   foreach(){
     ...
     if ((f[] >= VTOL) && (f[] <= 1.0 - VTOL) && (f2[] >= VTOL) && (f2[] <= 1.0 - VTOL)){
        ...
        if (sqrt(c1.phase_diffusivity/c2.phase_diffusivity)*henry_coef > 1){

            coord n_temp, n_temp2, p1_1, p1_2, p2_1, p2_2;
           ...
           double area_1 = plane_area_center(n_temp, alpha1[], &p1_1);

           double area_2 = plane_area_center(n_temp2, alpha2[], &p1_2);


            #if dimension == 2
              line_center(n_temp, alpha1[], f[], &p2_1);
              line_center(n_temp2, alpha2[], f2[], &p2_2);
            #else
              plane_center(n_temp, alpha1[], f[], &p2_1);
              plane_center(n_temp2, alpha2[], f2[], &p2_2);
            #endif

            double concentration_interface = 0; 

           //clamp interpolation such that gradients in both directions must be correct sign (or zero)
           if (tr_c1[] - henry_coef*tr_c2[] > 0.0){
              concentration_interface = clamp(embed_interpolate2(point, tr_c1, &p1_1, f), tr_c2[]*henry_coef, tr_c1[]);
           }
           else{
              concentration_interface = clamp(embed_interpolate2(point, tr_c1, &p1_1, f), tr_c1[], tr_c2[]*henry_coef);
           }




           double concentration_interface_2 = concentration_interface/henry_coef;

           double area = sqrt(area_1*area_2);

           double grad = dirichlet_gradient2(point, tr_c2, f2, fs2, &n_temp2, alpha2[], &p1_2, &p2_2, &concentration_interface_2);



           double flux = -grad*dt_diffusion/Delta*(c2.phase_diffusivity)*area;

           
           //limit flux such that maximum flux across interface would set interfacial cell to local equilibrium
           double limit = -f2[]*f[]*(henry_coef*tr_c2[] - tr_c1[])/(f2[] + henry_coef*f[]);


           //fprintf(ferr, "test %g %g %g %g %g\n", tr_c1[], tr_c2[], tr_c1[] - henry_coef*tr_c2[], flux, limit);

           assert(flux*limit >= 0.0);
           ...
           double area_2 = plane_area_center(n_temp2, alpha2[], &p1_2);
            ...
            double concentration_interface = 0.0;

           //clamp interpolation such that gradients in both directions must be correct sign (including zero)
           if (tr_c1[] - henry_coef*tr_c2[] > 0.0){
              concentration_interface = clamp(embed_interpolate2(point, tr_c2, &p1_2, f2), tr_c1[]/henry_coef, tr_c2[]);
           }
           else{
              concentration_interface = clamp(embed_interpolate2(point, tr_c1, &p1_2, f2), tr_c2[], tr_c1[]/henry_coef);
           }
           ...
           double grad = dirichlet_gradient2(point, tr_c1, f, fs1, &n_temp, alpha1[], &p1_1, &p2_1, &concentration_interface_2);

           double flux = -grad*dt_diffusion/Delta*(c1.phase_diffusivity)*area;
...
/*
We create a new version of the mass transfer algorithm with Henry's Law which uses the subgrid scale model to decribe mass transfer. We assume that the c2.D >> c1.D. 
This is practical for gas air systems and for the water oil experiment. We then interpolate the concentration at the interface from c2 and the interface gradient from the subgrid model from the c1 side.
This is different from what is done above where we interpolate the concentration at the interface from the donator and the interface gradient from the acceptor. We modify the effectve diffusivity for c2
...
//We use this assertion as this model is not accurate if c1.D > c2.D
assert(c2.phase_diffusivity > c1.phase_diffusivity);

//timestep is limited by minimum timestep of both phases
double dt_diffusion = FTCS_diffusion_stability(c2);
double num_it = clamp(ceil(dt_global/dt_diffusion), MIN_IT, MAX_IT);
dt_diffusion = dt_global/num_it;
//fprintf(ferr, "num it %g %g %g %g\n", dt_global, FTCS_diffusion_stability(c, f, fs2, transfer_beta), dt_diffusion, num_it);
