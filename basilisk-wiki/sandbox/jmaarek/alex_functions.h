...
*/
  ...
  for (int kk = 0;kk<=3;kk++){
    X[kk] = s[Arr[0]-1,Arr[1]-1+kk, offset]*XX[0] +
            s[Arr[0]  ,Arr[1]-1+kk, offset]*XX[1] +
            s[Arr[0]+1,Arr[1]-1+kk, offset]*XX[2] +
            s[Arr[0]+2,Arr[1]-1+kk, offset]*XX[3];
  }
...
double mytricubic(Point point, scalar s, int Arr[], coord p){
/**
We do a quadruple bicubic interpolation and then cubically interpolate the
result where the cubic interpolation is the value f(0) in f(x) = ax^3 + bx^2 + cx + d.
*/


  double dz = p.z;
  double arr[4];
  arr[0] = bicubic(point, s, Arr, p, (int)(-1.5+dz));
  arr[1] = bicubic(point, s, Arr, p, (int)(-0.5+dz));
  arr[2] = bicubic(point, s, Arr, p, (int)(0.5+dz));
  arr[3] = bicubic(point, s, Arr, p, (int)(1.5+dz));

  return (arr[1]+arr[2]-0.5*((arr[0]+arr[3]-arr[1]-arr[2])/4))/2;
...
void cell2node(scalar cell_scalar, vertex scalar node_scalar){
  foreach_vertex(){
    ...
    int Stencil[2] = {-1,-1};
#if dimension == 2
    node_scalar[] = bicubic(point , cell_scalar, Stencil, p_interp,0);
#else
    node_scalar[] = mytricubic(point , cell_scalar, Stencil, p_interp);
#endif
#elif QUADRATIC
#if dimension == 2
