...
double c_injection = 0.0;

n_front.n[bottom] = -1;
n_front.t[bottom] = 0;
n_front.r[bottom] = 0;

alpha_front[bottom] = -0.5 + c_injection;

foreach_dimension()
static void sweep_x (scalar c, scalar cc, scalar * tcl,  scalar * bl_tcl){
  vector n = n_front;
  scalar alpha = alpha_front;
  scalar flux[];
  double cfl = 0.;
     ...
     double cf1 = cf, ci = c[i], cd = c[d], cq = c[q];
      ...
      if (ci > 0.0) {
        ...
        if (ci < 1.0){
          ...
          if ((ci - cf1*s*un)/ci < 1e-5 || cf1*s*un == 0.0 || fabs(n.x[i]) < 1e-5 || delta_b_tracer[i] == -1 || t[i]/ci <= min(t.c_inf, c_boundary_tracer[i]) || t[i]/ci >= max(t.c_inf, c_boundary_tracer[i])){
            double ff = t[i]/ci;
            tflux[] = ff*cf1*uf.x[];}
          else{
        ...
        if (ci >= 1.0 && cd < 1.0){
          ...
          if ((ci - cf1*s*un)/ci < 1e-5 || cf1*s*un == 0.0 || signn*uf.x[] <= 0.0 || t[i]/ci <= min(t.c_inf, c_boundary_tracer[d]) || t[i]/ci >= max(t.c_inf, c_boundary_tracer[d])){
            double ff = t[i]/ci + s*min(1., 1. - s*un)*gf[i]*Delta/2.;
            tflux[] = ff*cf1*uf.x[];}
          else{
        ...
        if (ci >= 1.0 && cd >= 1.0 && cq < 1.0){
          ...
          if ((ci - cf1*s*un)/ci < 1e-5 || cf1*s*un == 0.0 || signn*uf.x[] >= 0.0 || t[i]/ci <= min(t.c_inf, c_boundary_tracer[q]) || t[i]/ci >= max(t.c_inf, c_boundary_tracer[q])){
            double ff = t[i]/ci + s*min(1., 1. - s*un)*gf[i]*Delta/2.;
            tflux[] = ff*cf1*uf.x[];}
          else{
	     ...
	     coord g = {g_temp[idx[0]], g_temp[idx[1]], g_temp[idx[2]]};
 	      ...
 	      double temp1 = clamp((un ? compute_eta_sgs(n_out, alpha_out, delta_b_link/Delta, s*un*cf1, a,b) : 0),0.0,1.0);
              ...
              double advect_conc = 0.0;
              if (un){
		            advect_conc = ((temp2*(t.c_inf-c_boundary_tracer[q])+c_boundary_tracer[q])*ci != 0.0) ? ((temp1*(t.c_inf-c_boundary_tracer[q])+c_boundary_tracer[q])*s*un*cf1)/((temp2*(t.c_inf-c_boundary_tracer[q])+c_boundary_tracer[q])*ci)*ci/(s*un*cf1)*t[i]/ci : t[i]/ci + s*min(1., 1. - s*un)*gf[i]*Delta/2;
             ...
             double ff = 0.0;
             if(t.c_inf < c_boundary_tracer[q]){
