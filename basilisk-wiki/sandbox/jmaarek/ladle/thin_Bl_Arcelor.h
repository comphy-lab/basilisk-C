...
coord interface_normal(Point point, scalar c){
  ...
  tag_temp[1] = 0.0;
  tag_temp[2] = 0.0;
  double triple_point = 0.0;

  foreach_neighbor(2){
      if ((f1[] > 0.0) && (f1[] < 1.0) && (f2[] > 0.0) && (f2[] < 1.0))
        tag_temp[0] = 1.0;
      if ((f1[] > 0.0) && (f1[] < 1.0) && (f3[] > 0.0) && (f3[] < 1.0))
        tag_temp[1] = 1.0;
      if ((f2[] > 0.0) && (f2[] < 1.0) && (f3[] > 0.0) && (f3[] < 1.0))
        tag_temp[2] = 1.0;
    }
    //need to force maximum refinement near the triple point
    if((tag_temp[0]+tag_temp[1]+tag_temp[2]) > 2 && Delta == (L0/(1 << grid->maxdepth)))
      triple_point = 1.0;


  coord n;
  if (!c.height.x.i || (n = height_normal (point, c, c.height)).x == nodata || triple_point == 1.0)
    n = mycs (point, c);
  else {
    double nn = 0.;
    foreach_dimension()
...
void boundary_layer(scalar bl_tracer){
  ...
  scalar f2 = bl_tracer.interface1;
  ...
  foreach(){
    ...
    if ((f[] > 0.0) && (f[] < 1) && (f2[] > 0.0) && (f2[] < 1)){ //decides if a cell is interfacial
        delta_b[] = -1;
 ...
 foreach(){
   if ((f[] > 0) && (f[] < 1) && (f2[] > 0.0) && (f2[] < 1) && (delta_b[] != -1.0)){
    tagged_cells[] = 1.0;}
   else{
    tagged_cells[] = 0.0;}
  }
  ...
  foreach(){
    if ((f[] > 0) && (f[] < 1) && (f2[] > 0.0) && (f2[] < 1) && (tagged_cells[] == 0.0) && ((bl_tracer[]/f[]-c_boundary[])/(c_inf-c_boundary[]) > 0.0) && ((bl_tracer[]/f[]-c_boundary[])/(c_inf-c_boundary[]) < 1.0)){
...
double lamda_rect(coord a, coord b, coord n, double alpha, double delta_b){
    return sq(delta_b)/(2*n.y*n.z)*(
      f_term3((coord){0.0,b.y,b.z}) - f_term3((coord){0.0,b.y,a.z})
    - f_term3((coord){0.0,a.y,b.z}) + f_term3((coord){0.0,a.y,a.z})
    + 1/sqrt(M_PI)*(f_xyz((coord){0.0,b.y,b.z},n,alpha,delta_b)*exp(-sq(f_xyz((coord){0.0,b.y,b.z},n,alpha,delta_b)))
      -f_xyz((coord){0.0,b.y,a.z},n,alpha,delta_b)*exp(-sq(f_xyz((coord){0.0,b.y,a.z},n,alpha,delta_b)))
      -f_xyz((coord){0.0,a.y,b.z},n,alpha,delta_b)*exp(-sq(f_xyz((coord){0.0,a.y,b.z},n,alpha,delta_b)))
      +f_xyz((coord){0.0,a.y,a.z},n,alpha,delta_b)*exp(-sq(f_xyz((coord){0.0,a.y,a.z},n,alpha,delta_b)))));}

double lamda_tri(coord a, coord b, coord n, double alpha, double delta_b){
    return sq(delta_b)/(2*n.y*n.z)*(
      f_term3((coord){0.0,b.y,b.z}) - f_term3((coord){0.0,b.y,a.z})
    + 1/sqrt(M_PI)*(f_xyz((coord){0.0,b.y,b.z},n,alpha,delta_b)*exp(-sq(f_xyz((coord){0.0,b.y,b.z},n,alpha,delta_b)))
      -f_xyz((coord){0.0,b.y,a.z},n,alpha,delta_b)*exp(-sq(f_xyz((coord){0.0,b.y,a.z},n,alpha,delta_b))))
    - 2*n.z*(b.z-a.z)/(sqrt(M_PI)*delta_b));}
...
double zeta_tri(coord a, coord b, coord n, double alpha, double delta_b){
    return delta_b/(n.y*n.z)*(
      f_xyz((coord){0.0,b.y,b.z},n,alpha,delta_b)*erf(f_xyz((coord){0.0,b.y,b.z},n,alpha,delta_b))
    - f_xyz((coord){0.0,b.y,a.z},n,alpha,delta_b)*erf(f_xyz((coord){0.0,b.y,a.z},n,alpha,delta_b))
    + 1/sqrt(M_PI)*(exp(-sq(f_xyz((coord){0.0,b.y,b.z},n,alpha,delta_b)))
