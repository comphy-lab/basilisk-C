#define BGHOSTS 2
#define VTOL 1e-6 //threshold for determining if cell is interfacial in flux calculation
#define THETA_LIMIT 0.1 //used in computing timestep restriction in diffusion solver
#define MIN_IT 1 //minimum number of iterations of diffusion solver per global timestep
#define MAX_IT 1 //maximum number of iterations of diffusion solver per global timestep
#define SORPTION_SCALING 1500.0 //used for limiting timestep restriction represents what is the maximum ratio between the subgrid and grid level gradient at interface
...
double FTCS_diffusion_stability(scalar c){
  double tmin = 0.5*THETA_LIMIT/dimension*sq(L0/(1 << grid->maxdepth))/(c.phase_diffusivity + SEPS);
...
static inline double dirichlet_gradient2_x(Point point, scalar s, scalar cs, face vector fs, coord * n_in, double alpha, coord * p_in, coord * p2_in, double * bc_in)
{
  ...
  if (v[1] != nodata) // second order VOF weighted gradient
      //if third order gradient has same sign as 1st order gradient use it
      if (((cs[]*(bc - v[0])/d[0] + (1. - cs[])*(bc - v[1])/d[1])/Delta)*((bc - s[])/(d[0]*Delta)) > 0.0){
        //fprintf(fout, "%g %g\n", Delta, (d[1]*(bc - v[0])/d[0] - d[0]*(bc - v[1])/d[1])/((d[1] - d[0])*Delta));
        return (cs[]*(bc - v[0])/d[0] + (1. - cs[])*(bc - v[1])/d[1])/Delta; //(d[1]*(bc - v[0])/d[0] - d[0]*(bc - v[1])/d[1])/((d[1] - d[0])*Delta);
      }
...
void diffusion_implicit(scalar * clist, scalar f, scalar f2, double dt_global, double bc, int i){
 ...
 // MPI_Barrier(MPI_COMM_WORLD);
  ...
  foreach_face(){
    ...
    Diff.x[] = gamma_centroid*fm.x[]*fs2.x[];
  ...
  foreach(){
       if ((f[] >= VTOL) && (f[] <= 1.0 - VTOL) && (f2[] >= VTOL) && (f2[] <= 1.0 - VTOL)){
         coord n_temp, p, p2;
         foreach_dimension()
           n_temp.x = n.x[];
         ...
         double area = sqrt(plane_area_center(n_temp, alpha[], &p)*plane_area_center(n_temp2, plane_alpha(f2[], n_temp2), &p3));
         ...
         transfer_beta[] = -area/dist/pow(Delta,2);
         transfer_r[] = bc*area/dist/pow(Delta,2);
       }
       else{
         transfer_beta[] = 0.0;
         transfer_r[] = 0.0;
  ...
  for (scalar c in clist) {


    foreach_face()
      Diff2.x[] = Diff.x[]*c.phase_diffusivity;
    ...
    if ((int) num_it == MAX_IT)
      fprintf(ferr, "diffusive timescale cut standard %g %d\n", ceil(dt_global/dt_diffusion), MAX_IT);
    dt_diffusion = dt_global/num_it;
    fprintf(ferr, "num it normal %g %g %g %g\n", dt_global, c.phase_diffusivity, dt_diffusion, num_it);
    ...
    //  MPI_Barrier(MPI_COMM_WORLD);
     ...
     double tolerance_diff = 1e-3*sqrt(c.phase_diffusivity);
     ...
     for (int ii = 1; ii<=(int)num_it; ii++){
       ...
       foreach(){
        ...
        theta_idt[] = - 1./dt_diffusion*cm[]*max(f[], VTOL);
         ...
         r[] = theta_idt[]*tr_c[] - scaling*transfer_r[]*c.phase_diffusivity;
         lambda[] = scaling*transfer_beta[]*c.phase_diffusivity + theta_idt[];

      }

      restriction ((scalar *){Diff2, r, lambda, theta_idt});
      ...
      if (mg_diffusion.i == NITERMAX_DIFFUSION){
        fprintf(ferr, "save time by increasing tolerance %d\n", i);
...
void compute_sgs_flux_scaling(face vector sgs_scaling, scalar sgs_sorption_scaling, scalar c, scalar f, vector n, scalar alpha, face vector fs2){
  ...
  foreach(){
    ...
    if ((f[] >= VTOL) && (f[] <= 1.0 - VTOL) && (f2[] >= VTOL) && (f2[] <= 1.0 - VTOL) && delta_b_tracer[] > 0.0){
    ...
    sgs_sorption_scaling[] = ((c_boundary_tracer[] - c[]/f[])*-(c.c_inf - c_boundary_tracer[])) > 0.0 ? sgs_sorption_scaling[] : 1.0 ; 
    assert(sgs_sorption_scaling[] >= 0.0);
    }
  }
}

void diffusion_implicit_sgs(scalar * clist, scalar f, scalar f2, double dt_global, int i){

//  MPI_Barrier(MPI_COMM_WORLD);
//  fprintf(ferr, "start diffusion\n");
//  MPI_Barrier(MPI_COMM_WORLD);
...
//  MPI_Barrier(MPI_COMM_WORLD);
 ...
 foreach(){
   if ((f[] >= VTOL) && (f[] <= 1.0 - VTOL)){
     ...
     transfer_beta[] = -area/dist/pow(Delta,2);
     transfer_r[] = area/dist/pow(Delta,2);
   }
   else{
     transfer_beta[] = 0.0;
     transfer_r[] = 0.0;
   ...
   for (scalar c in clist) {
    ...
    double num_it = clamp(ceil(dt_global/dt_diffusion), MIN_IT, MAX_IT);
    if ((int) num_it == MAX_IT)
      fprintf(ferr, "diffusive timescale cut subgrid %g %d\n", ceil(dt_global/dt_diffusion), MAX_IT);
    dt_diffusion = dt_global/num_it;
    fprintf(ferr, "num it sgs %g %g %g %g\n", dt_global, c.phase_diffusivity, dt_diffusion, num_it);
     ...
     for (int ii = 1; ii<=(int)num_it; ii++){
      ...
      foreach(){
         theta_idt[] = - 1./dt_diffusion*cm[]*max(f[], VTOL);
         r[] = theta_idt[]*tr_c[] - transfer_r[]*c_boundary_tracer[]*c.phase_diffusivity*sgs_sorption_scaling[];
         lambda[] = transfer_beta[]*sgs_sorption_scaling[]*c.phase_diffusivity + theta_idt[];}

      foreach_face()
        Diff2.x[] = Diff.x[]*sgs_scaling.x[]*c.phase_diffusivity;
     ...
     /*
     At the end of each diffusion step we compute what is the max scaling factor for the grdient at the interface. This is used to decide max diffusive timestep for the next iteration
     */
     double sorption_coef_max = -HUGE;
     double count_interfacial_cells = 0.0;
     ...
     foreach(reduction(max:sorption_coef_max) reduction(+:count_interfacial_cells) reduction(+:count_clipped)){
        if (f[] >= 0.1 && f[] <= 1 - 0.1){
          ...
          count_interfacial_cells++;
          count_clipped += sgs_sorption_scaling[] > SORPTION_SCALING ? 1 : 0;
        }
      }
      ...
      if (count_clipped/count_interfacial_cells > 0.01) //if over 1 percent of interfacial cells are clipped send a warning
        fprintf(ferr, "Warning subgrid scale transfer clipped %d %d %d %g\n", i, (int)count_clipped, (int)count_interfacial_cells, sorption_coef_max);
  }

}


void compute_sgs_flux_scaling_NN(face vector sgs_scaling, scalar sgs_sorption_scaling, scalar c, scalar f, vector n, scalar alpha, face vector fs2){
      ...
      for (int i = -1; i <= 0; i++){
          ...
          if (f[i] < 1-VTOL){
            ...
            if (delta_b_tracer[i] > 0.0){
              ...
              double sgs1 = clamp(cvof1 > 0.0 ? compute_eta_sgs_NN(n_out, alpha_out, delta_b_tracer[i]/Delta, cvof1, g,h) : 0.0, 0.0,1);
              ...
              double sgs2 = clamp(cvof2 > 0.0 ? compute_eta_sgs_NN(n_out, alpha_out, delta_b_tracer[i]/Delta, cvof2, g,h) : 0.0,0.0,1);
              ...
              double sgs_flux = (c.c_inf - c_boundary_tracer[i])*clamp(compute_zeta_sgs_NN(n_out, alpha_out, delta_b_tracer[i]/Delta, a), 0, face_area3*interface_gradient(delta_b_tracer[i]/Delta))/Delta*-n.x[i,0,0]/sqrt(sq(n.x[i,0,0])+sq(n.y[i,0,0])+sq(n.z[i,0,0]));
              double pseudo_numerical_flux = output_gradient*face_area3*gamma_centroid3;
              flux_sgs[i+1] = fabs(pseudo_numerical_flux) > 0.0 ? max(sgs_flux/pseudo_numerical_flux, 0.0) : 0.0;
              //confirm that subgrid flux will have same sign as numerical flux if not do not use subgrid correction
              tag_sgs[i+1] = pseudo_numerical_flux*(c[]/f[]-c[-1]/f[-1]) >= 0.0 ? 1.0 : 0.0;
  ...
  foreach(){
    ...
    if ((f[] >= VTOL) && (f[] <= 1.0 - VTOL) && (f2[] >= VTOL) && (f2[] <= 1.0 - VTOL) && delta_b_tracer[] > 0.0){
      ...
      double sgs_conc = clamp(compute_eta_sgs_NN(n_out, alpha_out, delta_b_tracer[]/Delta, f[], (coord){0.,0.,0.},(coord){1.,1.,1.}),0.0,1)*(c.c_inf-c_boundary_tracer[])+c_boundary_tracer[];
    //fprintf(ferr, "%12e\n", ((1 - sgs_conc)/dist)/(2/sqrt(M_PI)/80));
    sgs_sorption_scaling[] = ((c_boundary_tracer[] - sgs_conc)/dist) != 0.0 ? max((-(c.c_inf - c_boundary_tracer[])*interface_gradient(delta_b_tracer[])*Delta)/((c_boundary_tracer[] - sgs_conc)/dist), 0.0) : 1.0;
    sgs_sorption_scaling[] = ((c_boundary_tracer[] - c[]/f[])*-(c.c_inf - c_boundary_tracer[])) > 0.0 ? sgs_sorption_scaling[] : 1.0 ;
    assert(sgs_sorption_scaling[] >= 0.0);
    }
  }
...
void diffusion_implicit_sgs_NN(scalar * clist, scalar f, scalar f2, double dt_global, int i){
...
//  MPI_Barrier(MPI_COMM_WORLD);
 ...
 foreach(){
   ...
   for (scalar c in clist) {
    ...
    compute_sgs_flux_scaling_NN(sgs_scaling, sgs_sorption_scaling, c, f, n, alpha, fs2); 
    ...
    if ((int) num_it == MAX_IT)
      fprintf(ferr, "diffusive timescale cut subgrid %g %d\n", ceil(dt_global/dt_diffusion), MAX_IT);
    dt_diffusion = dt_global/num_it;
    fprintf(ferr, "num it sgs NN %g %g %g %g\n", dt_global, c.phase_diffusivity, dt_diffusion, num_it);
...
void species_transfer_two_field_henry(scalar c1, scalar c2, scalar f, scalar f2, double dt_global, double henry_coef, double tolerance_diff){
  ...
  face vector Diff1[], Diff2[];
  face vector Diff1_sgs[], Diff2_sgs[];
  ...
  foreach_face(){
    ...
    Diff1.x[] = c1.phase_diffusivity*gamma_centroid*fm.x[]*fs1.x[];


    gamma_1 = 0.0, gamma_2 = 0.0;

    if ((f2[] < (1.-VTOL)) && (f2[] > VTOL)){
      n_temp.x = n2.x[0,0,0];
      n_temp.y = n2.y[0,0,0];
      n_temp.z = n2.z[0,0,0];
      ...
      plane_center (n_temp, plane_alpha (f2[], n_temp), f2[], &p);
      gamma_1 =  p.x;}

    if ((f2[-1] < (1.-VTOL)) && (f2[-1] > VTOL)){
      n_temp.x = n2.x[-1,0,0];
      n_temp.y = n2.y[-1,0,0];
      n_temp.z = n2.z[-1,0,0];
      ...
      plane_center (n_temp, plane_alpha (f2[-1], n_temp), f2[-1], &p);
      gamma_2 = p.x;}

    //double gamma_centroid = 1.0;

    gamma_centroid = 1./clamp((1. + gamma_1 - gamma_2), 0.1, 2.0); //minimum = 0.5, maximum = 10

    Diff2.x[] = c2.phase_diffusivity*gamma_centroid*fm.x[]*fs2.x[];
...
//timestep is limited by minimum timestep of both phases
double dt_diffusion = min(FTCS_diffusion_stability(c2), FTCS_diffusion_stability(c1));
double num_it = clamp(ceil(dt_global/dt_diffusion), MIN_IT, MAX_IT);
if ((int) num_it == MAX_IT)
  fprintf(ferr, "diffusive timescale cut standard %g %d\n", ceil(dt_global/dt_diffusion), MAX_IT);
dt_diffusion = dt_global/num_it;
fprintf(ferr, "num it standard henry %g %g %g %g\n", dt_global, min(c1.phase_diffusivity, c2.phase_diffusivity), dt_diffusion, num_it);
 ...
 for (int ii = 0; ii<(int)num_it; ii++){
   ...
   foreach( reduction(+:limit_check) reduction(+:limit_max) reduction(+:interfacial_cells) reduction(+:r2_sum)){
     ...
     if ((f[] >= VTOL) && (f[] <= 1.0 - VTOL) && (f2[] >= VTOL) && (f2[] <= 1.0 - VTOL)){
       ...
       plane_area_center(n_temp2, alpha1[], &p);

       double donator_concentration_interface = clamp(embed_interpolate2(point, tr_c1, &p, f), tr_c2[]*henry_coef, tr_c1[]);

       double acceptor_concentration_interface = donator_concentration_interface/henry_coef;

      double area = sqrt(plane_area_center(n_temp, alpha2[], &p)*plane_area_center(n_temp2, alpha1[], &p3));
      ...
      limit_check += ((flux >= 0.0 && limit >= 0.0) ? 1.0 : 0.0);

      //enforce flux direction and use flux limiter with maximum flux equal to equilibriation in interfacial cell
      flux = clamp(flux, 0.0, limit);

      limit_max += ((flux == limit) ? 0.0 : 1.0);
      ...
      //compute value at interface on donator side
       ...
       plane_area_center(n_temp2, alpha2[], &p);
      ...
      double area = sqrt(plane_area_center(n_temp, alpha1[], &p)*plane_area_center(n_temp2, plane_alpha(f2[], n_temp2), &p3));
 ...
 fprintf(ferr, "test transfer standard %g %g %g\n", limit_check/interfacial_cells, limit_max/interfacial_cells, r2_sum/interfacial_cells);
 ...
 fprintf(ferr, "diffusion poisson out %g %g %d %d %d %d %d\n", dt_global, num_it, ii, mg_diffusion.i, mg_diffusion.nrelax, mg_diffusion_2.i, mg_diffusion_2.nrelax);
...
double species_transfer_two_field_henry_sgs(scalar c1, scalar c2, scalar f, scalar f2, double dt_global, double henry_coef, double tolerance_diff){
...
//compute_c_boundary(scalar c2, scalar c3, scalar f2, scalar f3)



face vector sgs_scaling1[];
scalar sgs_sorption_scaling1[]; 

face vector sgs_scaling2[];
scalar sgs_sorption_scaling2[]; 

compute_c_boundary(c2, c1, f2, f);
boundary_layer(c1);
boundary_layer_NN(c2);
compute_sgs_flux_scaling(sgs_scaling1, sgs_sorption_scaling1, c1, f, n1, alpha1, fs1); 
compute_sgs_flux_scaling_NN(sgs_scaling2, sgs_sorption_scaling2, c2, f2, n2, alpha2, fs2);
...
double num_it = clamp(ceil(dt_global/dt_diffusion), MIN_IT, MAX_IT);
//if ((int) num_it == MAX_IT)
//  fprintf(ferr, "diffusive timescale cut sgs %g %d\n", ceil(dt_global/dt_diffusion), MAX_IT);
dt_diffusion = dt_global/num_it;
//fprintf(ferr, "num it sgs %g %g %g %g\n", dt_global, max(c1.phase_diffusivity, c2.phase_diffusivity), dt_diffusion, num_it);
 ...
 for (int ii = 0; ii<(int)num_it; ii++){
  ...
  /* double limit_check = 0.0;
   ...
   double r2_sum = 0.0;
   /*double flux_grt_0 = 0.0;
   double limit_grt_0 = 0.0;
   double water_grad_mean = 0.0;
   double water_grad_min = HUGE;
   double water_grad_max= -HUGE;
   double water_boundary_min = HUGE;
   double water_boundary_max= -HUGE;
   double water_boundary_avg = 0.0;

   double water_scaling_min = HUGE;
   double water_scaling_max= -HUGE;
   double water_scaling_avg = 0.0;
   ...
   scalar water_bc = c2.c_boundary;

   foreach(reduction(+:r2_sum)){


   /*foreach(reduction(+:limit_check) reduction(+:limit_max) reduction(+:interfacial_cells) 
 	reduction(+:r2_sum) reduction(+:flux_grt_0) reduction(+:limit_grt_0) reduction(+:water_grad_mean) reduction(min:water_grad_min) reduction(max:water_grad_max)
	reduction(min:water_boundary_min) reduction(max:water_boundary_max) reduction(+:water_boundary_avg)
        reduction(min:water_scaling_min) reduction(max:water_scaling_max) reduction(+:water_scaling_avg)
	reduction(min:eq_min) reduction(max:eq_max) reduction(+:eq_avg)){*/
     ...
     if ((f[] > VTOL) && (f[] < (1.0 - VTOL)) && (f2[] > VTOL) && (f2[] < (1.0 - VTOL))){
          ...
          //compute value at interface on oil side
           coord n_temp3, n_temp2, p, p2, p3, p4;
            foreach_dimension()
              n_temp3.x = n1.x[];
        ...
        // water side geometry
        plane_area_center(n_temp2, alpha2[], &p);
        plane_center(n_temp2, alpha2[], f2[], &p2);

	/*water_boundary_min = min(water_boundary_min, water_bc[]);
        water_boundary_max = max(water_boundary_max, water_bc[]);
        water_boundary_avg += water_bc[];*/

        //water side gradient
        double water_side_gradient = -dirichlet_gradient_first_order(point, tr_c2, &n_temp2, alpha2[], &p2, water_bc[])*sgs_sorption_scaling2[];
        /*water_grad_min = min(water_grad_min, sgs_sorption_scaling1[]);
        water_grad_max = max(water_grad_max, sgs_sorption_scaling1[]);
	water_grad_mean += sgs_sorption_scaling1[];

   	water_scaling_min = min(water_scaling_min, sgs_sorption_scaling2[]);
        water_scaling_max = max(water_scaling_max, sgs_sorption_scaling2[]);
        water_scaling_avg += sgs_sorption_scaling2[];*/

	

        double area = sqrt(plane_area_center(n_temp3, alpha1[], &p3)*plane_area_center(n_temp2, alpha2[], &p4));
        //double flux = c2.phase_diffusivity*area*water_side_gradient*dt_diffusion/Delta;
	double flux = -water_side_gradient*dt_diffusion/Delta*(c2.phase_diffusivity)*area;
        double limit = -f2[]*f[]*(henry_coef*tr_c2[] - tr_c1[])/(f2[] + henry_coef*f[]);

        /*flux_grt_0 += ((flux < 0.0) ? 1.0 : 0.0); 
        limit_grt_0 += ((limit < 0.0) ? 1.0 : 0.0);

        interfacial_cells += 1.0;
        limit_check += ((flux*limit > 0.0) ? 1.0 : 0.0);

      	double eq_water = (tr_c2[]*f2[] + limit)/f2[];
        double eq_oil = (tr_c1[]*f[] - limit)/f[];


       	eq_min = min(eq_min, eq_oil/eq_water);
        eq_max = max(eq_max, eq_oil/eq_water);
        eq_avg += flux/limit;*/



        flux = flux*limit > 0.0 ? sign(flux)*min(fabs(flux),fabs(limit)) : 0.0;
    ...
    foreach_face(){
      Diff1_sgs.x[] = Diff1.x[]*sgs_scaling1.x[];
      Diff2_sgs.x[] = Diff2.x[]*sgs_scaling2.x[];
    }
  ...
  //fprintf(ferr, "test transfer %g %g %g %g %g\n", limit_check/interfacial_cells, limit_max/interfacial_cells, r2_sum/interfacial_cells, flux_grt_0/interfacial_cells, limit_grt_0/interfacial_cells);
  //fprintf(ferr, "oil scaling %g %g %g\n", water_grad_min, water_grad_max, water_grad_mean/interfacial_cells);
  //fprintf(ferr, "water bc %g %g %g %g\n", water_boundary_min, water_boundary_max, water_boundary_avg/interfacial_cells, c2.c_inf);
  //fprintf(ferr, "water scaling %g %g %g\n", water_scaling_min, water_scaling_max, water_scaling_avg/interfacial_cells); 
  //fprintf(ferr, "eq %g %g %g\n", eq_min, eq_max, eq_avg/interfacial_cells);


 restriction ((scalar *){Diff1_sgs, Diff2_sgs, r1, r2, lambda1, lambda2});

  struct Poisson pp, pp2;
  pp.a = tr_c1, pp.b = r1; //variables in poisson equation
  pp.alpha = Diff1_sgs, pp.lambda = lambda1; //coefficients in poisson equation

  mgstats mg_diffusion = mg_solve_weighted2({tr_c1}, {r1}, residual, relax, &pp, tolerance = tolerance_diff, cs2 = f);


  pp2.a = tr_c2, pp2.b = r2; //variables in poisson equation
  pp2.alpha = Diff2_sgs, pp2.lambda = lambda2; //coefficients in poisson equation
 ...
 foreach(){
  ...
  boundary_layer_NN(c2);
  boundary_layer(c1);
  compute_sgs_flux_scaling_NN(sgs_scaling1, sgs_sorption_scaling1, c2, f2, n2, alpha2, fs2);
  compute_sgs_flux_scaling(sgs_scaling2, sgs_sorption_scaling2, c1, f, n1, alpha1, fs1); 
  ...
  foreach(reduction(max:sorption_coef_max) reduction(+:count_interfacial_cells) reduction(+:count_clipped)){
    if (f[] >= 0.1 && f[] <= 1 - 0.1){
      sorption_coef_max = max(sorption_coef_max,sgs_sorption_scaling2[]);
      count_interfacial_cells++;
      count_clipped += sgs_sorption_scaling2[] > SORPTION_SCALING ? 1 : 0;
    }
  }
  c1.sorption_scaling = clamp(sorption_coef_max, 1.0, SORPTION_SCALING);
  if (count_clipped/count_interfacial_cells > 0.01) //if over 1 percent of interfacial cells are clipped send a warning
    fprintf(ferr, "Warning subgrid scale transfer clipped c1 %d %d %g\n", (int)count_clipped, (int)count_interfacial_cells, sorption_coef_max);


  sorption_coef_max = -HUGE;
  count_interfacial_cells = 0.0;
  count_clipped = 0.0;
  foreach(reduction(max:sorption_coef_max) reduction(+:count_interfacial_cells) reduction(+:count_clipped)){
    if (f2[] >= 0.1 && f2[] <= 1 - 0.1){
      sorption_coef_max = max(sorption_coef_max,sgs_sorption_scaling1[]);
      count_interfacial_cells++;
      count_clipped += sgs_sorption_scaling1[] > SORPTION_SCALING ? 1 : 0;
    }
  }
  c2.sorption_scaling = clamp(sorption_coef_max, 1.0, SORPTION_SCALING);
  if (count_clipped/count_interfacial_cells > 0.01) //if over 1 percent of interfacial cells are clipped send a warning
    fprintf(ferr, "Warning subgrid scale transfer clipped c2 %d %d %g\n", (int)count_clipped, (int)count_interfacial_cells, sorption_coef_max);
