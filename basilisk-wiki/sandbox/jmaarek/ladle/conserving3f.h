...
static void momentum_refine (Point point, scalar u) {
  ...
  double du = u[] - rhou/((1 << dimension)*(cm[] + SEPS)*rhom(f1[], f2[], f3[]));
  foreach_child()
    u[] += du;
...
static void momentum_restriction (Point point, scalar u)
{
  ...
  foreach_child()
    rhou += cm[]*rhom(f1[], f2[], f3[])*u[];
  u[] = rhou/((1 << dimension)*(cm[] + SEPS)*rhom(f1[], f2[], f3[]));

  }
  else{

   restriction_embed_linear (point, u);
...
#if ARCELOR
q1.y[bottom] = rho1*VELOCITY*f10[]*cs[];
q1.x[bottom] = 0.0;
q1.z[bottom] = 0.0;
...
q3.z[bottom] = 0.0;
  ...
  foreach()
    foreach_dimension() {
      ...
      q2.x[] = cs[]*fc2*rho2*u.x[];
      q3.x[] = cs[]*fc3*rho3*u.x[];
      q1.x[] = cs[]*fc1*rho1*u.x[];
  ...
  scalar * tracers1 = f1.tracers;
  scalar * tracers2 = f2.tracers;
  scalar * tracers3 = f3.tracers;

  f1.tracers = list_concat (tracers1, (scalar *){q1});
  f2.tracers = list_concat (tracers2, (scalar *){q2});
  f3.tracers = list_concat (tracers3, (scalar *){q3});
  ...
  foreach()
	...
	if (cs[] >= 1){
		u.x[] = (q1.x[] + q2.x[]+ q3.x[])/(rhom(f1[],f2[],f3[])*max(1e-16, f1[]+f2[]+f3[]));
	}
	else{
		 u.x[] = (q1.x[] + q2.x[]+ q3.x[])/(rhom(f1[],f2[],f3[])*max(1.0, f1[]+f2[]+f3[]));
	}
    }
 ...
 foreach(reduction(min:uxmin) reduction(min:uymin) reduction(min:uzmin) reduction(max:uxmax) reduction(max:uymax) reduction(max:uzmax)){
        ...
        fprintf(ferr, "after momentum %12e %12e %12e %12e %12e %12e\n", uxmin, uymin, uzmin, uxmax, uymax, uzmax);
        MPI_Barrier(MPI_COMM_WORLD);


      MPI_Barrier(MPI_COMM_WORLD);
    fprintf(ferr, "finish momentum\n"); 
      MPI_Barrier(MPI_COMM_WORLD);
