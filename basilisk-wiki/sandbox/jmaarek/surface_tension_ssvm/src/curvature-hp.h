/**
# Filtered Curvature of an interface

Here we modify the standard height function based curvature method to 
filter grid level oscillations. We do this by modifying the computation 
of the diagonal second order spatial derivates. We apply a binomial filter
to the 1-D height functions. After differentiation this is equivalent
to evaluating the diagonal second-order spatial derivatives with a 5 point 
stencil. This explicitly filters the Nyquist mode of the curvature computation,
necessary for attenuation of grid level spurious currents when combined with 
surface tension and breaching the capillary time step constraint. This is
necessary as the discretization of the viscous term does not resolve the 
currents generated by the Nyquist mode. A drawback is that the method now needs
height functions defined for the second neighbors, causing the fallback to
less accurate curvature methods at relatively finer grids.
...
static double kappa_hp_z (Point point, vector h)
{
  ...
  // First derivatives: Delta spacing is sufficient
  ...
  // Cross derivative: Delta spacing is sufficient 
