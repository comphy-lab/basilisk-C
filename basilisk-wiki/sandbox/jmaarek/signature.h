...
*/
void change_topology (scalar f, scalar s, scalar M, int lev){

  const int max_change = 10;  // max number of topology changes
  double f_avg[max_change];  // average volume fraction in the neighbor
  ...
  foreach_cache(to_perf){
    s[] = -2.;
    M[] = f[] - (1 - f_avg[_flags]);  //Mass added/removed with perforations
    f[] = 1 - f_avg[_flags];
  }
