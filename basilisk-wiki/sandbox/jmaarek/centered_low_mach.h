...
#include "bcg.h"
#if EMBED
#include "contact_embed/viscosity_embed_compressible.h"
#else
#include "contact_embed/viscosity-compressible.h"
#endif
...
face vector uf[];
scalar drhodt[];
double P_ref = 101325.0; //thermodynamic pressure
double P_ref_0 = 101325.0; //thermodynamic pressure
...
(const) face vector mu = zerof, lambdav = zerof, a = zerof, alpha = unityf;
(const) scalar rho = unity;
mgstats mgp = {0}, mgpf = {0}, mgu = {0};
bool stokes = false;

trace
mgstats project_sf (face vector uf, scalar p, (const) face vector alpha = unityf, double dt = 1., int nrelax = 4){
  ...
  //rho.restriction = restriction_average;
  //boundary({rho});
...
event viscous_term (i++,last)
{
  if (constant(mu.x) != 0.) {
    correction (dt);
    mgu = viscosity (u, mu, rho, dt, mgu.nrelax, lambdav = lambdav);
