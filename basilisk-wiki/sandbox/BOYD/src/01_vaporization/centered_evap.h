...
#define neumann_pressure(i)                     \
    (alpha.n[i] ? a.n[i] * fm.n[i] / alpha.n[i] \
                : a.n[i] * rho[] / (cm[] + SEPS))
#else
...
event defaults(i = 0) {
  ...
  for (scalar s in{p, pf}) s.embed_gradient = pressure_embed_gradient;
#endif  // EMBED
#endif  // TREE
}
...
event advection_term(i++, last) {
  if (!stokes) {
    prediction();
    // evap
    mgpf = project_evap(uf, pf, alpha, dt / 2., mgpf.nrelax);
...
event projection(i++, last) {
  // evap
  mgp = project_evap(uf, p, alpha, dt, mgp.nrelax);
