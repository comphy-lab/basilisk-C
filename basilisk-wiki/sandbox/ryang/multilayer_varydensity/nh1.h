			   ...
			   scalar * resl, void * data)
{
  ...
  foreach (reduction(max:maxres)) {
    ...
    */
	...
	double hk = h[0,0,k];
	if (hk > dry)
	  res[] += 8.*s*(phi[0,0,k] - phi[0,0,k+1])*h[]*rho[point.l]/(rho[point.l+k]*hk);
      }
      if (fabs (res[]) > maxres)
	maxres = fabs (res[]);
      foreach_dimension()
...
~~~
  ...
  foreach (reduction(+:h1) reduction(+:v1)) {
	...
	foreach_dimension()
      ...
      for (int k = - 1, s = -1; k >= - point.l; k--, s = -s)
	rhs[] += 4.*s*(w[0,0,k] - theta_H*dt*G);
      rhs[] *= 2.*h[]*rho[point.l]/(theta_H*dt);
      foreach_dimension()
	dz.x += hf.x[1] - hf.x[];
      h1 += dv()*h[];
      v1 += dv();
  ...
  */
  ...
  foreach() {
    ...
    foreach_layer()
      if (h[] > dry) {
	if (point.l == nl - 1)
	  w[] += dt*(phi[]/(h[]*rho[point.l]) - G);
	else
	  w[] -= dt*((phi[0,0,1] - phi[])/(h[]*rho[point.l]) + G);
	if (fabs(w[]) > wmax)
	  w[] = (w[] > 0. ? 1. : -1.)*wmax;
      }
  }
