#ifndef _MY_POISSON_H
#define _MY_POISSON_H
/**
The original Poisson solver is modified when solid boundaries are considered. At the fluid-solid boundary, we always impose Neumann(0) for the pressure. This is achieved by adjusting the coefficients accordingly.
*/
#if USE_MY_SOLID
...
mgstats project (struct Project q)
{
  ...
  foreach_face()
  {
    //alpha at the solid wall can be set to zero 
    //as neumann(0) boundary condition will be imposed for pressure
...
mgstats project2 (struct Project q)
{
  face vector uf = q.uf;
  scalar p = q.p;
  (const) face vector alpha = q.alpha.x.i ? q.alpha : unityf;
...
#if USE_MY_SOLID
mgstats poisson_solid (struct Poisson p)
{
  ...
  restriction((scalar *){alpha2});
  foreach_face()
  {
    bool is_solid_p = (int)is_solid[] == 1;
    bool is_solid_m = (int)is_solid[-1] == 1;

    double isf = (is_solid_p || is_solid_m) ? 1.0 : 0.0;
    alpha2.x[] = (1.0 - isf) * alpha.x[];
  }
  restriction((scalar *){alpha2});
  p.alpha = alpha2;
