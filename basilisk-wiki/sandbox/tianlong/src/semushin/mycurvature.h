#ifndef _MY_CURVATURE_H
#define _MY_CURVATURE_H
/**
The original curvature.h file is modified to impose correct boundary conditions when considering solid boundaries inside the domain.
...
cstats curvature (struct Curvature p)
{
  ...
  extern void boundarySolidHeight_x(scalar s, int dir);
  extern void boundarySolidHeight_y(scalar s, int dir);
  foreach_dimension()
  {
    if(IS_SOLID_x)
    {
      h.x.boundarySolid_x = boundarySolidHeight_x;
    }
  }
#endif
  if (!ch.x.i)
