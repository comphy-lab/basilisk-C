...
This file is mainly copied from Edoardo's sandbox, with only a few changes made to include solid boundaries. Note that this method is a one-fluid method, i.e., a velocity field with discontinuities at the interface will be obtained
...
mgstats project_sf (struct Project q)
{
  ...
  extern void setStefanFlow(scalar stefanflow);
  setStefanFlow(stefanflow);
...
#if 0
  face vector difu[];
  // ATTENSION: this is not compatible with embded
  difu.n[bottom] = 0.0;
  difu.n[left] = 0.0;
  foreach_face()
  {
    difu.x[] = (uf.x[1] - 2.0 * uf.x[] + uf.x[-1]) / 4.0;
    //difu.x[] = 0.0;
  }

  trash({uf});
  foreach_face()
      uf.x[] = (cm[] * u.x[] + cm[-1] * u.x[-1]) / 2.0 - difu.x[] + fm.x[] * dt * a.x[];
#else
  trash({uf});
  boundary({uf, a});
  foreach_face()
...
#if SEMUPC
  // extern void setCenterVfromFaceV(vector u, face vector uf);
  // setCenterVfromFaceV(u, uf);
  // foreach ()
  // {
  //   foreach_dimension()
  //       g.x[] = 0.0;
