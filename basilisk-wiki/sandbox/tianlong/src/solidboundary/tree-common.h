...
#if  USE_MY_SOLID
extern bool IS_SOLID_x, IS_SOLID_y;
#endif
...
#if USE_MY_SOLID
//fixme: with foreach_dimension(), the part of obtaining 
//scalars, vectors and faces is wrong
bool imposeSolidBoundaryTree(scalar * list, int level, int idim)
{
  bool is_imposed = false;
  if(level < depth() || depth() < grid->maxdepth)
  {
    return is_imposed;
  }
  disable_fpe_for_mpi();
  ...
  for(scalar s in scalars)
  {
    if(idim == 1)
    {
      if(s.boundarySolid_x)
      {
        is_imposed = true;
        s.boundarySolid_x(s, idim);
      }
    }
    else if(idim == 2)
    {
      if(s.boundarySolid_y)
      {
        is_imposed = true;
        s.boundarySolid_y(s, idim);
      }
    }
  }
  ...
  for (vector v in vectors)
  {
    if(idim == 1)
    {
      if (v.x.boundarySolid_x)
      {
        is_imposed = true;
        v.x.boundarySolid_x(v.x, idim);
      }
    }
    else if(idim == 2)
    {
      if (v.y.boundarySolid_y)
      {
        is_imposed = true;
        v.y.boundarySolid_y(v.y, idim);
      }
    }
  }

  for (vector vf in faces)
  {
    if(idim == 1)
    {
      if (vf.x.boundarySolid_x)
      {
        is_imposed = true;
        vf.x.boundarySolid_x(vf.x, idim);
      }
    }
    else if(idim == 2)
    {
      if (vf.y.boundarySolid_y)
      {
        is_imposed = true;
        vf.y.boundarySolid_y(vf.y, idim);
      }
    }
  }
...
static void tree_boundary_level (scalar * list, int l)
{
  ...
  if (tree_is_full()) {
    ...
    foreach_dimension()
    {
      ...
      if(IS_SOLID_x)
      {
        bool is_solidbc_imposed = imposeSolidBoundaryTree(list, depth, idim);
        if(is_solidbc_imposed)
        {
          boundary_iterate (level, list, depth);
        }
      }
  ...
  if (listdef || listc) {
    ...
    foreach_dimension()
    {
      ...
      if(IS_SOLID_x)
      {
        bool is_solidbc_imposed = imposeSolidBoundaryTree(list2, depth, idim);
        if(is_solidbc_imposed)
        {
          boundary_iterate (restriction, list2, depth);
        }
