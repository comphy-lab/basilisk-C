#ifndef _MY_HEIGHTS_H
#define _MY_HEIGHTS_H
/**
# Height-Functions
...
static void column_propagation (vector h)
{
  foreach (serial) // not compatible with OpenMP
  {
    ...
    if(is_solid[] == 0.0)
      ...
      for (int i = -2; i <= 2; i++)
        foreach_dimension()
          if (fabs(height(h.x[i])) <= 3.5 && fabs(height(h.x[i]) + i) < fabs(height(h.x[])))
            h.x[] = h.x[i] + i;
  }
}
...
void heights (scalar c, vector h)
{
  ...
  extern void boundarySolidVectorNeumann_x(scalar s, int dir);
  extern void boundarySolidVectorNeumann_y(scalar s, int dir);
  foreach_dimension()
  {
    if(IS_SOLID_x)
    {
      s.x.boundarySolid_x = boundarySolidVectorNeumann_x;
    }
  }
#endif
  /**
...
void heights (scalar c, vector h)
{
  ...
  for (int j = -1; j <= 1; j += 2) {
    ...
    {
      idim ++;
      bool is_solidbc_imposed = imposeSolidBoundaryTree((scalar *){s}, l, idim);
  ...
  //trick for setting the boundary height
  scalar is_contact[];
  scalar h_bc[];
  foreach ()
  {
    is_contact[] = 0.0;
    h_bc[] = 0.0;
    if (is_solid[] == 0 && is_solid[-1] == 1) // near the solid boundary
    {
      double f = c[];
      double fm = c[0, -1];
      double fp = c[0, 1];
      ...
      if (f > 0.0 && f < 1.0)
      {
        ...
        if (is_contact[] == 1.0)
        {
          int sign = fm > fp ? -1 : 1;
          double h1 = h.x[];
          double h2 = h.x[0, sign];
          if (h2 != nodata)
          {
            double s = orientation(h2) ? 1.0 : -1.0;
            h_bc[] = h2 + 2.0 * s * tan(theta0 / 180.0 * pi);
          }
          else if (h1 != nodata)
          {
            double s = orientation(h1) ? 1.0 : -1.0;
            h_bc[] = h1 + s * tan(theta0 / 180.0 * pi);
          }
        }
      }
    }
