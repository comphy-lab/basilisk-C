...
{
  scalar phi; 
  int imax;       // The maximum number of iterations
  double cfl;   // The CFL number
  int order;      // The order of time integration
  double eps;  // The maximum error on $|\nabla\phi| - 1$
  double band; // The width of the band in which to compute the error
  scalar resf;
};
...
trace 
int redistance(struct Redistance p)
{
  scalar phi = p.phi;
  int imax = p.imax ? p.imax : 1;       // The maximum number of iterations
  double cfl = p.cfl ? p.cfl : 0.5;   // The CFL number
  int order = p.order ? p.order : 3;      // The order of time integration
  double eps = p.eps ? p.eps : 1e-6;  // The maximum error on $|\nabla\phi| - 1$
  double band = p.band ? p.band : HUGE; // The width of the band in which to compute the error
  scalar tmp = {-1};
  scalar resf = p.resf.i ? p.resf : tmp; // The residual $|\nabla\phi| - 1$
