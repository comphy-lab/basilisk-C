...
static double vof_concentration_gradient_x (Point point, scalar c, scalar t)
{
  ...
  double Tst[5] = {0.0};
  Tst[0] = cll > F_thd ? t[-2] / cll : TIntVal;
  Tst[1] = cl > F_thd ? t[-1] / cl : TIntVal;
  Tst[2] = cc > F_thd ? t[] / cc : TIntVal;
  Tst[3] = cr > F_thd ? t[1] / cr : TIntVal;
  Tst[4] = crr > F_thd ? t[2] / crr : TIntVal;
  ...
  double vel_c = uf.x[] + uf.x[1];
  double dTc = (cl - cr) > 0.0 ? weno3P(&dT[1]) : weno3M(&dT[1]);

  grad = cc > 0.5 ? dTc / Delta : 0.0;
  return grad;
