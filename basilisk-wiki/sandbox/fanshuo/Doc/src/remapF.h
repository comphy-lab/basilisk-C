...
void geometric_beta (double ratio,bool top)
{
  //double r = 1. + 0.05*(1./rmin - 1.)/(nl - 1.);
  double hmin = (ratio - 1.)/(pow(ratio, nl) - 1.);
  for (int l = 0; l < nl; l++)
    beta[l] = hmin*pow(ratio, top ? nl - 1 - l : l);
  printf("geometric progression for layer thinckness is adopted\n");
  printf("rmin : %lf, ratio : %lf\n",hmin,ratio);
}
/**
