...
$$ */
double muBagnoldC(Point,double,int,scalar,scalar,int,double,double,double,double,double,double);
double muBingham(Point,int, scalar,scalar,int, double, double);
double muBagnold(Point,double,int,scalar,scalar,int,double,double,double,double,double);
void vertical_diffusion (Point point, scalar h, scalar s, double dt, double D,
			 double dst, double s_b, double lambda_b,double nu,double mu, double tauy,
			 double deltamu, double I0, double mu0, double dg, double rho, double tauc)
{
  double a[nl], b[nl], c[nl], rhs[nl],nueq[nl];
  double shear;
  int nlc = 1e6;
  /**
  ...
  */
  ...
  #if Poiseuille
    for(int l=0 ; l<nl;l++)  nueq[l] = nu;
  #else
    //shear between layers are firstly computed
    for(int l=0 ; l<nl;l++){
      if(l>0&&l<(nl-1))
        shear = (s[0,0,l+1]-s[0,0,l-1])/(h[0,0,l]+0.5*(h[0,0,l-1]+h[0,0,l+1]));
      else if(l==0) 
        shear = s[0,0,0]/(0.5*h[0,0,0]);
	  else if(l==nl-1)
	    shear = (s[0,0,nl-1]-s[0,0,nl-2])/(0.5*(h[0,0,nl-1]+h[0,0,nl-2]));
	  if (shear<=0.005) {nlc = l-1;break;}
    }

    for(int l=0 ; l<nl;l++){
	  if (l<=nlc){	
            #if BagnoldC
              nueq[l]               =muBagnoldC(point,slope,nl,s,h,l,deltamu,I0,mu0,dg,rho,tauc);
            #elif Bingham
              nueq[l] = muBingham(point,nl,s,h,l,tauy,mu);
            #elif Bagnold
              nueq[l] = muBagnold(point,slope,nl,s,h,l,deltamu,I0,mu0,dg,rho);
            #endif
	  }
...
double nu;
double mu, tauy;
double mu0,deltamu,I0,dg,rho,tauc;
/*
...
event viscous_term (i++,last)
{
  if (1 > 0.) {
	...
	vertical_diffusion (point, h, u.x, dt, nu,
			    dut.x[], u_b.x[], lambda_b.x[],nu,mu,tauy,deltamu,I0,mu0,dg,rho,tauc);
      foreach_layer()
	foreach_dimension()
	  u.x[] -= dt*(ha.x[] + ha.x[1])/(hf.x[] + hf.x[1] + dry);
    }
