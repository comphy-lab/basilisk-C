/**
# Remarks 
The following codes present [Multilayer hydrostatic](https://www.basilisk.fr/src/layered/hydro.h) solver. Compared to the orignial version, the only change is that we include "diffusionRheo.h" to take various viscosity into account. 

As the following codes will not be renew as the source code, it is recommand to check regularily the latest changes made in [Multilayer hydrostatic](https://www.basilisk.fr/src/layered/hydro.h) solver and to renew this file.

The current version is from avril 2026.
...
![Definition of the $n$ layers.](https://www.basilisk.fr/src/figures/layers.svg){ width="60%" }
