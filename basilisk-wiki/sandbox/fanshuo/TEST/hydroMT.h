...
event face_fields (i++, last)
{
  ...
  foreach_face (reduction (min:dtmax)) {
    ...
    if (H > dry) {
      //fprintf(stderr,"in the CFL\n");
      double c = um/CFL + sqrt(G*(hydrostatic ? H : Delta*tanh(H/Delta)))/CFL_H;
      if (c > 0.) {
	double dt = min(cm[], cm[-1])*Delta/(c*fm.x[]);
	if (dt < dtmax){
	  dtmax = dt;  //fprintf (stderr, "in %g %g %g %g %g %g\n", H, dt, dtmax, 2*um/CFL, 2*sqrt(G*H)/CFL_H, c);
    }
      }
    }
    //else fprintf(stderr,"out the CFL\n");
...
void advect (scalar * tracers, face vector hu, face vector hf, double dt)
{
  ...
  //double temp = t; int modul = temp;
    //if ( modul%10 == 0 ) fprintf (stdout, "flux check %g %d %g %g %g %g\n", t, _layer, x, h[], h1, h1-h[] );
