...
/** We measure the sphere's rotation rate in the z-direction by
imposition a simple shear along the x direction. The velocity field
reads $\vec{u} = \left(u,v,w\right) = \left(\dot{\gamma}y,0,0
...
/** Given $\vec{x} = \left(x,y,z\right)$, a point on the surface of the sphere and $\vec{\omega^p} = \left(\omega^p_x,\omega^p_y,\omega^p_z \right)$, it comes $$\frac{1}{2}\dot\gamma y  = \omega^p_y z - \omega^p_z y$$ $$-\frac{1}{2}\dot\gamma x  = \omega^p_z x - \omega^p_x z$$ $$ \omega^p_x y = \omega^p_y x$$*/

/** for $\vec{x} = \left(D/2,0,0\right)$, it comes $$-\frac{1}{2}\dot{\gamma} = \omega^p_z$$ and $\omega^p_y = 0$*/
...
~~~gnuplot particle's angular velocities $\omega^p_x$, $\omega^p_y$
set grid
show grid
...
~~~gnuplot particle's angular velocity $\omega^p_z$ 
set grid
show grid
Uc = 1.;
L = 20;
...
<source src="shear3D_x_zRotation/movie.mp4" type="video/mp4">
</video>
*/
