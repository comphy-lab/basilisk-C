/**
# Sphere advected in the z-direction in a tri-periodic domain with DLMFD
*/

/** A sphere twice as heavy as the surrounding fluid is advected in
  the z-direction by a pressure gradient */
...
# define mydt (Tc/200) // maximum time-step (the time step is also adaptive in time but it won't exceed this value)
# define maxtime (1.5)
# define tsave (Tc/200.)
...
event output_data (t += tsave; t < maxtime) {
  ...
  clear();
  box();
  squares ("u.z", n = {1,0,0}, alpha = L0/2, map = cool_warm, min = statsvelox.min, max = statsvelox.max);
...
<source src="sphere-periodic-z/movie.mp4" type="video/mp4">
</video>
*/
