...
#define S0 0. // 0.001 oppose de la pente du canal [m/m]
...
double q_injection(double t) // debit injecte en amont au temps t
{
    ...
    return ( QBASE + fmax( 0., ( t < t_pic ? pente_crue * (t - t_0) : pente_decrue * (t - t_pic) + q_max - QBASE) ) ); // debit au temps t (hydrogramme triangulaire)
...
event init(t=0)
{
    ...
    h_base = pow( NMANNING*NMANNING * QBASE*QBASE / tilt.x , 3./10. );
    //h_base = 1.026;
...
event flows(i++) 
{
    ...
    foreach()
    {
        ...
        q[]  = h[] * u.x[];
        hc[] = pow( q[] * q[] / G, (1./3.) );
        hn[] = pow( NMANNING*NMANNING * q[]*q[] / tilt.x , 3./10. );
    }
}
...
event outputs_tab(t+=60.)
{
  ...
  foreach()
    fprintf (fp, "%g %g %g %g %g\n",x,zb[]-x*tilt.x,h[],h[]+zb[]-x*tilt.x,u.x[]);
...
~~~gnuplot
   ...
   plot 'profile-0.dat' u 1:2 w lines lc rgb "black" lw 2 title 'zb', \
        ...
        'profile-300.dat' u 1:4 w lines lc rgb "magenta" title 'eta(t=5min)', \
        'profile-1800.dat' u 1:4 w lines lc rgb "red" title 'eta(t=30min)', \
        'profile-3600.dat' u 1:4 w lines lc rgb "green" title 'eta(t=60min)', \
        'profile-5400.dat' u 1:4 w lines lc rgb "yellow" title 'eta(t=90min)'
~~~
~~~gnuplot
   reset
   set xlabel "x (m)"
   ...
   plot 'profile-0.dat' u 1:5 w lines lc rgb "blue" title 'u(t=0)', \
        'profile-300.dat' u 1:5 w lines lc rgb "magenta" title 'u(t=5min)', \
        'profile-1800.dat' u 1:5 w lines lc rgb "red" title 'u(t=30min)', \
        'profile-3600.dat' u 1:5 w lines lc rgb "green" title 'u(t=60min)', \
        'profile-5400.dat' u 1:5 w lines lc rgb "yellow" title 'u(t=90min)'
~~~
*/
