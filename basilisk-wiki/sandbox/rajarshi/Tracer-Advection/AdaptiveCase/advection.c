/**
# Advection of a scalar field - Adaptive grid simulation
...
$$ Tracer(x_c,y_c) = \frac{1}{\Delta^2} \int_{y_c - \frac{\Delta}{2}}^
...
The volume average is constructed over each cell by means of the standard Gaussian quadrature 
three point formulation.
...
/**
#Face Velocity computation function : 

The velocity field is derived from the following Stream Function formulation. 
The face centered velocity fields are then line integrated over the cell 
boundaries to obtain face averaged velocity values.

 $$\psi(x,y,t) = \frac{1.5}{\pi}  sin\left( \frac{2 \pi t}{5}\right )   cos( \pi x )   cos( \pi y ) $$
 $$\mathbf{u} = \frac{\partial \psi}{\partial y} \hat{\mathbf{i}}   -   \frac{\partial \psi}{\partial x} \hat{\mathbf{j}} $$
*/
...
int main()
{
  ...
  #if WENO
    for (cmax = 0.02; cmax >= 0.0005; cmax *= 0.5){
         start=clock();
         N=8;
         run();
         end=clock();
  ...
  #else
    for (cmax = 0.05; cmax >= 0.0025; cmax *= 0.5){
         N=8;
         start=clock();
         run();
         end=clock();
  ...
  #endif
  fclose(fp2);
  system( "paste ErrorvsGrid.dat TimeComputing.dat > ErrorvsTime.dat" );
}
...
{ 
  do {
       ...
       boundary({f});
       statistics = adapt_wavelet ({f}, (double []){cmax}, maxlevel = 15);
     }while (statistics.nf > 1 || statistics.nc > 1);  

}

/**
#Simulation statistics events
*/
...
event field (t = 5) {
  ...
  scalar e[];
  foreach()
    e[] = f[] - VolumeAvg ( x, y, Delta, -0.2, -0.236338 ) ;
  norm n = normf (e);
...
This module uses a higher order prolongation function for WENO implementation
while using a bilinear formulation for the bcg case. The details are in the 
