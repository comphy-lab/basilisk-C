/**
# Advection of a scalar field on adaptive grids

 Using either the second-order Bell--Colella--Glaz scheme or the
5th-order WENO scheme */

static double FuncValues (double x, double y, double offsetx, double offsety) {
  if(sq(x-offsetx)<=0.0225 && sq(y-offsety)<=0.0225) 
    return (pow(1-sq((x-offsetx)/0.15),7)*pow(1-sq((y-offsety)/0.15),7));
  else
    return 0;
}

static double Neumann (double x, double y, double Delta, double offsetx, double offsety) {
  double sum=0.;
  double q = (Delta/2.)*sqrt(3./5.);
  sum += (5./18.)*( (5./18.)*FuncValues(x-q,y-q,offsetx,offsety) + (8./18.)*FuncValues(x,y-q,offsetx,offsety) + (5./18.)*FuncValues(x+q,y-q,offsetx,offsety) );
  sum += (8./18.)*( (5./18.)*FuncValues(x-q,y,offsetx,offsety) + (8./18.)*FuncValues(x,y,offsetx,offsety) + (5./18.)*FuncValues(x+q,y,offsetx,offsety) );
  sum += (5./18.)*( (5./18.)*FuncValues(x-q,y+q,offsetx,offsety) + (8./18.)*FuncValues(x,y+q,offsetx,offsety) + (5./18.)*FuncValues(x+q,y+q,offsetx,offsety) );
  return (sum);
}

#include "grid/quadtree.h"
#include "../Header_Advection/advection.h"
#include "view.h"

void face_velocity (face vector u, double t)
{
  foreach_face(x)
    u.x[] =  (1.5/(pi*Delta))*sin(2.*pi*t/5.)*cos(pi*x)*(cos(pi*(y+Delta/2.))-cos(pi*(y-Delta/2.)));
  foreach_face(y)
    u.y[] = -(1.5/(pi*Delta))*sin(2.*pi*t/5.)*cos(pi*y)*(cos(pi*(x+Delta/2.))-cos(pi*(x-Delta/2.))); 
  boundary((scalar *){u});

}

#if !WENO
event velocity (i++) {
  face_velocity (u, t);
}
#endif

scalar f[];
scalar * tracers = {f};

double cmax;
clock_t start,end;

int main()
{
  system("rm -f ErrorvsGrid.dat");
  origin (-0.5, -0.5);
  DT = 0.1;
  CFL = 0.8;

  FILE * fp2 = fopen ("TimeComputing.dat","w");
  #if WENO
    for (cmax = 0.02; cmax >= 0.0005; cmax *= 0.5){
         start=clock();
         N=8;
         run();
         end=clock();
         fprintf(fp2,"%g \n",(end-start)/((double) CLOCKS_PER_SEC));
    }
  #else
    for (cmax = 0.05; cmax >= 0.0025; cmax *= 0.5){
         N=8;
         start=clock();
         run();
         end=clock();
         fprintf(fp2,"%g \n",(end-start)/((double) CLOCKS_PER_SEC));
    }
  #endif
  fclose(fp2);
  system("paste ErrorvsGrid.dat TimeComputing.dat > ErrorvsTime.dat");
}

scalar Error[];
astats statistics;

event init (i = 0)
{ 
  do {
       foreach()
          f[] = Neumann(x,y,Delta,-0.2,-0.236338);
       boundary({f});
       statistics = adapt_wavelet ({f}, (double []){cmax}, maxlevel = 15);
     }while (statistics.nf > 1 || statistics.nc > 1);  

}


event logfile (i++) {
  stats s = statsf (f);
  fprintf (stderr, "# %f %ld %.12f %g %g\n", t, grid->tn, s.sum, s.min, s.max);
}


event deformed (t = 2.5) {
  view (fov = 19.35, width = 400, height = 400);
  clear();
  squares ("f", linear = true);
  save ("f.png");
}


event field (t = 5) {

  scalar e[];
  foreach()
    e[] = f[] - Neumann(x,y,Delta,-0.2,-0.236338);
  norm n = normf (e);
  FILE * fp1 = fopen("ErrorvsGrid.dat","a");
  fprintf (fp1, "%g \t %g \t %g \t %g \t %g\n",cmax, sqrt(grid->tn), n.avg, n.rms, n.max);
  fclose(fp1);

  view (fov = 19.35, width = 400, height = 400);
  clear();
  squares ("e", linear = true);
  save ("e.png");
  
}

event adapt (i++,last){
  statistics = adapt_wavelet ({f}, (double []){cmax}, maxlevel = 15);
}

/**

#BCG SCHEME RESULTS

![Tracer field at $t=2.5$](advection/f.png)

![Error field at $t=5$ (BCG)](advection/e.png)

~~~gnuplot Convergence with spatial resolution (BCG)
ftitle(a,b) = sprintf("%.0f/x^{%4.2f}", exp(a), -b)
f(x)=a+b*x
fit f(x) 'ErrorvsGrid.dat' u (log($2)):(log($5)) via a,b
f2(x)=a2+b2*x
fit f2(x) 'ErrorvsGrid.dat' u (log($2)):(log($3)) via a2,b2
set xlabel 'Maximum resolution'
set ylabel 'Maximum error'
set logscale
set xrange [16:64]
set xtics 16,2,64
set grid ytics
set cbrange [1:2]
plot 'ErrorvsGrid.dat' u 2:5 t 'max', 'ErrorvsGrid.dat' u 2:3 t 'norm1', exp(f(log(x))) t ftitle(a,b), \
      exp(f2(log(x))) t ftitle(a2,b2)
~~~ 

#WENO SCHEME RESULTS

![Error field at $t=5$ for $N=128$ (WENO)](advection-weno/e.png)

![Convergence with spatial resolution (WENO)](advection-weno/_plot0.png)

#Computation Time

~~~gnuplot Error vs Time 
set xlabel 'Computing Time'
set ylabel 'Maximum-Error'
set logscale
set xrange [2:64]
set xtics 2,2,64
set grid ytics
plot 'ErrorvsTime.dat' u 6:5 w lp t 'Time-BCG' ps 2, \
     '../advection-weno/ErrorvsTime.dat' u 6:5 w lp t 'Time-WENO' ps 2 
~~~ 

*/
