/**
#Shallow Water Test Case with Flux limiter order 2 reconstruction (Error-Scalability) - Le Metayer Test Case
*/
...
void Compute_Error(double time){
  ...
  foreach(){
   ...
   fprintf(fp1,"%g %g %g \n",log10(N),log10(normf(u_error).rms),log10(normf(h_error).rms));  
   fclose(fp1);
