/**
## Testing the Residual operator -  4th order solver (Heterogenous Refinement / Quadtree)

This test case is performed to study the accuracy of the residual operator. The case is simplified, in that - the multigrid cycles have been left out. The results demonstrate that for the fourth order case converged solution is not obtained, hence we may conclude the residual & relaxation operators are not fine.
...
void relax4th(scalar da, scalar residue, int NRelax){
   ...
   for(int i=0;i<NRelax;i++){
      foreach(){
         ...
         d=0;
         foreach_dimension(){
            n += ( (da[-1,2]-27*da[0,2]+27*da[1,2]-da[2,2]) - (da[-2,2]-27*da[-1,2]+27*da[0,2]-da[1,2]) )*(-17.)/(5760.*24.);
            n += ( (da[-1,1]-27*da[0,1]+27*da[1,1]-da[2,1]) - (da[-2,1]-27*da[-1,1]+27*da[0,1]-da[1,1]) )*(308.)/(5760.*24.);
            n += ( (da[-1,0]+27*da[1,0]-da[2,0]) - (da[-2,0]-27*da[-1,0]-da[1,0]) )*(5178.)/(5760.*24.);
            n += ( (da[-1,-1]-27*da[0,-1]+27*da[1,-1]-da[2,-1]) - (da[-2,-1]-27*da[-1,-1]+27*da[0,-1]-da[1,-1]) )*(308.)/(5760.*24.);
            n += ( (da[-1,-2]-27*da[0,-2]+27*da[1,-2]-da[2,-2]) - (da[-2,-2]-27*da[-1,-2]+27*da[0,-2]-da[1,-2]) )*(-17.)/(5760.*24.);
            d += (27.*2.*5178.)/(5760.*24.);
           }
...
double residual4th (scalar a, scalar b, scalar residue){
   ...
   foreach(){
      residue[] = b[];
      foreach_dimension(){
         residue[] -= ( (a[-1,2]-27*a[0,2]+27*a[1,2]-a[2,2]) - (a[-2,2]-27*a[-1,2]+27*a[0,2]-a[1,2]) )*(-17.)/(5760.*24.*sq(Delta));
         residue[] -= ( (a[-1,1]-27*a[0,1]+27*a[1,1]-a[2,1]) - (a[-2,1]-27*a[-1,1]+27*a[0,1]-a[1,1]) )*(308.)/(5760.*24.*sq(Delta));
         residue[] -= ( (a[-1,0]-27*a[0,0]+27*a[1,0]-a[2,0]) - (a[-2,0]-27*a[-1,0]+27*a[0,0]-a[1,0]) )*(5178.)/(5760.*24.*sq(Delta));
         residue[] -= ( (a[-1,-1]-27*a[0,-1]+27*a[1,-1]-a[2,-1]) - (a[-2,-1]-27*a[-1,-1]+27*a[0,-1]-a[1,-1]) )*(308.)/(5760.*24.*sq(Delta));
         residue[] -= ( (a[-1,-2]-27*a[0,-2]+27*a[1,-2]-a[2,-2]) - (a[-2,-2]-27*a[-1,-2]+27*a[0,-2]-a[1,-2]) )*(-17.)/(5760.*24.*sq(Delta));
       }
      if(fabs(residue[]) > maxres)
...
*/
   ...
   while(maxresidue > 1E-06 && ctr < 10000){
      ctr++;
      boundary({A,dA});
      maxresidue = residual4th(A,B,Res);
      relax4th(dA,Res,NRelaxation);
      foreach(){
        A[] += dA[];
        dA[] = 0;
       }
