/**
# Taylor Green Vortex - WENO5 + RK4 + O4 Projection.
*/

#include "grid/multigrid.h"
#include "centered_weno5_poisson4_rk4.h"
...
event error (t = 2)
{
  scalar e[];
  foreach(){
    double u0 =   (sin(2.*pi*(x+Delta/2.))-sin(2.*pi*(x-Delta/2.)))*(cos(2.*pi*(y+Delta/2.))-cos(2.*pi*(y-Delta/2.)))/(sq(2.*pi*Delta));
    double v0 = - (cos(2.*pi*(x+Delta/2.))-cos(2.*pi*(x-Delta/2.)))*(sin(2.*pi*(y+Delta/2.))-sin(2.*pi*(y-Delta/2.)))/(sq(2.*pi*Delta));  
...
![Animation of the pressure
 field.](taylorgreen_weno_poisson4/Pressure.mp4)

![Animation of the kinetic energy
 field.](taylorgreen_weno_poisson4/Kinetic-Energy.mp4)

For this particular case, the WENO scheme converges at order 5
...
By fitting the decrease of kinetic energy we get an estimate of the
numerical viscosity
