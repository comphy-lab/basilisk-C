...
static void relax_viscosity (scalar *a, scalar *b, int l, void * data){
   ...
   foreach_level_or_leaf (l) {
      foreach_dimension()
         w.x[] =   (dt/rho[])* (   2.*mu.x[1]*(u.x[-1] + 15.*u.x[1] - u.x[2])/(24.*Delta)
                                 - 2.*mu.x[0]*(u.x[-2] - 15.*u.x[-1] - u.x[1])/(24.*Delta)
         #if dimension > 1
                   + mu.y[0,1]*(    (u.x[0,-1]+15.*u.x[0,1]-u.x[0,2])/(24.)
                   ...
                   - mu.y[0,0]*(    (u.x[0,-2]-15.*u.x[0,-1]-u.x[0,1])/(24.)
...
static double residual_viscosity (scalar * a, scalar * b, scalar * resl, void * data){
   ...
   double maxres = 0.;
      ...
      foreach_dimension() {
          face vector taux[];
          foreach_face (x)
              taux.x[] = 2.*mu.x[]*(u.x[-2]-15.*u.x[-1]+15.*u.x[]-u.x[1])/(24.*Delta);
          #if dimension > 1
          foreach_face (y)
              taux.y[] = 2.*mu.y[]*( (u.x[0,-2]-15.*u.x[0,-1]+15.*u.x[0,0]-u.x[0,1])/(24.*Delta) 
                       + (   1.*(-u.y[-2,1] + 7.*u.y[-2,0] + 7.*u.y[-2,-1] - u.y[-2,-2]) 
                           - 8.*(-u.y[-1,1] + 7.*u.y[-1,0] + 7.*u.y[-1,-1] - u.y[-1,-2])  
                           + 8.*(-u.y[1,1]  + 7.*u.y[1,0]  + 7.*u.y[1,-1]  - u.y[1,-2])
                           - 1.*(-u.y[2,1]  + 7.*u.y[2,0]  + 7.*u.y[2,-1]  - u.y[2,-2])
      ...
      foreach(reduction(max:maxres))
         foreach_dimension(){
             res.x[] = r.x[] - lambda.x*u.x[] + (dt/rho[])*(
                                                           2.*mu.x[1,0]*(u.x[-1] - 15.*u.x[0]  + 15.*u.x[1] -u.x[2])/(24.*Delta) 
                                                         - 2.*mu.x[0,0]*(u.x[-2] - 15.*u.x[-1] + 15.*u.x[]  -u.x[1])/(24.*Delta)
             #if dimension > 1
                       + mu.y[0,1]*(    (u.x[0,-1]-15.*u.x[0,0]+15.*u.x[0,1]-u.x[0,2])/(24.)
                       ...
                       - mu.y[0,0]*(    (u.x[0,-2]-15.*u.x[0,-1]+15.*u.x[0,0]-u.x[0,1])/(24.)
