...
void RK3_TVD (scalar *fL, face vector u, double dt){

   scalar f = fL[0];
   scalar * fList = list_clone ({f,f});
   scalar f1 = fList[0], f2 = fList[1], k1[], k2[], k3[];
   face vector flux_value[];
    
   Tracer_fluxes (f,u,flux_value);
   foreach(){
      k1[] = 0.;
      foreach_dimension()
   ...
   Tracer_fluxes (f1,u,flux_value);
   foreach(){
      k2[] = 0.;
      foreach_dimension()
         k2[] += (flux_value.x[]-flux_value.x[1])/Delta;
      f2[] = 3.*f[]/4. + f1[]/4. + k2[]*dt/4.;
   }
   boundary({f2});

   Tracer_fluxes (f2,u,flux_value);
   foreach(){
      k3[] = 0.;
      foreach_dimension()
         k3[] += (flux_value.x[]-flux_value.x[1])/Delta;
      f[] = f[]/3. + 2.*f2[]/3. + 2.*k3[]*dt/3.;
   }
   boundary({f});
}
