...
static double height_curvature (Point point, scalar c, vector h)
{
  ...
  if (cet.i) {
    assert (cet[] < nodata);
    foreach_dimension()
      if (cet[-1] < nodata && cet[1] < nodata)
        n.x.n = cet[1] - cet[-1];
      else if (cet[-1] < nodata)
        n.x.n = cet[] - cet[-1];
      else if (cet[1] < nodata)
        n.x.n = cet[1] - cet[];
      else
...
#if EMBED
  fprintf (ferr, "Warning: position() is not implemented for embed boundary!\n");
#endif
