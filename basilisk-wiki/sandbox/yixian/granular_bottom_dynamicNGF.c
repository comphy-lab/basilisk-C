...
# Flow in a Sandglass / Hourglass / Silo with a bottom orifice
 We propose an implementation of the dynamic non-local granular fluidity(dynamic NGF) model proposed by Kamrin and Henann(2015) for the flow in a silo with a bottom orifice.
...
scalar gnonloc[];   /* nonlocal fluidity (solution) */
scalar gold[];     //  g^n
...
/**
We use the full dynamic form of the NGF equation:
 
$$
t_0\dot{g} = A^{2}d^{2}\nabla^{2}g - \Delta{\mu}\left(\frac{\mu_{s} - \mu}{\mu_{2} - \mu}\right)g - b\sqrt{\frac{\rho_{s}d^{2}}{p}}\mu g^{2}.
$$

In this equation, $t_0$ is a relaxation time scale, and $b = \Delta\mu / I_0$ is a material constant. 
Following Dunatunga and Kamrin(2022), we employ an operator-splitting scheme via an intermediate variable $g^*$:
 
$\frac{g^* - g^n}{\Delta t} = \frac{1}{t_0} A^2 d^2 \nabla^2 g^*_{i,j},$

$\frac{g^{n + 1} - g^*}{\Delta t} = \frac{1}{t_0}\left(-\Delta \mu \left(\frac{\mu_s - \mu^{n+1}_\mathrm{loc}}{\mu_2 - \mu^{n+1}_\mathrm{loc}}\right)g^{n + 1} - \frac{\Delta\mu}{I_0}\sqrt{\frac{\rho_s d^2}{p^{n + 1}}}\mu^n g^n g^{n + 1}\right).$

In the diffusion step, $g^*$ is solved implicitly. In the reaction step, we use the local friction coefficient $\mu_{\mathrm{loc}}^{n+1}$, whereas \cite{Dunatunga2022} employs the global $\mu^{n+1}$.
...
event properties (i++) {
    ...
    foreach () {
      ...
      zeta_loc[] = 0.;
        ...
        if (p[] > 0. && f[] > 0.) {
            ...
            if (D2 > 0.) {
                D2 = sqrt(D2)/(2.*Delta);        //
                double sD2 = sqrt(2.)*D2;        // shear rate gammatot
                double In  = sD2*D/sqrt(p[]);    // I
                
                double mu_loc = mu_s + (mu_2 - mu_s)*In/(I0 + In);
                muIloc[] = mu_loc;
                
                gloc[]   = sD2/(mu_loc);     // g_loc = gammatot/μ_loc
    ...
    if (t >= t_switch) {
        ...
        // 3.1 solve： g* - dt ∇·(ξ² ∇g*) = g^n
        mgstats mg_g = diffusion (gnonloc, dt/t0_g, alphaf);
        if (mg_g.i > 0 && mg_g.resa > 0.)
            fprintf (stderr,
                     "#   gnonloc diffusion: i=%d resa=%g resb=%g\n",
        ...
        foreach() {
            if (p[] > 0. && f[] > 0.) {
                ...
                if (D2 > 0.) {
                    ...
                    double mu_new  = muIloc[];    // μ^{n+1}
                    double mu_old  = muIold[];   // μ^n
                    double g_old  = gold[];     // g^n
                    double gstar   = gnonloc[];   // g*
                    
                    // splitting scheme α^{n+1} = Δμ (μ_s - μ^{n+1})/(μ_2 - μ^{n+1})
                    double alpha = (mu_2-mu_s)*(mu_s - mu_new)/
                    (mu_2 - mu_new);
                    
                    // β^{n+1} = (Δμ/I0) sqrt(ρ_s d^2 / p^{n+1}) μ^n g^n
                    double beta = ((mu_2-mu_s)/I0)*
                        sqrt(rhos*D*D/p[])*mu_old*g_old;
                    
                    double denom = 1. + (dt/t0_g)*(alpha + beta);
                    double gnew = gstar/denom;
                    gnonloc[] = gnew;          //  g^{n+1}
                    double mu_eff = sD2/(gnew);
                    muI[] = mu_eff;
...
/* output including gnonloc for diagnostics */
...
# to run
 ...
 ~~~bash
 qcc  -g -O2 -o granular_bottom_dynamicNGF granular_bottom_dynamicNGF.c -lm
 ./granular_bottom_dynamicNGF > vol
 ~~~
 ...
 * Kamrin, K. and Henann, D. L. (2015). Nonlocal modeling of granular flows down inclines. Soft Matter,
 11:179–185.
 
 * Dunatunga, S. and Kamrin, K. (2022). Modelling silo clogging with non-local granular rheology. J. Fluid Mech., 940:A14.
