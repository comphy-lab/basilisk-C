...
# Flow in a Sandglass / Hourglass / Silo with a bottom orifice
 We propose an implementation of the linearised non-local granular fluidity(NGF) model for the flow in a silo with a bottom orifice.
...
/**
 ...
 We introduce a simplified variant of the NGF framework (Linearised NGF).
 
 $\eta_{\text{eff}} = \frac{p}{g} = \frac{p}{g_{\text{loc}} + \xi^2 \nabla^2 g} \approx \eta_{\text{loc}} \left( 1 - \xi^2 \frac{\nabla^2 g}{g_{\text{loc}}} \right),$
 
 where $\eta_{\text{loc}} = p / g_{\text{loc}}$ is the local viscosity, and the cooperativity length retains its full state-dependent form:
...
event properties (i++) {
    ...
    foreach() {
        if (p[] > 0. && f[] > 0.) {
            gloc_lap[] = (gfield[1,0] + gfield[-1,0] + gfield[0,1] + gfield[0,-1]- 4.*gfield[])/sq(Delta);
            double D2 = 0.;
            foreach_dimension() {
                double dxx = u.x[1,0] - u.x[-1,0];
                double dxy = (u.x[0,1] - u.x[0,-1] + u.y[1,0] - u.y[-1,0])/2.;
            ...
            if (D2 > 0.) {
                ...
                double Delta_mu = mu2 - mus;
                double mu_loc = mus + Delta_mu * In / (I0 + In);
                double xinew = fabs(mu2 - mu_loc)/(Delta_mu * (fabs(mu_loc - mus)));
                double eta1= (p[]/gfield[])*(1-sq(zetaA)*sq(D)*xinew*gloc_lap[]/gfield[]);
...
# to run
 ...
 ~~~bash
 qcc  -g -O2 -o granular_bottom_linearisedNGF granular_bottom_linearisedNGF.c -lm
 ./granular_bottom_linearisedNGF > vol
 ~~~
