...
# Flow in a Sandglass / Hourglass / Silo with a bottom orifice
 We propose an implementation of the constant non-local granular fluidity (NGF) model for the flow in a silo with a bottom orifice.
...
/**
 ...
 we consider a simplified variant in which the state-dependent cooperativity length is replaced by a constant value, while the full Helmholtz-type equation for the fluidity.
...
# to run
 ...
 ~~~bash
 qcc  -g -O2 -o granular_bottom_constantNGF granular_bottom_constantNGF.c -lm
 ./granular_bottom_constantNGF > vol
 ~~~
