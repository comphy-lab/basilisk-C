...
#define rhom(f1, f2, f3) max(rho(f1, f2, f3), min(rho1, min(rho2, rho3)))
#endif

#ifndef mu
#define mu(f1, f2, f3) (clamp(f1, 0., 1.) * mu1 + clamp(f2, 0., 1.) * mu2 + clamp(f3, 0., 1.) * mu3)
//We define also rhoM to avoid zero value for mu
#define mum(f1, f2, f3) max(mu(f1, f2, f3), min(mu1, min(mu2, mu3)))
#endif
...
#if dimension <= 2
  foreach()
    sf1[] = (4.*f1[] + 2.*(f1[0,1] + f1[0,-1] + f1[1,0] + f1[-1,0]) +
