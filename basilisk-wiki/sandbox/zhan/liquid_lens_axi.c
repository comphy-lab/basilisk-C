...
![The three phase system can be simulated with three tracers of VOF method.](liquid_lens_axi/movie.mp4)
...
event init (t = 0){
  if(!restore("dumpfile")){
    refine(sq(x - L/2.0) + sq(y) + sq(z) < sq(1.1 * R) && sq(x - L/2.0) + sq(y) + sq(z) > sq(0.9 * R) && level < MAXLEVEL);
    refine((x < 1.1* L/2.0 && x > 0.9 * L/2.0 && y > R) && level < MAXLEVEL);
    foreach_vertex()
      phi[] = sq(R) - ((sq(x-L/2.) + sq(y) + sq(z)));
    boundary({phi});
    restriction({phi});
    fractions(phi, f3);
    foreach(){
...
event logfile (t=end){
  coord p1;
  double d1 = locate_triple_point(&p1, f1, f2, f3, 0., L, 0., L, 1.1);
  output_cells(stdout);
