...
# Speical norm for three phase simulation.
 
## Some useful functions for locating the triple point 
*/
...
## Triple point center positioning function
  ...
  Now we can write the triple point locating function, this function will return the position of the triple point $\mathbf{p} = (p_x,p_y)$ and corresponding diameter $D$ in a domain $\left[x_{min}, x_{max}\right]\times \left[y_{min}, y_{max}\right]$.

### Process to locate the center of the triple point

  * Locate all the intersections of $f_1$ and $f_2$, $f_2$ and $f_3$, as well as $f_3$ and $f_1$. Then we have three groups of intersections.
...
double locate_triple_point (coord * p, scalar f1, scalar f2, scalar f3, double xmin,
...
## Fractions error
 ...
 we define an error which is more numerical, which describes the dissatisfaction of the condition $\sum_i^3 f_i = 1$, which can be expressed as

 $$
\epsilon_f = \frac{\left|\sum_i^3 f_i - 1\right|}{V_0}
 $$
 */
...
$$
L_1(\mathrm{shape}) = \frac{\sum_i\left(|f_1-f_1^{\mathrm{exact}}|+|f_2-f_2^{\mathrm{exact}}|+|f_3-f_3^{\mathrm{exact}}|\right)}{V_0}
$$
