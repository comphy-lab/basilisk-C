...
// [nothing new]
static inline double embed_geometry(Point point, coord *b, coord *n)
{
...
// [nothing new]
static inline double embed_area_center(Point point, double *x1, double *y1, double *z1)
{
   double area = 0.;
   if (cs[] > 0. && cs[] < 1.)
   {
...
// [nothing new]
trace int fractions_cleanup(scalar c, face vector s,
...

{
   ...
   for (int i = 0; i < 100 && changed; i++)
   {
      ...
      foreach_face() if (s.x[] && ((!c[] || !c[-1]) || s.x[] < smin))
          s.x[] = 0.;
      ...
      * or one face is solid, and in this case we set all the other
      ...
      foreach_face() if (s.x[] > 0 && (c[-1] == 1 || c[] == 1))
          s.x[] = 1.;
...
// [nothing new]
macro2 double dirichlet(double expr, Point point = point,
...
// [nothing new]
macro2 double dirichlet_homogeneous(double expr, Point point = point,
                                    scalar s = _s, bool *data = data)
{
...
// [nothing new]
macro2 double neumann(double expr, Point point = point,
                      scalar s = _s, bool *data = data)
{
   return data ? embed_area_center(point, &x, &y, &z),
...
// [nothing new]
macro2 double neumann_homogeneous(double expr, Point point = point,
                                  scalar s = _s, bool *data = data)
{
...
// [nothing new]
foreach_dimension() static inline double embed_face_gradient_x(Point point, scalar a, int i)
{
   ...
   if (face_condition(fs, cs, emerged, csm1))
      return ((1. + fs.x[i]) * (a[i] - a[i - 1]) +
              (1. - fs.x[i]) * (a[i, j] - a[i - 1, j])) /
             (2. * Delta);
   return (a[i] - a[i - 1]) / Delta;
}

// [nothing new]
foreach_dimension() static inline double embed_face_value_x(Point point, scalar a, int i)
{
   assert(cs[i] && cs[i - 1] && (emerged || (csm1[i] && csm1[i - 1])));
   int j = sign(fs.x[i, 1] - fs.x[i, -1]);
   return face_condition(fs, cs, emerged, csm1) ? ((1. + fs.x[i]) * cs_avg(a, i, 0, 0) + (1. - fs.x[i]) * cs_avg(a, i, j, 0)) / 2. : cs_avg(a, i, 0, 0);
}
...
#else // dimension == 3
// [nothing new]
foreach_dimension() static inline coord embed_face_barycentre_z(Point point, int i)
{
...
foreach_dimension() static inline double embed_face_gradient_x(Point point, scalar a, int i)
{
   ...
   if (face_condition(fs, cs, emerged, csm1))
   {
      ...
      return (((a[i, 0, 0] - a[i - 1, 0, 0]) * (1. - p.y) +
               (a[i, j, 0] - a[i - 1, j, 0]) * p.y) *
                  (1. - p.z) +
              ((a[i, 0, k] - a[i - 1, 0, k]) * (1. - p.y) +
               (a[i, j, k] - a[i - 1, j, k]) * p.y) *
                  p.z) /
             Delta;
   }
   return (a[i] - a[i - 1]) / Delta;
...
#define face_gradient_x(a, i) \
   (a.third && fs.x[i] < 1. && fs.x[i] > 0. ? embed_face_gradient_x(point, a, i) : (a[i] - a[i - 1]) / Delta)

#undef face_gradient_y
#define face_gradient_y(a, i) \
   (a.third && fs.y[0, i] < 1. && fs.y[0, i] > 0. ? embed_face_gradient_y(point, a, i) : (a[0, i] - a[0, i - 1]) / Delta)

#undef face_gradient_z
#define face_gradient_z(a, i) \
   (a.third && fs.z[0, 0, i] < 1. && fs.z[0, 0, i] > 0. ? embed_face_gradient_z(point, a, i) : (a[0, 0, i] - a[0, 0, i - 1]) / Delta)

#undef face_value
#define face_value(a, i) \
   (a.third && fs.x[i] < 1. && fs.x[i] > 0. ? embed_face_value_x(point, a, i) : cs_avg(a, i, 0, 0))
...
#define center_gradient(a) (fs.x[] && fs.x[1] ? (a[1] - a[-1]) / (2. * Delta) : fs.x[1] ? (a[1] - a[]) / Delta  \
                                                                            : fs.x[]    ? (a[] - a[-1]) / Delta \
                                                                                        : 0.)
...
foreach_dimension() double embed_face_avg_gradient_t1_x(Point point, scalar a, int i)
{
   double up = nodata, down = nodata;
   if (cs[0, i] && (emerged || csm1[0, i]))
      up = (fs.x[0, i] && fs.x[1, i] &&
                    cs[1, i] && cs[-1, i] &&
                    (emerged || (csm1[1, i] && csm1[-1, i]))
                ? (a[1, i] - a[-1, i]) / (2. * Delta)
            : fs.x[1, i] && fs.x[2, i] &&
                    cs[1, i] && cs[2, i] &&
                    (emerged || (csm1[1, i] && csm1[2, i]))
                ? (-a[2, i] + 4. * a[1, i] - 3. * a[0, i]) / (2. * Delta)
            : fs.x[0, i] && fs.x[-1, i] &&
                    cs[-1, i] && cs[-2, i] &&
                    (emerged || (csm1[-1, i] && csm1[-2, i]))
                ? (a[-2, i] - 4. * a[-1, i] + 3. * a[0, i]) / (2. * Delta)
   ...
   if (cs[0, i - 1] && (emerged || csm1[0, i - 1]))
      down = (fs.x[0, i - 1] && fs.x[1, i - 1] &&
                      cs[1, i - 1] && cs[-1, i - 1] &&
                      (emerged || (csm1[1, i - 1] && csm1[-1, i - 1]))
                  ? (a[1, i - 1] - a[-1, i - 1]) / (2. * Delta)
              : fs.x[1, i - 1] && fs.x[2, i - 1] &&
                      cs[1, i - 1] && cs[2, i - 1] &&
                      (emerged || (csm1[1, i - 1] && csm1[2, i - 1]))
                  ? (-a[2, i - 1] + 4. * a[1, i - 1] - 3. * a[0, i - 1]) / (2. * Delta)
              ...
              : fs.x[1, i - 1] && cs[1, i - 1] &&
                      (emerged || csm1[1, i - 1])
                  ? (a[1, i - 1] - a[0, i - 1]) / Delta
              : fs.x[0, i - 1] && cs[-1, i - 1] &&
                      (emerged || csm1[-1, i - 1])
                  ? (a[0, i - 1] - a[-1, i - 1]) / Delta
                  : nodata);
   return (up == nodata && down == nodata ? 0. : up == nodata ? down
                                             : down == nodata ? up
                                             : fs.y[0, i]     ? (down + up) / 2.
                                                              : 0.);
}

foreach_dimension() double embed_face_avg_gradient_t2_x(Point point, scalar a, int i)
{
   double up = nodata, down = nodata;
   if (cs[0, 0, i] && (emerged || csm1[0, 0, i]))
      up = (fs.x[0, 0, i] && fs.x[1, 0, i] &&
                    cs[1, 0, i] && cs[-1, 0, i] &&
                    (emerged || (csm1[1, 0, i] && csm1[-1, 0, i]))
                ? (a[1, 0, i] - a[-1, 0, i]) / (2. * Delta)
            : fs.x[1, 0, i] && fs.x[2, 0, i] &&
                    cs[1, 0, i] && cs[2, 0, i] &&
                    (emerged || (csm1[1, 0, i] && csm1[2, 0, i]))
                ? (-a[2, 0, i] + 4. * a[1, 0, i] - 3. * a[0, 0, i]) / (2. * Delta)
            ...
            : fs.x[1, 0, i] && cs[1, 0, i] &&
                    (emerged || csm1[1, 0, i])
                ? (a[1, 0, i] - a[0, 0, i]) / Delta
            : fs.x[0, 0, i] && cs[-1, 0, i] &&
                    (emerged || csm1[-1, 0, i])
                ? (a[0, 0, i] - a[-1, 0, i]) / Delta
                : nodata);
   if (cs[0, 0, i - 1] && (emerged || csm1[0, 0, i - 1]))
      down = (fs.x[0, 0, i - 1] && fs.x[1, 0, i - 1] &&
                      cs[1, 0, i - 1] && cs[-1, 0, i - 1] &&
                      (emerged || (csm1[1, 0, i - 1] && csm1[-1, 0, i - 1]))
                  ? (a[1, 0, i - 1] - a[-1, 0, i - 1]) / (2. * Delta)
              : fs.x[1, 0, i - 1] && fs.x[2, 0, i - 1] &&
                      cs[1, 0, i - 1] && cs[2, 0, i - 1] &&
                      (emerged || (csm1[1, 0, i - 1] && csm1[2, 0, i - 1]))
                  ? (-a[2, 0, i - 1] + 4. * a[1, 0, i - 1] - 3. * a[0, 0, i - 1]) / (2. * Delta)
              : fs.x[0, 0, i - 1] && fs.x[-1, 0, i - 1] &&
                      cs[-1, 0, i - 1] && cs[-2, 0, i - 1] &&
                      (emerged || (csm1[-1, 0, i - 1] && csm1[-2, 0, i - 1]))
                  ? (a[-2, 0, i - 1] - 4. * a[-1, 0, i - 1] + 3. * a[0, 0, i - 1]) / (2. * Delta)
              : fs.x[1, 0, i - 1] && cs[1, 0, i - 1] &&
                      (emerged || csm1[1, 0, i - 1])
                  ? (a[1, 0, i - 1] - a[0, 0, i - 1]) / Delta
              : fs.x[0, 0, i - 1] && cs[-1, 0, i - 1] &&
                      (emerged || csm1[-1, 0, i - 1])
                  ? (a[0, 0, i - 1] - a[-1, 0, i - 1]) / Delta
                  : nodata);
   return (up == nodata && down == nodata ? 0. : up == nodata ? down
                                             : down == nodata ? up
                                             : fs.z[0, 0, i]  ? (down + up) / 2.
                                                              : 0.);
}
...
// [new!!!]
foreach_dimension() static inline void embed_evaluate_x(Point point, scalar s, scalar cs,
                                                        coord n, coord b,
                                                        double *d0, double *v0,
                                                        double *d1, double *v1)
{
   ...
   bool defined = true;
   foreach_dimension() if (defined && !fs.x[(n.x > 0.)])
       defined = false;
                       ...
                       coord *Fmu, coord *Fval)
{
   ...
   foreach_dimension()
   {
      Fmu->x = -area * mua * (dsdn.x * (sq(n.x) + 1.) + (dsdn.y + val.y * s.y[]) * n.x * n.y) / Delta;
      Fval->x = -area * mua * (val.x * (sq(n.x) + 1.)) / Delta;
   }
#else  // dimension == 3
   foreach_dimension()
   {
      Fmu->x = -area * mua * (dsdn.x * (sq(n.x) + 1.) + (dsdn.y + val.y * s.y[]) * n.x * n.y + (dsdn.z + val.z * s.z[]) * n.x * n.z) / Delta;
      Fval->x = -area * mua * (val.x * (sq(n.x) + 1.)) / Delta;
   }
#endif // dimension
...
static inline coord embed_gradient(Point point, vector u, coord b, coord n)
{
   coord dudn;
   foreach_dimension()
   {
...
trace void embed_force(scalar p, vector u, face vector mu, coord *Fp, coord *Fmu)
{
   coord Fps = {0}, Fmus = {0};
...
trace void embed_torque(scalar p, vector u, face vector mu, coord c, coord *Tp, coord *Tmu)
{
   coord Tps = {0}, Tmus = {0};
...
trace void update_tracer(scalar f, face vector uf, face vector flux, double dt)
{
   /**
   ...
   foreach (nowarning)
   {
      ...
      else if (cs[] >= 1.)
      {
         ...
         *fb* based on neighboring values. */
         ...
         int sn = 0;
         double fb = 0.;
         foreach_neighbor(1) if (cs[] > 0. && cs[] < 1.)
         {
            sn += 1;
            coord b, n;
   ...
   */
   ...
   foreach (nowarning)
   {
      ...
      else
      {
         ...
         for (int i = 0; i <= 1; i++)
            foreach_dimension() if (fabs(uf.x[i]) > umax)
                umax = fabs(uf.x[i]);
