...
extern double dt;
extern const double V_PRESS;
extern scalar p0_col, p1_col;
extern scalar csm1;
...
foreach_dimension() static double vof_concentration_gradient_x(Point point, scalar c, scalar t)
{
...
foreach_dimension() static void sweep_x(scalar c, scalar cc, scalar *tcl, int dim)
{
   vector n[];
   scalar alpha[], flux[];
   double cfl = 0.;
   ...
   foreach_face(x, reduction(max : cfl))
   {
      ...
      double cf = (c[i] <= 0. || c[i] >= 1.) ? c[i] : rectangle_fraction((coord){-s * n.x[i], n.y[i], n.z[i]}, alpha[i], (coord){-0.5, -0.5, -0.5}, (coord){s * un - 0.5, 0.5, 0.5});
      ...
      flux[] = cf * uf.x[];
      // if(dim==1 && p0_col[]>0 && p0_col[]<1) flux[]=uf.x[];
   ...
   // if (dim == 1) fprintf(stderr, "uf.y 계면셀 하단면속도: ");
   foreach ()
      if (cs[] > 0.)
      {
         if (dim == 1 && p0_col[] > 0 && p0_col[] < 1. && y > 0.)
         {
            if (c[] > 1e-9)
            {
               // c[] += dt * cs[] * (flux[]) / (cm[] * Delta);
               // c[] += (0.99 * V_PRESS * dt / (cm[] * Delta));
               // c[] += dt * cs[] * (flux[] + (V_PRESS - uf.x[])) / (cm[] * Delta);
               
               // c[] += dt * cs[] * (flux[] - c[]*V_PRESS +(V_PRESS - uf.x[])) / (cm[] * Delta);
               // c[] += (V_PRESS * dt * flux[] / (cs[] * Delta));
               // c[]=1;
               // c[]=cm[];
               // c[]=csm1[];
               // c[] = cs[] + (0.99 * V_PRESS * dt / Delta) - 1e-9;
               // else c[]+= (V_PRESS * dt)
               // if(c[]!=cs[]) c[] += dt * cs[] * (flux[] - flux[1] + cc[] * (uf.x[1] - uf.x[])) / (cm[] * Delta);
               // else c[] += (0.99 * V_PRESS * dt / (cm[] * Delta));
               // c[] += dt * cs[] * (flux[] - flux[1]) / (cm[] * Delta);

               if(fabs(cs[]-c[])>0.1) c[] += dt * cs[] * (flux[] - flux[1] + cc[] * (uf.x[1] - uf.x[])) / (cm[] * Delta);
               else c[]=cs[];
            }
            else
               c[] = 0.;
...
void vof_advection(scalar *interfaces, int i)
{
   for (scalar c in interfaces)
   {
      ...
      void (*sweep[dimension])(scalar, scalar, scalar *, int);
      int d = 0;
      foreach_dimension()
          sweep[d++] = sweep_x;
      ...
      foreach()
      {
         ...
         {
            min_cs = fmin(min_cs, Delta * cs[]);
            max_cs = fmax(max_cs, Delta * cs[]);
            if(c[]) min_c = fmin(min_c,c[]);
         }
      }
      global_min_c=fmin(min_c,global_min_c);

      fprintf(stderr, "[VOF] min_c=%.12f global_min_c=%.12f\n", min_c, global_min_c);
      fflush(stderr);
   }
}
...
// {
//    bool changed = false;
//    double threshold = 0.0013;
//    foreach ()
//    {
//       if (p0_col[] > 0 && p0_col[] < 1)
//       {
//          if (Delta * cs[] < threshold)
//          {
//             cs[] = 0;
//             changed = true;
//          }
