...
foreach_dimension() static inline double velocity_noslip_x(Point point,
...
foreach_dimension() static inline double pressure_acceleration_x(Point point,
                                                                 coord dup, coord dwp,
                                                                 coord wp, coord pp,
                                                                 double xc, double yc, double zc)
{
...
{
  FILE *fp = p.fp;
  char def[] = "p_dump", *file = p.file ? p.file : p.fp ? NULL
                                                        : def;
    ...
    foreach (nowarning)
    {
      if (csm1[] <= 0. && cs[] > 0.)
      {
        ...
        else
        {
          int navg = 0;
          double savg = 0.;
          foreach_neighbor(1) if (cs[] > 0. && (emerged || csm1[] > 0.))
          {
            navg += 1;
            savg += s[];
          }
