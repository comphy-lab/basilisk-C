/**
# A two layer solver with the Green-Naghdi equations implemented on the top layer
...
static double update_green_naghdi_twolayer (scalar * current, scalar * updates,
				   double dtmax);

event defaults (i = 0)
  update = update_green_naghdi_twolayer;
...
$$
\begin{array}{lll}
  \mathcal{R}_1 \left[ h, h_s, z_b \right] w & = & - \frac{1}{3 h} \nabla \left(
  h^3 w \right) - \frac{h}{2} w \nabla \left ( z_b + h_s \right )\\
  & = & - h \left[ \frac{h^{}}{3} \nabla w + w \left( \nabla h + \frac{1}{2}
  \nabla \left ( z_b + h_s \right ) \right)\right]\\
  \mathcal{R}_2 \left[ h, h_s, z_b \right] w & = & \frac{1}{2 h} \nabla \left( h^2
  w \right) + w \nabla \left ( z_b + h_s \right )\\
  & = & \frac{h}{2} \nabla w + w \nabla \left( z_b + h_s + h \right)
\end{array}
$$ */

#define R1(h,zb,hs,w) (-h[]*(h[]/3.*dx(w) + w[]*(dx(h) + (dx(zb)+dx(hs))/2.)))//Changed here
#define R2(h,zb,hs,w) (h[]/2.*dx(w) + w[]*((dx(zb)+dx(hs)) + dx(h)))//Changed here
...
static double residual_GN (scalar * a, scalar * r, scalar * resl, void * data)
{
  ...
  scalar h = list[0], zb = list[1], wet = list[2], hs = list[3]; //Changed here
  vector D = vector(a[0]), b = vector(r[0]), res = vector(resl[0]);

  /**
  The general form for $\mathcal{T}$ is
  $$
  \mathcal{T} \left[ h, h_s, z_b \right] w = \mathcal{R}_1 \left[ h, h_s, z_b
  \right]  \left( \nabla \cdot w \right) + \mathcal{R}_2 \left[ h, h_s, z_b
  \right]  \left( \nabla \left ( z_b + h_s \right ) \cdot w \right)
  $$
  ...
  \mathcal{R}_2 \left( \nabla \left ( z_b + h_s \right ) \cdot D \right) + hD = b
  $$
  $$
  - \alpha_d h^2  \left[ \frac{h}{3} \nabla \left( \nabla \cdot D \right)
  + \left( \nabla \cdot D \right)  \left( \nabla h + \frac{1}{2} \nabla \left ( z_b + h_s \right )
  \right)\right] + \alpha_d h \left[ \frac{h}{2} \nabla \left( \nabla \left ( z_b + h_s \right ) \cdot D
  \right) + \left( \nabla \left ( z_b + h_s \right ) \cdot D \right) \nabla \eta \right] + hD = b
  $$
  Expanding the operators for the $x$-component gives
  $$
  \begin{aligned}
  - \frac{\alpha_d}{3} \partial_x \left( h^3 \partial_x D_x \right) + h \left[
  \alpha_d \left( \partial_x \eta \partial_x \left ( z_b + h_s \right ) + \frac{h}{2} \partial^2_x \left ( z_b + h_s \right )
  \right) + 1 \right] D_x + & \\
  \alpha_d h \left[ \left( \frac{h}{2} \partial^2_{xy} \left ( z_b + h_s \right ) + \partial_x \eta
  \partial_y \left ( z_b + h_s \right ) \right) D_y + \frac{h}{2} \partial_y \left ( z_b + h_s \right ) \partial_x D_y -
  \frac{h^2}{3} \partial^2_{xy} D_y - h \partial_y D_y  \left( \partial_x h +
  \frac{1}{2} \partial_x \left ( z_b + h_s \right ) \right) \right] & = b_x
  \end{aligned}
  $$
  ...
  foreach (reduction(max:maxres))
	...
	double hc = h[], dxh = dx(h), dxzb = dx(zb), dxhs = dx(hs), dxeta = dxzb + dxh + dxhs; //Changed here
	...
	res.x[] = b.x[] -
	  (-alpha_d/3.*(hr3*D.x[1] + hl3*D.x[-1] - 
	   ...
	   hc*(alpha_d*(dxeta*(dxzb+dxhs) + hc/2.*(d2x(zb)+d2x(hs))) + 1.)*D.x[] +//Changed here
	   alpha_d*hc*((hc/2.*(d2xy(zb)+d2xy(hs)) + dxeta*(dy(zb)+dy(hs)))*D.y[] + //Changed here
		       hc/2.*(dy(zb)+dy(hs))*dx(D.y) - sq(hc)/3.*d2xy(D.y)//Changed here
		       - hc*dy(D.y)*(dxh + (dxzb+dxhs)/2.)));//Changed here
  ...
  foreach_level_or_leaf (l)
	...
	D.x[] = (b.x[] -
		 (-alpha_d/3.*(hr3*D.x[1] + hl3*D.x[-1])/sq(Delta) +
		  alpha_d*hc*((hc/2.*(d2xy(zb)+d2xy(hs)) + dxeta*(dy(zb)+dy(hs)))*D.y[] + //Changed here
			      hc/2.*(dy(zb)+dy(hs))*dx(D.y) - sq(hc)/3.*d2xy(D.y)//Changed here
			      - hc*dy(D.y)*(dxh + (dxzb+dxhs)/2.))))/
	  (alpha_d*(hr3 + hl3)/(3.*sq(Delta)) + 
	   hc*(alpha_d*(dxeta*(dxzb+dxhs) + hc/2.*(d2x(zb)+d2x(hs))) + 1.));//Changed here
      }
      else
	D.x[] = 0.;
    }
...
static double update_green_naghdi_twolayer (scalar * current, scalar * updates,
				   double dtmax)
{
  double dt = update_two_layer (current, updates, dtmax);
  scalar h = current[0];
  scalar hs = current[1];
  vector u = vector(current[2]);

  /**
  We first compute the right-hand-side $b$. The general form for the
  $\mathcal{Q}_1$ operator is (eq. (9) of Bonneton et al, 2011).
  $$
  \mathcal{Q}_1 \left[ h, h_s, z_b \right] \left( u \right) = - 2 \mathcal{R}_1 
  ...
  & = u_x  \left( u_x \partial_x^2 \left( z_b + h_s \right) + u_y \partial_{xy}^2 \left ( z_b + h_s \right ) \right) +
  u_y  \left( u_y \partial_y^2 \left ( z_b + h_s \right ) + u_x \partial_{xy}^2 \left ( z_b + h_s \right ) \right)\\
  & = u^2_x \partial_x^2 \left ( z_b + h_s \right ) + u^2_y \partial_y^2 \left ( z_b + h_s \right ) + 2 u_x u_y
  \partial_{xy}^2 \left ( z_b + h_s \right )
  \end{aligned}
  $$
  ...
  vector b[];
  {
    ...
    foreach() {
      ...
      d[] = sq(u.x[])*(d2x(zb)+d2x(hs)) + sq(u.y[])*(d2y(zb)+d2y(hs)) + 2.*u.x[]*u.y[]*(d2xy(zb)+d2xy(hs));//Changed here
    }
    boundary ({c,d});
    ...
    foreach()
      foreach_dimension()
      b.x[] = h[]*(G/alpha_d*dx(eta) - 2.*R1(h,zb,hs,c)  + R2(h,zb,hs,d));
  }
  ...
  scalar * list = {h, zb, wet, hs}; // should it be zb? or zb+hs?
  restriction (list);
