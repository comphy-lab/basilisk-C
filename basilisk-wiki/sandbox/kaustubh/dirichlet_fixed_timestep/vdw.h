...
In addition we will need the centered Laplacian $\nabla^{2,c} = \partial_x^+\partial_x^-  + \partial_y^+\partial_y^-$ that is defined and written as in [here](/Basilisk C#stencils).
...
event stability (i++,last) {
  ...
  foreach(reduction(min:mydtmax)){
    if ( lambda != 0.) {
// #if dimension == 2
//       double dt = rho[]*Delta/(3*mu[]);
// #else
      double dt = 0.5*sq(Delta)/lambda;
// #endif
...
{
/**
To force symmetry in the maccormack scheme
*/
  int j = i%2;
...
We pre-calculate some terms that will be needed hereafter:

$$
\dfrac{\nabla \rho}{||\nabla \rho||}
$$

and the normal to the interface. Here we set the order parameter to be $\rho$.
*/
...
/** Predictor*/
  ...
  foreach(){
    // laprho[] = 0.;
    laprho[] = rho[1] + rho[-1] + rho[0,1] + rho[0,-1] - 4*rho[]; 

    rhop[] = rho[];
    foreach_dimension(){
      // laprho[] += uCenLap_x(point,rho);

      rhop[]   -= (dt/Delta)*(q.x[j]  - q.x[j-1,0]);
      u.x[]     = q.x[]/rho[];
...
*/
#if naught
- (2/3)*(  mu.x[ 0,0] * (2*(u.x[1,0]-u.x[ 0,0])/Delta - 0.5*(u.y[ 0,1]-u.y[ 0,-1]))/Delta 
         - mu.x[-1,0] * (2*(u.x[0,0]-u.x[-1,0])/Delta - 0.5*(u.y[-1,1]-u.y[-1,-1]))/Delta 
	 )/Delta  
-        (  mu.x[ 0,0] * ( (u.x[0,1] - u.x[0, 0])/Delta + 0.5*(u.y[1, 0] - u.y[-1, 0])/Delta ) 
          - mu.x[0,-1] * ( (u.x[0,0] - u.x[0,-1])/Delta + 0.5*(u.y[1,-1] - u.y[-1,-1])/Delta )
	 )/Delta   
#endif
/**
$$
...
/**
  ...
  foreach()
    foreach_dimension()
      qp.x[] = q.x[] - (dt/Delta)*
  (                          rho[ j,0]*sq(u.x[ j,0]) + P0(rho[ j,0]) 
                           - rho[j-1,0]*sq(u.x[j-1,0]) - P0(rho[j-1,0]) 
                           + rho[0,j]*u.x[0,j]*u.y[0,j] - rho[0,j-1]*u.x[0,j-1]*u.y[0,j-1] 
                           
      + 0.5*lambda*( sq(rho[1,0]-rho[]    ) - 0.25*sq(rho[j,1] -rho[j,-1] ) - 2*rho[ j,0]*laprho[j] 
                    - sq(rho[0,0]-rho[-1,0]) + 0.25*sq(rho[j-1,1]-rho[j-1,-1]) +2*rho[j-1,0]*laprho[j-1,0] 
	             + (rho[0,1]-rho[0, 0]) * (rho[1, j]-rho[-1, j]) 
                     - (rho[0,0]-rho[0,-1]) * (rho[1,j-1]-rho[-1,j-1]) 
      )/Delta
  );
  boundary(( scalar *) {qp});
/** ### Corrector step 
...
*/
  ...
  foreach(){
    // laprho[] = 0.;
    laprho[] = rhop[1] + rhop[-1] + rhop[0,1] + rhop[0,-1] - 4*rhop[]; 
    rho[] = 0.5*(rhop[]+rho[]);
    foreach_dimension(){
      // laprho[] += uCenLap_x(point,rhop);
/**
Corrector for density 
$$
\rho^{n+1} = \frac 12 [ \rho^n + \rho^* ]  - \frac \tau 2 [ \partial_x^+ (\rho^* u^*_x) + \partial_y^+ ( \rho^* u^*_y )]
$$
*/
       rho[] -= 0.5*(dt/Delta)*(qp.x[1-j] - qp.x[-j]);
       u.x[] = qp.x[]/rhop[]; // CFL condition for this second velocity field ? 
     }
   }
   boundary({laprho,rho});
...
/**
 ...
 $$
q_x^{n+1} = \frac 12 (q_x^* + q_x^n )  - \frac \tau 2  \{  \partial_x^+ (\rho u_x^{* 2} + p_0^* ) + \partial_y^+ (\rho u_x^* u_y^*)
$$
$$
-  (2/3) \partial_x^+ [\mu( 2 \partial_x^- u_x^* - \partial_y^c u_y^* )] -  \partial_y^+ [\mu( \partial_y^- u_x^* + \partial_x^c u_y^* )]
$$
$$
+ (\lambda/2) [ \partial_x^+ ( (\partial_x^- \rho^*)^2  -  (\partial_y^c \rho^*)^2 - 2\rho^* \nabla^{2,c} \rho^* ) 
+ 2 \partial_y^+ (\partial_y^- \rho \partial_x^c \rho^*)] \}
$$
...
*/
  ...
  (                          rhop[1-j,0]*sq(u.x[1-j,0]) + P0(rhop[1-j,0]) 
                           - rhop[-j,0]*sq(u.x[-j,0]) - P0(rhop[-j,0]) 
			   + rhop[0,1-j]*u.x[0,1-j]*u.y[0,1-j]
                           - rhop[0,-j]*u.x[0,-j]*u.y[0,-j] 
			   
      + 0.5*lambda*( sq(rhop[1,0]-rhop[]    ) - 0.25*sq(rhop[1-j,1] -rhop[1-j,-1] ) - 2*rhop[1-j,0]*laprho[1-j,0] 
                   -sq(rhop[0,0]-rhop[-1,0]) + 0.25*sq(rhop[-j,1]-rhop[-j,-1]) + 2*rhop[-j,0]*laprho[-j,0] 
	           +  (rhop[0,1]-rhop[]    ) * (rhop[1,1-j]-rhop[-1,1-j]) 
             - (rhop[0,0]-rhop[0,-1])*(rhop[1,-j]-rhop[-1,-j]) 
      )/Delta
	);
  boundary(( scalar *) {q});
