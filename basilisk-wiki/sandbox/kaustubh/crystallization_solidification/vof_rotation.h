  ...
  static void sweep_x (scalar c, scalar cc, scalar * tcl)
  {
    ...
    foreach_face(x, reduction (max:cfl)) {
      ...
      double cf = (c[i] <= 0. || c[i] >= 1.) ? c[i] :
        rectangle_fraction ((coord){-s*n.x[i], 
                            n.y[i]-n.x[i]*dt*(uf.x[0,1]-uf.x[0,-1])/(2.*Delta),
                            n.z[i]},
                            alpha[i],
                            (coord){-0.5, -0.5, -0.5},
                            (coord){s*un - 0.5, 0.5, 0.5});
      double cf2 = (c[i] <= 0. || c[i] >= 1.) ? c[i] :
        rectangle_fraction ((coord){-s*n.x[i], n.y[i], n.z[i]}, alpha[i],
          (coord){-0.5, -0.5, -0.5},
          (coord){s*un - 0.5, 0.5, 0.5});
        if((n.x[i] != 0. || n.y[i] != 0.) && uf.x[] !=0.)
            fprintf(stderr, "## %g %g %d %4.2g %4.2g %4.2g %4.2g %4.2g %4.2g %5.3g %5.3g\n",
             x/Delta,y/Delta,i, n.x[i],n.y[i],n.x[],n.y[],n.x[i]*dt*(uf.x[0,1]-uf.x[0,-1])/(2.*Delta), uf.x[],cf,cf2);
