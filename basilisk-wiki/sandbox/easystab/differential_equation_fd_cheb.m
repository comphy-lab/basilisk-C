...
%}
   ...
   [dxcheb,dxxcheb,wxcheb,xcheb]=dif1D('cheb',0,L,N);
    ...
    print('-dsvg','fd_vs_cheb.svg'); % save the figure
...
%}
   ...
   [dxcheb,dxxcheb,wxcheb,xcheb]=dif1D('cheb',0,L,N);
    ...
    %{
    ![**Figure :** The results. Note that the Chebyshev disctetization results in a clustering of the point along the boundaries of the interval.  ](differential_equation_fd_cheb/fd_vs_cheb.svg)
    %}
  
%% plotting the Error     
  figure;
  plot(xfd,abs(f-(xfd.^2/2+(1-L)*xfd)),'g+',xcheb,abs(g-(xcheb.^2/2+(1-L)*xcheb)),'r+');
  set(gcf,'paperpositionmode','auto')
  print('-dsvg','Error.svg'); % save the figure
   
   %{
    ![**Figure :** Error obtained with the two methods  ](differential_equation_fd_cheb/Error.svg)
    %}
