...
(This code is adapted from [KH_temporal_viscous.m](sandbox/easystab/KH_temporal_viscous.m) from the easystab project).
...
We will consider separately symmetrical and antisymmetrical modes and use discretization of a semi-infinite domain.
...
Here we use Chebyshev discretization with stretching, considering a semi-infinite domain. 
See [differential_equation_infinitedomain.m]() to see how this works.

%}

discretization = 'chebSemiInfAlg'; % 
...
%{
![** Figure : Temporal spectrum of a gaussian jet](GaussianJet_temporal_viscous/KH_temporal_viscous_spectrum.png)
%} 
...
%{
![** Figure : Unstable eigenmmode of the tanh shear layer for k=0.5](GaussianJet_temporal_viscous/KH_temporal_viscous_mode.png)
%} 
...
if loopk
  for Re = [10 30 100 300 1000]
    alphatab = 0:.025:3;
    lambdatab = [];
    for alpha=alphatab
        [s,UU] = KH(alpha,Re,N,-1);
        smode = s(1);
        if(real(smode)<0)
            sfilter = s(-imag(s)/alpha>0.1&-imag(s)/alpha<0.9);
    ...
    xlabel('k');ylabel('\omega_i');ylim([-.25 .25]);
...
![** Figure : results for the temporal instability of a gaussian jet](GaussianJet_temporal_viscous/KH_temporal_viscous_curves.png)
...
%%
function [s,UU] = KH(alpha,Re,N,symmetry)
...
% Boundary conditions
    ...
    III=eye(3*N);
    indBC=[1,N,N+1,2*N,2*N+1];
    C=III(indBC,:);
    if(symmetry==1)
        % change condition for ux and p to Neumann
        C(1,1:N) = dy(1,:);
        C(5,1:N) = dy(1,:);
    else    
        % change condition for uy to Neumann
        C(3,N+1:2*N) = dy(1,:); 
    end
    A(indBC,:)=C;
    E(indBC,:)=0;  
