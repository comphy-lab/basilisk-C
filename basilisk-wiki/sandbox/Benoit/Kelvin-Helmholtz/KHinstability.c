...
#define rho_2 (1e0)
#define L0 (2.)
#define MAXTIME 1.  //Simulation time
...
/*  L0 = 1.;*/
  ...
  NITERMAX = 20;
  TOLERANCE = 1e-12; // We lower the minimum residu that needs to be reached in the poisson solver
/*  CFL = 0.01;*/
...
/*    diver += u.y[1] - u.y[];
    divergence[] += diver/(dt*Delta);
*/
    divergence[] =  (u.y[0, 1] - u.y[0, -1])/(2*Delta) + (u.x[1, 0] - u.x[-1, 0])/(2*Delta);
  }
...
event compute_divergence (i++)
{

  foreach(){
    divergence[] =  fabs((u.y[0, 1] - u.y[0, -1])/(2*Delta) + (u.x[1, 0] - u.x[-1, 0])/(2*Delta));
  }
  boundary ({divergence});
}
...
event measurements (t += 0.001)
{
    ...
    stats func1 = statsf(divergence);
    double maxi  = func1.max;
    double mini  = func1.min;
    norm func3 = normf(divergence);
    ...
    fprintf (stderr,"%g %g %g %g %g %g %g %g %g %d %g %g\n", t, vmax,thgrowth,maxi,mini,rmsi,avgi,volumei,maxnormi, mgp.i,mgp.resa, mgp.resb);
}
