...
levich(ut, D) = 2.*sqrt(2.*r0*ut/D/pi)

# get the rise velocity averaged for the last 50 steps
array ut[4]
do for [i in "1 2 3 4"] {
  stats 'log' index 'case '.i u 1:4 every ::250 nooutput
  ut[i] = STATS_mean_y
}
...
set yrange [0:14]
plot for [i = 1:4]							\
     'log' index 'case '.i u 1:(sherwood(D[i])) w l t 'Axi Case '.i lt i,  \
...
@article{levich1959,
  ...
  author={Levich, Veniamin GrigorÊ¹evich},
  year={1959},
...
{	
  ...
  foreach (reduction(+:yb) reduction(+:vb) reduction(+:vbx)
    ...
    ci += c1[]*(1. - f[])/(f[]*c1.alpha_c + (1. - f[]))*dv();
    co += c1[]*f[]*c1.alpha_c/(f[]*c1.alpha_c + (1. - f[]))*dv();
