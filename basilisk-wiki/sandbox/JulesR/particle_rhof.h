/**
Antoon's stokes particle code, adapted slightly to use rhof for fluid density. This enables the particles to `feel' a variable fluid density, while the density used by the Navier-Stokes solver stays constant. Relevant when using the Boussinesq approximation.
...
extern scalar rhof;      //Fluid medium density
...
updated in this function. 
   */
  double muc, tau, itau, rhof2; //computed in `foreach_point`
  double rp = pa.u2.y;
  double rhop = pa.u2.x;
  ...
  foreach_point(xp, yp, zp, serial) {
    ...
    rhof2 = is_constant(rhof) ? constant(rhof) : interpolate_linear (point, rhof, xp, yp, zp);
  ...
  // Gravity
  foreach_dimension() 
    inp.pp->u.x += rhop ? (G.x*(rhop - rhof2)/rhop)/(idt + itau) : 0;
