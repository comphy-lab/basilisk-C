...
foreach_dimension() static double vof_concentration_gradient_x(Point point, scalar c, scalar t)
{
  ...
  if (t.inverse) cl = 1. - cl, cc = 1. - cc, cr = 1. - cr;
...
static void vof_concentration_refine(Point point, scalar s)
{
  ...
  if (cm[] == 0. || (!s.inverse && f[] <= 0.) || (s.inverse && f[] >= 1.))
    ...
    coord g;
    foreach_dimension() g.x = Delta * vof_concentration_gradient_x(point, f, s);
    double sc = s.inverse ? s[] / (1. - f[]) : s[] / f[], cmc = 4. * cm[];
    foreach_child()
    {
      s[] = sc;
      foreach_dimension() s[] += child.x * g.x * cm[-child.x] / cmc;
      s[] *= s.inverse ? 1. - f[] : f[];
    }
  }
}
...
foreach_dimension() static void sweep_x(scalar c, scalar cc, scalar *tcl)
{
  vector n[];
  scalar alpha[], flux[];
  double cfl = 0.;
  ...
  foreach_face(x, reduction(max : cfl))
  {
    ...
    if (cs[] >= 1.)
#endif
      if (un * fm.x[] * s / (cm[] + SEPS) > cfl) cfl = un * fm.x[] * s / (cm[] + SEPS);
    ...
    double cf = (c[i] <= 0. || c[i] >= 1.) ? c[i]
                                           : rectangle_fraction((coord){-s * n.x[i], n.y[i], n.z[i]}, alpha[i],
                                               (coord){-0.5, -0.5, -0.5}, (coord){s * un - 0.5, 0.5, 0.5});
    ...
    for (t, gf, tflux in tracers, gfl, tfluxl) {
      ...
      if (t.inverse) cf1 = 1. - cf1, ci = 1. - ci;
      if (ci > 1e-10) {
  ...
  if (cfl > 0.5 + 1e-6)
    fprintf(ferr, "src/vof.h:%d: warning: CFL must be <= 0.5 for VOF (cfl - 0.5 = %g)\n", LINENO, cfl - 0.5),
      fflush(ferr);
...
void vof_advection(scalar *interfaces, int i)
{
  for (scalar c in interfaces) {
    ...
    int d = 0;
    foreach_dimension() sweep[d++] = sweep_x;
