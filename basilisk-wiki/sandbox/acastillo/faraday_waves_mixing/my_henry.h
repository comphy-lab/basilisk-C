...
## ***Changes with respect to [henry.h](/src/henry.h)***
We include an exception for the limit of zero-solubility, $\alpha\to\infty$. In
this limit, the jump in concentration at the interface is given by
$$
c_1 = c
$$
$$
...
\partial_t c + \nabla\cdot(\mathbf{u} c) =
   \nabla\cdot\left(D\nabla c - D c \nabla f\right)
$$
with everything else as above. This is useful since in this limit, the
coefficient $\lim_{\alpha\to\infty} \frac{c \alpha }{\alpha f}$ may be singular.
***We also include the required terms for taking into account embedded walls.***
*/
  ...
  for (scalar c in stracers) {
    ...
    if (c.alpha < HUGE){
      ...
      */
      ...
      /**
      ***In the limit of zero solubility, $\phi_1$ and $\phi_2$ are computed as***
      $$
      \phi_1 = c
      $$
      $$
...
#if EMBED
    if (p->embed_flux) {
      double c, e = p->embed_flux (point, a, D, &c);
      n -= c*sq(Delta);
      d += e*sq(Delta);
    }
    if (!d)
...
#endif // EMBED
    int nrelax = mgc.nrelax ? mgc.nrelax : 4;
    mgc = mg_solve ({c}, {r}, h_residual, h_relax, &q, tolerance = TOLERANCE/sq(dt), nrelax=nrelax);
