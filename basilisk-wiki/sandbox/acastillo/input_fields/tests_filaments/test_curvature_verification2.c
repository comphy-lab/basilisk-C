...
We check:
1. The shape of the computed velocity and vorticity fields.
2. The divergence-free condition of the computed velocity and vorticity fields.
...
im = ax[0].tripcolor(z, x, u[:,0], cmap='magma', rasterized=True)
cs = ax[0].tricontour(z, x, u[:,0], levels=5, colors='white', linewidths=0.5)
fig.colorbar(im, ax=ax[0], label=r'$\omega_x^{(1)}(x,y,z)$')
ax[0].clabel(cs, inline=True, fontsize=10, fmt='%1.2f')
ax[0].set_title('Local x-vorticity')

im = ax[1].tripcolor(z, x, u[:,1], cmap='magma', rasterized=True)
cs = ax[1].tricontour(z, x, u[:,1], levels=5, colors='white', linewidths=0.5)
fig.colorbar(im, ax=ax[1], label=r'$\omega_y^{(1)}(x,y,z)$')
ax[1].clabel(cs, inline=True, fontsize=10, fmt='%1.2f')
ax[1].set_title('Local y-vorticity')

im = ax[2].tripcolor(z, x, u[:,2], cmap='magma', rasterized=True)
cs = ax[2].tricontour(z, x, u[:,2], levels=5, colors='white', linewidths=0.5)
fig.colorbar(im, ax=ax[2], label=r'$\omega_z^{(1)}(x,y,z)$')
ax[2].clabel(cs, inline=True, fontsize=10, fmt='%1.2f')
ax[2].set_title('Local z-vorticity')
...
plt.savefig('slice_y_field_omega1_theory.png', dpi=150, bbox_inches='tight')
plt.close()
~~~
...
# Extract the field data (Cell Data)
u_array = vtk_to_numpy(data.GetCellData().GetArray('omega.x')) 
v_array = vtk_to_numpy(data.GetCellData().GetArray('omega0.x')) 
...
int main() {
  ...
  foreach () {
    ...
    if (vortex1.near == 1) {
      ...
      foreach_dimension() {
        u0.x[] = (sol.u0_r * e_r.x) + (sol.u0_th * e_th.x) + (sol.u0_xi * vortex1.Tvec.x);
        u1.x[] = (sol.u1_r * e_r.x) + (sol.u1_th * e_th.x) + (sol.u1_xi * vortex1.Tvec.x);
        omega0.x[] = (sol.w0_r * e_r.x) + (sol.w0_th * e_th.x) + (sol.w0_xi * vortex1.Tvec.x);
        omega1.x[] = (sol.w1_r * e_r.x) + (sol.w1_th * e_th.x) + (sol.w1_xi * vortex1.Tvec.x);
      }
    }
  }
  ...
  foreach_face() {
    ...
    if (vortex1.near == 1) {
      ...
      uf.x[] += (sol.u0_r * e_r.x) + (sol.u0_th * e_th.x) + (sol.u0_xi * vortex1.Tvec.x);
      uf.x[] += (sol.u1_r * e_r.x) + (sol.u1_th * e_th.x) + (sol.u1_xi * vortex1.Tvec.x);

      omegaf.x[] += (sol.w0_r * e_r.x) + (sol.w0_th * e_th.x) + (sol.w0_xi * vortex1.Tvec.x);
      omegaf.x[] += (sol.w1_r * e_r.x) + (sol.w1_th * e_th.x) + (sol.w1_xi * vortex1.Tvec.x);
    }
  }
  boundary({uf, omegaf});
  ...
  mgstats mgpf = {0};
  mgpf = project (uf, pf);
  fprintf (stderr, "uf:   %g %d %d \n", mgpf.resa, mgpf.i, mgpf.nrelax);

  mgpf = project (omegaf, pf);
  fprintf (stderr, "omegaf: %g %d %d \n", mgpf.resa, mgpf.i, mgpf.nrelax);
  ...
  foreach() {
    foreach_dimension()  {
      u.x[] = (uf.x[1]+uf.x[])/2.;
      omega.x[] = (omegaf.x[1]+omegaf.x[])/2.;
    }
  }

  output_slice_vtu({div_u, div_omega}, {u, omega, u0, omega0, u1, omega1}, "slice_x", (coord){1,0,0}, 0);
  output_slice_vtu({div_u, div_omega}, {u, omega, u0, omega0, u1, omega1}, "slice_y", (coord){0,1,0}, 0);
  output_slice_vtu({div_u, div_omega}, {u, omega, u0, omega0, u1, omega1}, "slice_z", (coord){0,0,1}, 0);
