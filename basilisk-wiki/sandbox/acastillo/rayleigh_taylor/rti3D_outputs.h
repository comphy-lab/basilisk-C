...
event movie(t += 0.005){
  ...
  for (scalar s in{f}){
    ...
    sprintf(fname, "iso_%s_R%g_B%g_D%g_l%d.mp4", s.name, sim.Re, sim.B, sim.D, MAXLEVEL);
    save(fname);
  }
}
...
event save_profiles(t += 0.005){
  ...
  profile_foreach_region(list, xmin = -L0/2., xmax = L0/2., ymin = -L0/2., ymax = L0/2., hmin = -L0/2. + _mindel, hmax = L0/2. - _mindel, n = n, m1 = n, m2 = n);
}
...
event save_slices(t += 0.1){
  char filename[99];
  sprintf(filename, "slice_x_t%.5f", t);
  output_xmf_slice({f, p}, {u}, filename, (coord){1, 0, 0}, 0);

  sprintf(filename, "slice_y_t%.5f", t);
  output_xmf_slice({f, p}, {u}, filename, (coord){0, 1, 0}, 0);

  sprintf(filename, "slice_z_t%.5f", t);
  output_xmf_slice({f, p}, {u}, filename, (coord){0, 0, 1}, 0);
}

event save_snapshots(t += 1.0){
  char filename[99];
