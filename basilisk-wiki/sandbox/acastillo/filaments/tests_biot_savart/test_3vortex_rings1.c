...
as function of $\phi(\xi,t)$. At time $t=0$,
$$
\begin{aligned}
x_i &= R\cos(\phi), \quad
\\
...
event init (t = 0) {
  ...
  for (int i = 0; i < nseg; i++){
    phi[i] = dphi * (double)i;
    C1[i] = (coord) { R * cos(phi[i]), R * sin(phi[i]), -2*d};
    C2[i] = (coord) { R * cos(phi[i]), R * sin(phi[i]), -1*d};
    C3[i] = (coord) { R * cos(phi[i]), R * sin(phi[i]),  0.};
    
    vol1[i] = pi * sq(a) * R * dphi;
    vol2[i] = pi * sq(a) * R * dphi;
    vol3[i] = pi * sq(a) * R * dphi;
...
event evaluate_velocity (i++) {  
  ...
  for (int j = 0; j < nseg; j++) {
    ...
    ring3.Uauto[j] = nonlocal_induced_velocity(ring3.C[j], &ring3, Gamma=1.0);

    coord U12 = nonlocal_induced_velocity(ring1.C[j], &ring2, Gamma=1.0);
    coord U13 = nonlocal_induced_velocity(ring1.C[j], &ring3, Gamma=1.0);
    coord U21 = nonlocal_induced_velocity(ring2.C[j], &ring1, Gamma=1.0);
    coord U23 = nonlocal_induced_velocity(ring2.C[j], &ring3, Gamma=1.0);
    coord U31 = nonlocal_induced_velocity(ring3.C[j], &ring1, Gamma=1.0);
    coord U32 = nonlocal_induced_velocity(ring3.C[j], &ring2, Gamma=1.0);
...
event slice (t += 5.0){  
  foreach(){
    ...
    coord u3_BS = nonlocal_induced_velocity(p, &ring3, Gamma=1.0);
    foreach_dimension(){
