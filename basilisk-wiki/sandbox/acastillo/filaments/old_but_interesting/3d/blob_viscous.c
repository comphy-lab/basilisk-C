...
The evolution of a blob of vorticity is simulated as in [../2d/blob_viscous.c]()
...
int main(){
  ...
  periodic(left);
  periodic(front);
  const face vector muc[] = {1./100,1./100,1./100};
  mu = muc;
  run();
}
...
event snapshots (t += 2.5) {
  ...
  static int nf = 0;
  char name[80];
  sprintf(name, "blob_viscous_%3.3d", nf);
