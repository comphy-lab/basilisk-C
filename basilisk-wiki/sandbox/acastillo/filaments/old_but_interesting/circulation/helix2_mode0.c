...
void helical_pair_filaments(vector omega, char * name){
  ...
  foreach(){
    coord pcar = {x,y,z};
    coord val_omega = get_vorticity_filament(pcar, n_seg, as/1.36, t0, c, tvec, nvec, bvec, 0);
    foreach_dimension()
      omega.x[] += val_omega.x;
  }
}
...
event adapt (i++){
  adapt_wavelet ((scalar*){u}, (double[]){1e-2,1e-2,1e-2}, MAXLEVEL, MINLEVEL);
}
...
event init (t = 0){
  ...
  helical_pair_filaments(b, "../mode0_hext.asc");
  helical_pair_filaments(b, "../mode0_hint.asc");
