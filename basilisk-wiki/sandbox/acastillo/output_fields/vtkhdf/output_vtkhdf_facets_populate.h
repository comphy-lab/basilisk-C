...
/** ### Populate points_dset for facets in vtkhdf, within a box (interleaved) */
trace
void populate_points_dset_facets_vtkhdf_box(scalar c, coord box[2], double **points_dset, long num_points, long *offset_points, hsize_t *count, hsize_t *offset) {
  // Each process defines dataset in memory and writes to an hyperslab
  count[0] = num_points;
  count[1] = 3;
  offset[0] = 0;
...
/** ### Populate offsets_dset for facets within a box (cumulative points per cell) */
void populate_offsets_dset_facets_vtkhdf_box(scalar c, coord box[2], long **offsets_dset, long num_cells_loc, long *offset_offset, hsize_t *count, hsize_t *offset) {
  count[0] = num_cells_loc + 1; // write N+1 values per process
  count[1] = 1;
  offset[0] = 0;
  offset[1] = 0;
...
/** ### Populate topo_dset (Connectivity) for facets within a box */
trace
void populate_topo_dset_facets_vtkhdf_box(scalar c, coord box[2], long **topo_dset, long num_points_loc, long *offset_points, hsize_t *count, hsize_t *offset) {
  // Connectivity length is exactly the number of points (since each point is unique)
  count[0] = num_points_loc;
  count[1] = 1;
  offset[0] = 0;
...
/** ### Populate scalar_dset for facets, within a box */
trace
void populate_scalar_dset_facets_vtkhdf_box(scalar c, coord box[2], scalar s, double *scalar_dset, long num_cells, long *offset_cells, hsize_t *count, hsize_t *offset) {
  count[0] = num_cells;
  count[1] = 1;
  offset[0] = 0;
  offset[1] = 0;
