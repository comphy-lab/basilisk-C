...
double reference_height(scalar yref, scalar f, scalar c, double cmin=0, double cmax=1, bool store=true, double H0=L0, bool reverse=true){
  ...
  /* Get the Cumulative distribution function of c. p_pdf isn't used here,
     but probability_distribution_1D_weighted() always computes it, so it
     needs a backing array -- heap-allocated since it's discarded right
     after. */
  double p_range[NBIN], p_sum[NBIN];
  double * p_pdf = malloc(NBIN*sizeof(double));
  probability_distribution_1D_weighted(f, c, cmin, cmax, NBIN, p_range,
                                        p_pdf, p_sum,
