#ifndef OUTPUT_XDMF_FACETS_LIST_H
#define OUTPUT_XDMF_FACETS_LIST_H

#include "acastillo/output_fields/output_common_helpers_facets.h"
#include "output_xdmf_facets_populate.h"

/** 
## output_facets_xmf_list(): Exports the VOF interface with multiple scalar fields.
...
Multiple scalar fields defined on the interface can be written to the same file.
...
trace
void output_facets_xmf_list(scalar c, scalar * slist, char *subname, int mode = HDF5_CHUNKED, int compression_level = 6){
#ifdef HAVE_HDF5
  ...
  // Allocate memory once and write one dataset per field
  ...
  if (!warning_printed && pid() == 0) {
    fprintf(stderr, "Warning: output_facets_xmf_list() called but HDF5 is not available. Output skipped.\n");
    warning_printed = 1;
  }
#endif
}

#endif // OUTPUT_XDMF_FACETS_LIST_H
