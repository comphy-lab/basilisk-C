...
void careful_refinement(){
  for(int ii = MINLEVEL ; ii <= MAXLEVEL; ii++){
    refine(level < ii /*&& sqrt(sq(x) + sq(y)) > (Rbub - 4.*sqrt(2.)*lambda/(1<<(ii-1)))*/ && sqrt(sq(x+bubcenter) + sq(y)) < (2.5*Rbub + 4.*sqrt(2.)*lambda/(1<<(ii-1))));
  }
}
  ...
  if (!restore (file = "restart"))
    {
      ...
      vertex scalar phi[];
      foreach_vertex() {
	phi[] = sq(x+bubcenter) + sq(y) - Rbub*Rbub;
      }
      boundary ({phi});
      fractions (phi, f);
      boundary({f});
	...
	double pL = pinf*(1. - Rbub/sqrt(sq(x+bubcenter) + sq(y))) + (pg0 - 2*f.sigma/Rbub/Rbub)*Rbub/sqrt(sq(x+bubcenter) + sq(y));
	double pg = pg0;
...
<source src="nearwall/bubble.mp4" type="video/mp4">
</video>
</center>
