...
This code comes as a complement for
[leeway_redistribution.c](leeway_redistribution.c) to show that advection scheme
defined in [vof.h](/src/vof.h) does not advect correctly a tracer with respect 
to an interfacial velocity.
...
field](leeway_classic/video_solute.mp4)
*/
...
during the advection, instead of being just advected perpendicularly to it.
The lateral displacement was around 5% of the advection velocity and depended
on the tilt of the interface. The variable *mytheta* define this tilt. */
...
event vof (i++) {
  ...
  /**
  We advect solute as f.tracer, see [vof.h](/src/vof.h). */
...
event outputs (t = 0.; t += max(T_END/100., DT_MAX); t <= T_END) {
  ...
  fprintf (fp, "%.17g %.17g %.17g %.17g %.17g\n", t, initial_solute_mass,
           total_solute, drop_mass, leeway-initial_tangential_position);
  fflush(fp);
  #if VIDEO
    /**
    solute */
...
The conservation of the solute seems good, but the patch moves along the
interface with a velocity of 5% of the imposed normal velocity. These results
have to be compared with the ones of
[leeway_redistribution.c](leeway_redistribution.c) where the functions of
redistribution of the solutes, defined in [mixtures.h](../mixtures.h), are used.
*/