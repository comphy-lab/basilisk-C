...
This file proposes a minor change to fractions.h to correct a small bug in the
definition of the surface fraction if the input Level-Set function is 0 exactly
at a cell border. [fraction_mwe.c](/sandbox/qmagdelaine/geometry/fraction_mwe.c)
gives a minimal working example of the problem. The only change is at the line
275 (line 268 in the original file). I am not sure my correction is good, see
...
void fractions (struct Fractions a)
{
...
#endif
  {
	...
	foreach_dimension(2)
      ...
      /**
      In the line above, I replaced $p.x[]$ with $max(p.x[], p.y[])$. */
    }
  }
