...
![Pressure field](contact_line/pressure.mp4)

![Horizontal velocity field. We see some
spurious currents.](contact_line/u_x.mp4)
...
/**
In this example, we use [integral_vof_quasi1D.h](integral_vof_quasi1D.h),
which extrapolates the interfaces as straight lines. With this implementation,
some spurious currents remain at the end (see the plots at the end or the video
above), but with a interpolation of the interface using circle, we obtain even
better results than with tension.h, see [contact_circle.c](contact_circle.c).
However, this implementation using circles is just a first try: a lot of problem
remains, notably it is not possible yet to have straight interface for the
moment. */
...
event movie (t = 0.; t += DELTA_T) {
  ...
  clear();
  draw_vof("f", edges = true, lw = 2., lc = {DG, DG, DG}, filled = 0);
  squares ("u.x", min = -4e-5, max = 4e-5, linear = false, map = cool_warm);
  draw_string(legend, 1, size = 30., lw = 2.);
  save ("u_x.mp4");
}
#endif
...
integral formulation seem be a little bit better.
...
How ever with this formulation of the integral fomulation, some spurius current
remain. We can suppress them by extrapolating the interface pieces with circle
arcs, as in [contact_circle.c](contact_circle.c). */
