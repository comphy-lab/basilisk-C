...
tension, we extrapolate the interface in each cell as a straight line. For a
more accurate yet more incomplete version see
[integral_vof_quasi1D_circle.h](integral_vof_quasi1D_circle.h).
...
/**
We should do a reduction operation to make it work in parallel, but adding the
reduction make the code crash.
...
/**
We define the functions which will be used to ensure the contact angles on
the boundaries. */

#define LINEAR_CURVATURE 0 // set to 1 to use linear interpolation of curvature

double Sxx_boundary (Point point, scalar hy, scalar kappa,
                     scalar sigma, double theta_c, bool left) {
  ...
  if (hy[ghost_Q] < 0.5 && hy[ghost_Q] >= -0.5) {
    ...
    double values[] = {kappa[ghost_Q], kappa[ghost_Q, l]};
    ...
    * "g" for in the gosth cell.
    ...
    coord tangentb = {sin(theta_c), left*cos(theta_c)};
    coord tangenti = tangent_center (point, hy);
    coord tangentg;
    foreach_dimension()
    ...
    double sigmag = sigma[];
    #else
    double sigma_values[] = {sigma[ghost_Q], coarse(sigma, ghost_Q, l)};
    double sigmag = do_average (sigma_values, weights, 2);
    sigmag = (sigmag == nodata ? coarse(sigma, ghost_Q, -l) : sigmag);
    #endif
...
/**
We define the prolongation functions for Sxx and Syy. We will need pointer to
a scalar defined as a global variable to use these functions (more details will
be given in the acceleration event below). */
...
event init(i = 0) {
  for (scalar c in interfaces) { 
    if (c.height.x.i) {
      ...
      vector h = c.height;
      if (c.theta_left)
        h.t[left] = contact_angle (_c.theta_left);
      if (c.theta_right)
        h.t[right] = contact_angle (_c.theta_right);
    }
  }
}
  ...
  for (scalar c in interfaces) {
    if (c.sigma.i) {
      ...
      tensor S[];
        ...
        for (int l = -1; l <= 1; l += 2) {
          if (hy[]*(hy[] + hy[l]) <= 0. && (l < 0. || hy[] + hy[l] != 0.) &&
              hy[] != nodata && hy[l] != nodata && hy[] != hy[l]) {
            Syy_defined = true;
            double xi = hy[]/(hy[] - hy[l]); // > 0
            double weights[] = {1. - xi, xi};
#if LINEAR_CURVATURE // does not make much difference
...
#endif
            coord principal_tangent = tangent_left(neighborp((1 + l)/2), hy);
            coord other_tangent = tangent_left(neighborp((1 - l)/2), hy);
            coord tangenti;
      ...
      if (c.theta_left) {
        scalar s = S.x.x;
        s[left] = Sxx_boundary (point, _c.height.y, _c.kappa, _c.sigma,
                                _c.theta_left, true);
      }
      if (c.theta_right) {
        scalar s = S.x.x;
        s[right] = Sxx_boundary (point, _c.height.y, _c.kappa, _c.sigma,
                                 _c.theta_right, false);
      }

      #if TREE
        for (scalar s in {S}) {
          s.refine = s.prolongation = curvature_prolongation;
          s.restriction = curvature_restriction;
        }
        scalar s = S.x.x;
        s.refine = s.prolongation = stress_prolongation_x;
        s = S.y.y;
        s.refine = s.prolongation = stress_prolongation_y;
      #endif
      ...
      foreach_vertex() {
        ...
        if (hy[] + hy[-1] <= 0. && hy[] + hy[-1] > -2.) {
          double hyv = (hy[] + hy[-1, 0])/2. + 0.5;
                         
          double values[] = {sigma[-1], sigma[], sigma[-1, -1], sigma[0, -1]};
          double weights[] = {(0.5 + hyv)/2., (0.5 + hyv)/2.,
                              (0.5 - hyv)/2., (0.5 - hyv)/2.};
          double sigmai = do_average (values, weights, 4);
          
          coord tangenti = tangent_left(point, hy);
        ...
        if (((hy[] + 0.5)*(hy[-1] + 0.5) < 0. || hy[] == -0.5)
             && hy[] != nodata && hy[-1] != nodata) {
          double hyv = (hy[] + hy[-1])/2. + 0.5;
          double xi = - hyv/(hy[] - hy[-1]);
          ...
          coord principal_tangent = tangent_left (point, hy);
          coord other_tangent = tangent_left (neighborp(l), hy);
          coord tangenti;
