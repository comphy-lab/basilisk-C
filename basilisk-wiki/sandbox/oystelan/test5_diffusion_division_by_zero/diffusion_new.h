...
{
  /**
  Only apply diffusion if the full column is wet enough.
  We require both a non-dry total depth and all layers to be thicker than dry.
  */
  double hsum = 0., hmin = HUGE;
  foreach_layer() {
    hsum += h[];
    hmin = min (hmin, h[]);
  }
  if (hsum <= dry || hmin <= dry)
    return;
  ...
  */
  ...
  if (nl == 1) {
    ...
    /**
    We use a reduced one-layer implicit update with top and bottom forcing.
    */
    b[0] = h0 + 2.*dt*D/h0;
    rhs[0] += D*dt*dst;
    ...
    double h0 = max (h[], dry);
    double h1 = max (h[0,0,1], dry);
    double h01 = max (h0 + h1, dry);

    double den = h0*sq(h01) 
    ...
    */
    ...
    c[0] = - 2.*dt*D*(1./h01 + sq(h0)/den);
    rhs[0] += 2.*dt*D*s_b*(sq(h1) + 3.*h0*h1 + 2.*sq(h0))/den;
  }
  ...
  for (int l = 1; l < nl; l++) {
    /**
    Protect against zero or extremely small pivots.
    */
    if (fabs (b[l-1]) <= dry)
      b[l-1] = (b[l-1] >= 0. ? dry : -dry);
...
event viscous_term (i++,last)
{
  if (nu > 0.) {
    foreach() {
        /**
      Only apply viscosity if the full column is wet enough.
      */
      double hsum = 0., hmin = HUGE;
      foreach_layer() {
        hsum += h[];
