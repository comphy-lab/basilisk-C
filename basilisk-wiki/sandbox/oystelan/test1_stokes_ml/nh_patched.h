...
~~~
  ...
  foreach (reduction(+:h1) reduction(+:v1)) {
    ...
    foreach_layer() {
	  ...
	  u.x[]*slope_limited((dz.x + (fm.x[1]*(h[1] + h[]) - 
    fm.x[]*(h[-1] + h[]))/2.)/(Delta*cm[]));
      if (point.l > 0)
	foreach_dimension()
	...
	rhs[] += 4.*s*w[0,0,k];
      rhs[] *= 2.*h[]/(theta_H*dt);
      foreach_dimension()
	dz.x += (fm.x[1]*(h[1] + h[]) - fm.x[]*(h[-1] + h[]))/2.;
      h1 += dv()*h[];
      v1 += dv();
    }
  }
  ...
  /** We optionally some relaxation, to avoid buildup of resonant modes */
  if (theta_R > 0.){
    foreach()
    foreach_layer()
      phi[] = (1.-theta_R)*phi[] + theta_R*phi_prev[];
    delete ({phi_prev});
  }
