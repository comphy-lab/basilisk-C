...
scalar zb[], eta, h;
vector u;
double G = 1., dry = 1e-12, CFL_H = 1e40, h_lim = 1e-6;
...
event face_fields (i++, last)
{
  ...
  #if IMPLICIT
	double un = (1.-theta_H)*pdt*hu.x[]/Delta, a = sign(un);
  #else
...
/** 
Function which updates face centered heights for a specified dt.
Needed for consistensy when momentum advection is used since advection
is done twice (halfsteps).
 */
void update_hf (face vector hf, double dt)
{
  foreach_face (reduction (min:dtmax)) {
    double Hr = 0., Hl = 0.;
    foreach_layer() {
      ...
      /**
      The face velocity is computed as the height-weighted average of
      the cell velocities. A minimum cell height limiter is introduced
      to prevent numerical noise in the velocity field due to division
      by very small numbers.*/
      
      double hl = h[-1] > dry ? h[-1] : 0.;
      double hr = h[] > dry ? h[] : 0.;
      double utemp = hl > 0. || hr > 0. ? (hl*u.x[-1] + hr*u.x[])/(hl + hr) : 0.;
      ...
      if (Hl <= dry)
        ...
        In the default case, the face height is computed using a variant of the
        [BCG](/src/bcg.h) scheme. */
        double un = dt*utemp/Delta, a = sign(un);
        int i = - (a + 1.)/2.;
        double g = h.gradient ? h.gradient (h[i-1], h[i], h[i+1])/Delta :
          (h[i+1] - h[i-1])/(2.*Delta);
        hff = h[i] + a*(1. - a*un)*g*Delta/2.;
...
event pressure (i++, last)
{
  ...
  #if IMPLICIT
  /** When using the implicit solver we update hf the remaining halfstep */
  update_hf(hf,theta*dt);
  #endif
