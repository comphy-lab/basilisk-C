/**
	Single force-free cylindrical particle between parallel plates.
	Sedimenting towards a porous surface (Darcy-like wall)
*/
double RADIUS = 0.25; // cylinder radius
double HEIGHT = 100.0; // channel height
...
#define cmax (1.e-6) // Absolute refinement criteria for the velocity field
										 //
double P_ref = 1.0; // 
double KAPPA = -0.01; // this is the _PERMEABILITY_ of the lower wall.
...
int main ()
{
  ...
  display_control(lmax,1,9);
	...
	display_control(P_ref,-1e4,1e4);
	display_control(KAPPA,-1e4,1e4);
...
//  mask (y < -HEIGHT/2. ? bottom : none);
  ...
  u.t[left]  = dirichlet(0.);
    p[left]  = dirichlet(0.);//neumann(0.);
  u.n[right] = dirichlet(0.);
  u.t[right] = dirichlet(0.);
    p[right] = dirichlet(0.);//neumann(0.);

  u.n[top]  = dirichlet(0.);
  u.t[top]  = dirichlet(0.);
    p[top]  = dirichlet(0.);//neumann(0.);
														//
  u.n[bottom] = dirichlet(KAPPA*p[]); // lower wall is modelled as a simple porous medium
...
![Sedimentation of a 2D confined cylinder, getting closer to a wall. Stokes regime](sedimentation_driver_moving_darcy_wall/output.mp4)(loop)
*/