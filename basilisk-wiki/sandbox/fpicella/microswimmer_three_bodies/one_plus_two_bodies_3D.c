...
*/
double RADIUS = 1.0; // cylinder radius
...
int lmin = 1;  // Min mesh refinement level (l=6 is 2pt/d)
int lmax = 7; // Max mesh refinement level (l=10 is 36pt/d, l=13 is 292pt/d)
#define cmax (1.0e0) // Absolute refinement criteria for the velocity field
...
#define DT_STOKES 1e-2 // (pseudo-)timestep for the Stokes solver
#define MAXIT_STOKES 1 // maximum number of subiterations for Stokes solver
...
//	display_control(pTHETA,-10.,+10.);
  ...
  We initialize the grid. */
	/**
	Impose maximum _viable_ resolution here, for startup only.*/
...
{
//	/**
//	As done by Arthur, use third-order face interpolation.*/
//#if ORDER2
//  for (scalar s in {u, p})
//    s.third = false;
//#else
...
//#endif // ORDER2
  ...
  /**
	...
	foreach_particle(){
		...
		p().thrust  = 0.;
		p().theta.z = pTHETA;
		p().alpha   = 100.0*p().r; // particle-center--force center distance.
		p().beta    = +100.; // puller--neutra--pusher.
	}
/**
Super-fancy configuration, three particles _as a puller!_ .*/
	foreach_particle(){
		
		if(_j_particle == 0){
		}
		if(_j_particle == 1){
			p().x = p().alpha*cos(+atan(p().beta)+p().theta.z);
			p().y = p().alpha*sin(+atan(p().beta)+p().theta.z);
			p().B.y /=-2.;
		}
		if(_j_particle == 2){
			p().x = p().alpha*cos(-atan(p().beta)+p().theta.z);
...
//  mask (x < -HEIGHT/2. ? left : none);
  ...
  uf.t[top]    = dirichlet(FLOW);
  uf.n[bottom] = dirichlet(0.);
  uf.t[bottom] = dirichlet(FLOW);
  uf.t[right]  = dirichlet(0.);
  uf.n[right]  = dirichlet(FLOW);
  uf.n[left]   = dirichlet(FLOW);
...
//	uf.t[embed] = dirichlet(1.); // pOmg.x = pOmg.y
#if dimension == 3
	u.r[embed]  = dirichlet(velocity_noslip_z(point,x,y,z));
	uf.r[embed] = dirichlet(velocity_noslip_z(point,x,y,z));
...
event logfile (t += dt; i <= 1000) {
	// Dummy output
	foreach_particle(){
		if(_j_particle==0){
		fprintf(stderr,"PARTICLE 0 %+6.5e %+6.5e %+6.5e %+6.5e %+6.5e %+6.5e \n",p().u.x,p().u.y,p().u.z,p().F.x,p().F.y,p().F.z);
		...
		if(_j_particle==1)
		fprintf(stderr,"PARTICLE 1 %+6.5e %+6.5e %+6.5e %+6.5e %+6.5e %+6.5e \n",p().u.x,p().u.y,p().u.z,p().F.x,p().F.y,p().F.z);
		if(_j_particle==2)
		fprintf(stderr,"PARTICLE 2 %+6.5e %+6.5e %+6.5e %+6.5e %+6.5e %+6.5e \n",p().u.x,p().u.y,p().u.z,p().F.x,p().F.y,p().F.z);
	}
}
...
event end_timestep(i++){
	...
	while (j<MAXIT_STOKES){
		fprintf(stderr,"STOKES %04d over %04d\n",j,MAXIT_STOKES);
		/**
		...
		/**
		Debugging, rotation is still not perfectly ok...*/
		// Set it to zero to start with.
		foreach_particle()
			foreach_dimension()
				p().T.x = 0.;
		/**
