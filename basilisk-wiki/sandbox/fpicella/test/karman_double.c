...
/**
Modified so to account for 2 cylinders...
using RBM color
 */
...
// how to shift the second cylinder...
double x_shift = + 0.25;
double y_shift = - 0.25;
scalar cs1[];
scalar cs2[];
...
int main()
{
  ...
  output_file = fopen ("cylinders_forces.dat", "w");
...
event init (t = 0)
{
  ...
  solid (cs, fs, intersection (
								 intersection (0.5 - y, 0.5 + y), // outer domain
			           intersection (sqrt(sq(x) + sq(y)) - D/2., // first cylinder
															 sqrt(sq(x-x_shift)+sq(y-y_shift))-D/2. // second cylinder
								 )
								 )
			  );
	/**
	Definition of cs and fs fractions...for each cylinder
	*/
	solid(cs1,fs1,sqrt(sq(x-x_shift*0.)+sq(y-y_shift*0.))-D/2.); // first cylinder
	solid(cs2,fs2,sqrt(sq(x-x_shift*1.)+sq(y-y_shift*1.))-D/2.); // first cylinder
...
event compute_forces(i++){
	coord Fp1,Fm1,Fp2,Fm2;
	coord Omega = {0., 0., 0.};
	double RADIUS = D/2.;
	coord center1 = {0.,0.,0.};
	coord center2 = {x_shift,y_shift,0.};
	embed_color_force_RBM(p,u,mu,cs1,center1,Omega,RADIUS,&Fp1,&Fm1);
	embed_color_force_RBM(p,u,mu,cs2,center2,Omega,RADIUS,&Fp2,&Fm2);

	fprintf(output_file,"%04d %+6.5e %+6.5e %+6.5e %+6.5e %+6.5e\n",
				 i, t, Fp1.x, Fm1.x, Fp2.x, Fm2.x
			   );
	fflush(output_file);
}
