/**
This is an adaptation of ghigo/src/myviscosity.h, so to account for embedded boundaries AND variable viscosity.
**/
#include "poisson.h"
...
//      embed_stress_flux (point, u, mu, &c, &d);
	if (embed_flux)
		foreach_dimension()
			d.x = embed_flux ? embed_flux (point, u.x, mu, &c.x) : 0.;
#endif // EMBED
    foreach_dimension() {
