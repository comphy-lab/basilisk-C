...
*/
double RADIUS = 0.1; // cylinder radius
double HEIGHT = 1.0; // channel height
double nu     = 1.0; // fluid viscosity;
double pTHETA  = M_PI/2*1.; // swimmer's angle
...
event end_timestep(i++){
	...
	foreach_particle()
		foreach_dimension()
			p().u2.x = p().u.x;
	/**
	Are-there any repulsion forces to account for? */
	foreach_particle()
		foreach_dimension()
			p().R.x = 0.;
	//compute_top_wall_repulsion();
	//compute_right_wall_repulsion();
	//compute_left_wall_repulsion();
	//compute_bottom_wall_repulsion();
	//compute_particle_particle_repulsion();
