...
event end_timestep(i++){
	/**
		nakayama yamamoto 2005 style. */
	compute_sp(microswimmers);
	/**
		Compute RBM correction. */
	coord G = {1.,0.};
	for(int j = 0; j<100; j++){
	foreach()
		foreach_dimension()
			phiupu.x[] = sp[]*(G.x-u.x[]);
	foreach_face()
		phiupuf.x[] = face_value(phiupu.x,0);

	 mgp = project (phiupuf, pp, alpha, dt, mgp.nrelax);
	foreach()
		foreach_dimension()
			phifp.x[] = phiupu.x[]/dt - center_gradient(pp);
	/**
		Add RBM correction. */	
	foreach()
		foreach_dimension()
