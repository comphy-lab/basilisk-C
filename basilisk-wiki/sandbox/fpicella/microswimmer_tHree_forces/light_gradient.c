...
### Sensibility to a non-uniform field
(e.g. light gradient?)

Reorientation owing to imposed field is computed following
equation 5 in [Dervaux et a. 2017](https://doi.org/10.1038/nphys3926)

![Microswimmers attired by a light source](light_gradient/light_gradient.mp4)(loop)
*/
...
/**
	Radius and width of the light donout.*/
double r0 = 10.;
double sigma = 2.0;
double B_light = 2.5; // reorientation owing to light gradient
...
/**
Gradient-driven reorientation.*/
scalar  light[];
vector glight[]; // light gradient
...
double NP = 4.; // number of agents I want to work with...
...
/**
  ...
  //mask (x < -HEIGHT/2. ? left : none);
	...
	//* Initialize the light intensity field.*/
	foreach()
    light[]= exp(-(sq(sqrt(sq(x) + sq(y)) - r0)) / (2*sq(sigma)) );
	//* Compute the gradient of the light field.*/
	gradients ((scalar *) {light}, (vector *) {glight});

}


event log_make_video (t += 2.5) {
  squares ("light", linear = true);
...
event end_timestep(i++){
	...
	foreach_particle(){
		...
		/** Evolve microswimmer orientation.
				For the moment, I use a simple Explicit Euler.*/
		p().theta += p().omega*dt;
	}
}