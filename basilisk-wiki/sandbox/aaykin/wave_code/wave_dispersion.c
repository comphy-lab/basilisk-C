...
# **Wave**
## Set up for the code 
* Download [bash.sh](/sandbox/aaykin/wave_code/bash.sh) and wave.c
* Run:. */
...
#define multiple_run_nl 1
#define multiple_run_KH 1
...
int t_end=2,LEVEL=8; //t_end: Final time of the simulation, LEVEL: Linked to the number such as POINT=2^LEVEL; Defines the number of points 
	...
	for (CAM = pow(10,-4)*5 ; CAM <=  pow(10,-2); CAM *= 1.2)
#endif
	  {
	    ...
	    for (LEVEL = 1; LEVEL <= 10; LEVEL += 1)
	      ...
	      {
#if multiple_run_KH
		for (KH = 0.1 ; KH <=  100; KH += (KH>=3)?10:0.1)
#endif
		  {
#if multiple_run_RD
...
/**
~~~gnuplot Dispersion
set xlabel 'k H' font "Times-Roman,20"
set ylabel 'C/C_e' font "Times-Roman,20"
set logscale x
set logscale y
set grid
set yr [0.96:1.04]
...
plot  omega1_keller(x)/sqrt(tanh(x)) t 'Analytical; 1 layer', \
      omega2_keller(x/2.,x/2.)/sqrt(tanh(x)) t 'Analytical; 2 layer', \
      omega3_keller(x/3.,x/3.,x/3.)/sqrt(tanh(x)) t 'Analytical; 3 layer', \
      omega4_keller(x/4.,x/4.,x/4.,x/4.)/sqrt(tanh(x)) t 'Analytical; 4 layer', \
      omega5_keller(x/5.,x/5.,x/5.,x/5.,x/5.)/sqrt(tanh(x)) t 'Analytical; 5 layer', \
      for [i in nl] "phase.txt" using 7:($10==i?(2*pi)/($2*sqrt(9.81*$8*tanh($7))):1/0) w p t sprintf("Numerical; %s layer",i)
~~~
*/
