/**
# Friction in saint-venant : Torrential test case with Poiseuille

In this example, we reproduce the MacDonald benchmark$^{[1]}$. We test
the friction source term in a torrential flow. The leading equations can
be written as : 

$$ 
\begin{array}{cc} \partial_t h + \partial_x q = 0
\\ \partial_t q + \partial_x \left(\frac{q^2}{h}+ \frac{1}{2} g h^2
\right) = -gh (S_0 + S_f) \end{array} 
$$ 

$S_0$ is the slope and $S_f$ is the friction term in its kinematic
form.*/
/**   
## Declarations

We call the saint venant solver and we add the Poiseuille friction term.*/
#include "grid/cartesian1D.h"
#include "saint-venant.h"
#include "saintvenant/poiseuille.h"
...
int ne = 0;
double pause, tmax, q0 = 6, z0, h0, tb = 400 ;
...
// Poiseuille friction term in kinematic formulation
double sf(double x){
  return  -3*nu/G*q0/pow(hex(x),3);
}
...
event error (i++; t<=tmax) {
  ...
  if(t > tb){
   ...
   if( N == 256){
    static FILE * fp1 = fopen("ErrorN256.dat","w");
...
*/
/**
Print the water profile along the channel at different time
*/
...
/**
## Results
![Error convergence](/tor_pois_error.png)

![Error time](/tor_pois_errt.png)

![Water depth profiles](/tor_pois_height.png)
