/**
# Friction in saint-venant : Torrential test case with Darcy at first order

In this example, we reproduce the MacDonald benchmark$^{[1]}$. We test
the friction source term in a torrential flow treated at first
order. The leading equations can be written as :
...
We call the saint venant solver without friction term.*/
...
int ne = 0;
double pause, tmax, f, q0 = 2.5, z0, h0, tb = 400 ;
...
We compute the source term at order 1 implicitly
 */
event friction (i++)
{
  foreach(){
...
![Error convergence](/tor_darcy0_error.png)

![Error time](/tor_darcy0_errt.png)

![Water depth profile](/tor_darcy0_height.png)
