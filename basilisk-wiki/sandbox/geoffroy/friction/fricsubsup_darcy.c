...
In this example, we reproduce the MacDonald benchmark[1]. We test the
friction source term for a fluvial flow becoming torrential. The
...
double hex(double x){
  if(x < 500)
    ...
    return pow(4/G,1/3.)*(1 - 1/6.*tanh(6*(-0.5 + x/1000.)));
}
double dhex(double x){
  if(x < 500)
    return -pow(4/G,1/3.)/(1000*sq(cosh(3*(-0.5 + x/1000.))));
  else
    return -pow(4/G,1/3.)/(1000*sq(cosh(6*(-0.5 + x/1000.))));
}
...
![Error convergence](/subsup_darcy_eror.png)

![Error versus time](/subsup_daryc_errt.png)

![Water depth profiles](/subsup_darcy_height.png)
