    ...
    if [[ -d "$PATCHES_DIR" ]]; then
        ...
        if [[ ${#patch_files[@]} -eq 0 ]]; then
            ...
            for patch_file in "${patch_files[@]}"; do
                ...
                local skip_reason
                if skip_reason="$(comphy_patch_skip_reason "$patch_name" "$LOCAL_BVIEW" "$COMPHY_BVIEW")"; then
        ...
        if [[ -z "$PATCH_FILES" ]]; then
            ...
            while read -r patch_file; do
                if [[ -n "$patch_file" ]]; then
                    local skip_reason
                    if skip_reason="$(comphy_patch_skip_reason "$patch_file" "$LOCAL_BVIEW" "$COMPHY_BVIEW")"; then
